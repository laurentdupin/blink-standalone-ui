...
CoreProbeSink::~CoreProbeSink() {
  ...
  if (HasInspectorWebMCPAgents() && --s_numSinksWithInspectorWebMCPAgent == 0)
    s_existingAgents &= ~kInspectorWebMCPAgent;
...
// static
unsigned CoreProbeSink::s_numSinksWithInspectorWebMCPAgent = 0;

void CoreProbeSink::AddInspectorWebMCPAgent(InspectorWebMCPAgent* agent) {
  bool already_had_agent = HasInspectorWebMCPAgents();
  inspector_web_mcp_agents_.AddAgent(agent);

  if (!already_had_agent) {
    base::AutoLock locker(AgentCountLock());
    if (++s_numSinksWithInspectorWebMCPAgent == 1)
      s_existingAgents |= kInspectorWebMCPAgent;
  }

  DCHECK(HasAgentsGlobal(kInspectorWebMCPAgent));
}

void CoreProbeSink::RemoveInspectorWebMCPAgent(InspectorWebMCPAgent* agent) {
  if (!HasInspectorWebMCPAgents())
    return;

  inspector_web_mcp_agents_.RemoveAgent(agent);

  if (!HasInspectorWebMCPAgents()) {
    base::AutoLock locker(AgentCountLock());
    if (--s_numSinksWithInspectorWebMCPAgent == 0)
      s_existingAgents &= ~kInspectorWebMCPAgent;
  }
...
void CoreProbeSink::Trace(Visitor* visitor) const
{
  ...
  visitor->Trace(inspector_web_mcp_agents_);
...
void DidClearDocumentOfWindowObjectImpl(LocalFrame* param_local_frame) {
  ...
  if (probe_sink->HasInspectorAnimationAgents()) {
    probe_sink->InspectorAnimationAgents().ForEachAgent([&](InspectorAnimationAgent* agent) {
...
void DidCreateMainWorldContextImpl(LocalFrame* param_local_frame) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->DidCreateMainWorldContext(param_local_frame);
    });
  }
}
...
void WillInsertDOMNodeImpl(Node* parent) {
  ...
  if (probe_sink->HasInspectorDOMDebuggerAgents()) {
    probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) {
...
void DidInsertDOMNodeImpl(Node* param_node) {
  ...
  if (probe_sink->HasInspectorDOMSnapshotAgents()) {
    probe_sink->InspectorDOMSnapshotAgents().ForEachAgent([&](InspectorDOMSnapshotAgent* agent) {
      agent->DidInsertDOMNode(param_node);
    });
  }
}
...
void DidModifyAdoptedStyleSheetsImpl(Node* param_node) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_node);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->DidModifyAdoptedStyleSheets(param_node);
    });
  }
}
...
void DidInvalidateStyleAttrImpl(Element* param_element) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_element);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorCSSAgents()) {
    probe_sink->InspectorCSSAgents().ForEachAgent([&](InspectorCSSAgent* agent) {
      agent->DidInvalidateStyleAttr(param_element);
    });
  }
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
...
void DidCreateCanvasContextImpl(Document* param_document) {
  ...
  if (probe_sink->HasInspectorEventBreakpointsAgents()) {
    probe_sink->InspectorEventBreakpointsAgents().ForEachAgent([&](InspectorEventBreakpointsAgent* agent) {
...
void DidCreateOffscreenCanvasContextImpl(OffscreenCanvas* param_offscreen_canvas) {
  ...
  if (probe_sink->HasInspectorEventBreakpointsAgents()) {
    probe_sink->InspectorEventBreakpointsAgents().ForEachAgent([&](InspectorEventBreakpointsAgent* agent) {
      agent->DidCreateOffscreenCanvasContext();
    });
  }
}
...
void ApplyHardwareConcurrencyOverrideImpl(CoreProbeSink* param_core_probe_sink, unsigned int& hardware_concurrency) {
  ...
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->ApplyHardwareConcurrencyOverride(hardware_concurrency);
    });
  }
}

void ApplyDataSaverOverrideImpl(CoreProbeSink* param_core_probe_sink, bool& data_saver) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->ApplyDataSaverOverride(data_saver);
    });
  }
}
...
void ApplyUserAgentMetadataOverrideImpl(CoreProbeSink* param_core_probe_sink, std::optional<blink::UserAgentMetadata>* ua_metadata_override) {
  ...
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->ApplyUserAgentMetadataOverride(ua_metadata_override);
    });
  }
}
...
void DidBlockRequestImpl(CoreProbeSink* param_core_probe_sink, const ResourceRequest& param_resource_request, DocumentLoader* param_document_loader, const KURL& fetch_context_url, const ResourceLoaderOptions& param_resource_loader_options, ResourceRequestBlockedReason param_resource_request_blocked_reason, ResourceType param_resource_type) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DidBlockRequest(param_resource_request, param_document_loader, fetch_context_url, param_resource_loader_options, param_resource_request_blocked_reason, param_resource_type);
    });
  }
}
...
void WillSendWorkerMainRequestImpl(ExecutionContext* param_execution_context, uint64_t identifier, const KURL& param_kurl) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WillSendWorkerMainRequest(identifier, param_kurl);
    });
  }
}
...
void DidReceiveDataImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, base::SpanOrSize<const char> data) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DidReceiveData(identifier, param_document_loader, data);
    });
  }
  if (probe_sink->HasInspectorTraceEventss()) {
    probe_sink->InspectorTraceEventss().ForEachAgent([&](InspectorTraceEvents* agent) {
...
void WillLoadXHRImpl(ExecutionContext* param_execution_context, const AtomicString& method, const KURL& url, bool async, const HTTPHeaderMap& headers, bool include_credentials) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WillLoadXHR(param_execution_context, method, url, async, headers, include_credentials);
    });
  }
}
...
void DidFinishSyncXHRImpl(ExecutionContext* context, const base::TimeDelta blocking_time) {
  ...
  if (probe_sink->HasAnimationFrameTimingMonitors()) {
    probe_sink->AnimationFrameTimingMonitors().ForEachAgent([&](AnimationFrameTimingMonitor* agent) {
      agent->DidFinishSyncXHR(blocking_time);
    });
  }
}
...
void DomContentLoadedEventFiredImpl(LocalFrame* param_local_frame) {
  ...
  if (probe_sink->HasDevToolsSessions()) {
    probe_sink->DevToolsSessions().ForEachAgent([&](DevToolsSession* agent) {
...
void FrameAttachedToParentImpl(LocalFrame* param_local_frame, const AdTracker::AdScriptAncestry& ad_script_ancestry) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_local_frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->FrameAttachedToParent(param_local_frame, ad_script_ancestry);
    });
  }
}
...
void FrameDetachedFromParentImpl(LocalFrame* param_local_frame, FrameDetachType param_frame_detach_type) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->FrameDetachedFromParent(param_local_frame, param_frame_detach_type);
    });
  }
}
...
void FrameSubtreeWillBeDetachedImpl(LocalFrame* param_local_frame, Frame* param_frame) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->FrameSubtreeWillBeDetached(param_frame);
    });
  }
}
...
void DidNavigateWithinDocumentImpl(LocalFrame* param_local_frame, blink::mojom::SameDocumentNavigationType param_blink) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->DidNavigateWithinDocument(param_local_frame, param_blink);
    });
  }
}
...
void DidRestoreFromBackForwardCacheImpl(LocalFrame* param_local_frame) {
  ...
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->DidRestoreFromBackForwardCache(param_local_frame);
    });
  }
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
...
void DidOpenDocumentImpl(LocalFrame* param_local_frame, DocumentLoader* param_document_loader) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->DidOpenDocument(param_local_frame, param_document_loader);
    });
  }
}
...
void WillCreateDocumentParserImpl(Document* param_document, bool& force_sync_parsing) {
  ...
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->WillCreateDocumentParser(force_sync_parsing);
    });
  }
}
...
void FrameRequestedNavigationImpl(LocalFrame* param_local_frame, Frame* target_frame, const KURL& url, ClientNavigationReason reason, NavigationPolicy policy) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->FrameRequestedNavigation(target_frame, url, reason, policy);
    });
  }
}
...
void WillCreateWebSocketImpl(ExecutionContext* param_execution_context, uint64_t identifier, const KURL& request_url, const String& protocol, std::optional<base::UnguessableToken>* devtools_token) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WillCreateWebSocket(param_execution_context, identifier, request_url, protocol, devtools_token);
    });
  }
}
...
void DidReceiveWebSocketMessageImpl(ExecutionContext* param_execution_context, uint64_t identifier, int opcode, bool masked, const Vector<base::span<const uint8_t>>& data) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DidReceiveWebSocketMessage(identifier, opcode, masked, data);
    });
  }
}

void DidSendWebSocketMessageImpl(ExecutionContext* param_execution_context, uint64_t identifier, int opcode, bool masked, base::span<const uint8_t> payload) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DidSendWebSocketMessage(identifier, opcode, masked, payload);
    });
  }
}
...
void WebTransportCreatedImpl(ExecutionContext* param_execution_context, uint64_t transport_id, const KURL& request_url) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WebTransportCreated(param_execution_context, transport_id, request_url);
    });
  }
}
...
void WebTransportConnectionEstablishedImpl(ExecutionContext* param_execution_context, uint64_t transport_id) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WebTransportConnectionEstablished(transport_id);
    });
  }
}
...
void WebTransportClosedImpl(ExecutionContext* param_execution_context, uint64_t transport_id) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->WebTransportClosed(transport_id);
    });
  }
}

void DirectTCPSocketCreatedImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& remote_addr, uint16_t remote_port, protocol::Network::DirectTCPSocketOptions& options) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketCreated(param_execution_context, identifier, remote_addr, remote_port, options);
    });
  }
}

void DirectTCPSocketOpenedImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& remote_addr, uint16_t remote_port, std::optional<String> local_addr, std::optional<uint16_t> local_port) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketOpened(identifier, remote_addr, remote_port, local_addr, local_port);
    });
  }
}

void DirectTCPSocketAbortedImpl(ExecutionContext* param_execution_context, uint64_t identifier, int net_error) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketAborted(identifier, net_error);
    });
  }
}

void DirectTCPSocketClosedImpl(ExecutionContext* param_execution_context, uint64_t identifier) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketClosed(identifier);
    });
  }
}

void DirectTCPSocketChunkSentImpl(ScriptState& param_script_state, uint64_t identifier, base::span<const uint8_t> data) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_script_state);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketChunkSent(identifier, data);
    });
  }
}

void DirectTCPSocketChunkReceivedImpl(ScriptState& param_script_state, uint64_t identifier, base::span<const uint8_t> data) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_script_state);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectTCPSocketChunkReceived(identifier, data);
    });
  }
}

void DirectUDPSocketCreatedImpl(ExecutionContext* param_execution_context, uint64_t identifier, protocol::Network::DirectUDPSocketOptions& options) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketCreated(param_execution_context, identifier, options);
    });
  }
}

void DirectUDPSocketOpenedImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& local_addr, uint16_t local_port, std::optional<String> remote_addr, std::optional<uint16_t> remote_port) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketOpened(identifier, local_addr, local_port, remote_addr, remote_port);
    });
  }
}

void DirectUDPSocketAbortedImpl(ExecutionContext* param_execution_context, uint64_t identifier, int net_error) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketAborted(identifier, net_error);
    });
  }
}

void DirectUDPSocketClosedImpl(ExecutionContext* param_execution_context, uint64_t identifier) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketClosed(identifier);
    });
  }
}

void DirectUDPSocketChunkSentImpl(ScriptState& param_script_state, uint64_t identifier, base::span<const uint8_t> data, std::optional<String> remote_addr, std::optional<uint16_t> remote_port) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_script_state);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketChunkSent(identifier, data, remote_addr, remote_port);
    });
  }
}

void DirectUDPSocketChunkReceivedImpl(ScriptState& param_script_state, uint64_t identifier, base::span<const uint8_t> data, std::optional<String> remote_addr, std::optional<uint16_t> remote_port) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_script_state);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketChunkReceived(identifier, data, remote_addr, remote_port);
    });
  }
}

void DirectUDPSocketJoinedMulticastGroupImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& ipAddress) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketJoinedMulticastGroup(identifier, ipAddress);
    });
  }
}

void DirectUDPSocketLeftMulticastGroupImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& ipAddress) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->DirectUDPSocketLeftMulticastGroup(identifier, ipAddress);
    });
  }
}
...
void LayerTreeDidChangeImpl(LocalFrame* param_local_frame) {
  ...
  if (probe_sink->HasInspectorLayerTreeAgents()) {
    probe_sink->InspectorLayerTreeAgents().ForEachAgent([&](InspectorLayerTreeAgent* agent) {
...
void AnimationUpdatedImpl(Document* param_document, Animation* param_animation) {
  ...
  if (probe_sink->HasInspectorAnimationAgents()) {
    probe_sink->InspectorAnimationAgents().ForEachAgent([&](InspectorAnimationAgent* agent) {
      agent->AnimationUpdated(param_animation);
    });
  }
}
...
void WindowOpenImpl(ExecutionContext* param_execution_context, const KURL& url, const AtomicString& window_name, const WebWindowFeatures& window_features, bool user_gestrue) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->WindowOpen(url, window_name, window_features, user_gestrue);
    });
  }
}
...
void ConsoleMessageAddedImpl(ExecutionContext* param_execution_context, ConsoleMessage* param_console_message) {
  ...
  if (probe_sink->HasInspectorLogAgents()) {
    probe_sink->InspectorLogAgents().ForEachAgent([&](InspectorLogAgent* agent) {
...
void InspectorIssueAddedImpl(CoreProbeSink* param_core_probe_sink, protocol::Audits::InspectorIssue* param_protocol) {
  ...
  if (probe_sink->HasInspectorAuditsAgents()) {
    probe_sink->InspectorAuditsAgents().ForEachAgent([&](InspectorAuditsAgent* agent) {
      agent->InspectorIssueAdded(param_protocol);
    });
  }
}
...
void DocumentWriteFetchScriptImpl(Document* param_document) {
  ...
  if (probe_sink->HasPerformanceMonitors()) {
    probe_sink->PerformanceMonitors().ForEachAgent([&](PerformanceMonitor* agent) {
...
void WillHandlePromiseImpl(ExecutionContext* context, ScriptState* script_state, bool resolving, const char* class_like_name, PropertyName property_like_name, LazySourceLocation* location) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(context);
  if (!probe_sink)
    return;
  if (probe_sink->HasAnimationFrameTimingMonitors()) {
    probe_sink->AnimationFrameTimingMonitors().ForEachAgent([&](AnimationFrameTimingMonitor* agent) {
      agent->WillHandlePromise(script_state, resolving, class_like_name, property_like_name, location);
    });
  }
}
...
RecalculateStyle::RecalculateStyle(Document* document) :
  ...
  if (probe_sink->HasInspectorPerformanceAgents()) {
    probe_sink->InspectorPerformanceAgents().ForEachAgent([&](InspectorPerformanceAgent* agent) {
      agent->Will(*this);
    });
  }
  if (probe_sink->HasPerformanceMonitors()) {
...
EvaluateScriptBlock::EvaluateScriptBlock(std::reference_wrapper<std::remove_reference_t<ScriptState&>> script_state, std::reference_wrapper<std::remove_reference_t<const KURL&>> source_url, bool is_module, bool sanitize) :
...
ExecuteScript::ExecuteScript(ExecutionContext* context, v8::Local<v8::Context> v8_context, std::reference_wrapper<std::remove_reference_t<const String&>> script_url, int script_id) :
  ...
  if (probe_sink->HasAdTrackers()) {
    probe_sink->AdTrackers().ForEachAgent([&](AdTracker* agent) {
      agent->Will(*this);
    });
  }
  if (probe_sink->HasAnimationFrameTimingMonitors()) {
  ...
  if (probe_sink->HasLCPScriptObservers()) {
    probe_sink->LCPScriptObservers().ForEachAgent([&](LCPScriptObserver* agent) {
      agent->Will(*this);
    });
  }
  if (probe_sink->HasPerformanceMonitors()) {
...
InvokeCallback::InvokeCallback(std::reference_wrapper<std::remove_reference_t<ScriptState&>> script_state, const char* name, CallbackFunctionBase* callback, v8::MaybeLocal<v8::Value> function) :
    script_state(script_state),
    name(name),
    callback(callback),
    function(function) {
...
UserEntryPoint::UserEntryPoint(ExecutionContext* context, v8::Local<v8::Object> callback_object, size_t index) :
    context(context),
...
FrameRelatedTask::~FrameRelatedTask() {
  if (!probe_sink)
    return;
  if (probe_sink->HasAnimationFrameTimingMonitors()) {
    probe_sink->AnimationFrameTimingMonitors().ForEachAgent([&](AnimationFrameTimingMonitor* agent) {
...
void ForceStartingStyleImpl(Element* element, bool* result) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(element);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorCSSAgents()) {
    probe_sink->InspectorCSSAgents().ForEachAgent([&](InspectorCSSAgent* agent) {
      agent->ForceStartingStyle(element, result);
    });
  }
}
...
void ConsoleTimeStampImpl(v8::Isolate* isolate, v8::Local<v8::String> label) {
  ...
  if (probe_sink->HasInspectorPerformanceAgents()) {
    probe_sink->InspectorPerformanceAgents().ForEachAgent([&](InspectorPerformanceAgent* agent) {
      agent->ConsoleTimeStamp(isolate, label);
    });
  }
}
...
void DidProduceCompilationCacheImpl(CoreProbeSink* param_core_probe_sink, const ClassicScript& param_classic_script, v8::Local<v8::Script> script) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->DidProduceCompilationCache(param_classic_script, script);
    });
  }
}
...
void ApplyCompilationModeOverrideImpl(ExecutionContext* param_execution_context, const ClassicScript& param_classic_script, v8::ScriptCompiler::CachedData** cached_data, v8::ScriptCompiler::CompileOptions* param_v8) {
  ...
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->ApplyCompilationModeOverride(param_classic_script, cached_data, param_v8);
    });
  }
}
...
void NodeCreatedImpl(Node* node) {
  ...
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->NodeCreated(node);
    });
  }
}

void FileChooserOpenedImpl(LocalFrame* frame, HTMLInputElement* element, bool multiple, bool* suppressed, bool* canceled) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(frame);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorPageAgents()) {
    probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) {
      agent->FileChooserOpened(frame, element, multiple, suppressed, canceled);
    });
  }
}
...
void PlayerErrorsRaisedImpl(ExecutionContext* context, String player_id, const Vector<InspectorPlayerError>& errors) {
  ...
  if (probe_sink->HasInspectorMediaAgents()) {
    probe_sink->InspectorMediaAgents().ForEachAgent([&](InspectorMediaAgent* agent) {
      agent->PlayerErrorsRaised(player_id, errors);
    });
  }
}
...
void PlayerEventsAddedImpl(ExecutionContext* context, String player_id, const Vector<InspectorPlayerEvent>& events) {
  ...
  if (probe_sink->HasInspectorMediaAgents()) {
    probe_sink->InspectorMediaAgents().ForEachAgent([&](InspectorMediaAgent* agent) {
      agent->PlayerEventsAdded(player_id, events);
    });
  }
}
...
void PlayerMessagesLoggedImpl(ExecutionContext* context, String player_id, const Vector<InspectorPlayerMessage>& messages) {
  ...
  if (probe_sink->HasInspectorMediaAgents()) {
    probe_sink->InspectorMediaAgents().ForEachAgent([&](InspectorMediaAgent* agent) {
      agent->PlayerMessagesLogged(player_id, messages);
    });
  }
}
...
void PlayerPropertiesChangedImpl(ExecutionContext* context, String player_id, const Vector<InspectorPlayerProperty>& properties) {
  ...
  if (probe_sink->HasInspectorMediaAgents()) {
    probe_sink->InspectorMediaAgents().ForEachAgent([&](InspectorMediaAgent* agent) {
      agent->PlayerPropertiesChanged(player_id, properties);
    });
  }
}

void PlayerCreatedImpl(ExecutionContext* context, const MediaPlayer& player) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorMediaAgents()) {
    probe_sink->InspectorMediaAgents().ForEachAgent([&](InspectorMediaAgent* agent) {
      agent->PlayerCreated(player);
    });
  }
}
...
void SetDevToolsIdsImpl(CoreProbeSink* param_core_probe_sink, ResourceRequest& request, const FetchInitiatorInfo& param_fetch_initiator_info) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->SetDevToolsIds(request, param_fetch_initiator_info);
    });
  }
}
...
void GetTextPositionImpl(Document* param_document, wtf_size_t offset, const String* text, TextPosition* result) {
  ...
  if (probe_sink->HasInspectorCSSAgents()) {
    probe_sink->InspectorCSSAgents().ForEachAgent([&](InspectorCSSAgent* agent) {
      agent->GetTextPosition(offset, text, result);
    });
  }
}
...
void LocalFontsEnabledImpl(ExecutionContext* param_execution_context, bool* result) {
  ...
  if (probe_sink->HasInspectorCSSAgents()) {
    probe_sink->InspectorCSSAgents().ForEachAgent([&](InspectorCSSAgent* agent) {
      agent->LocalFontsEnabled(result);
    });
  }
}
...
void DidUpdateComputedStyleImpl(Element* param_element, const ComputedStyle* old_style, const ComputedStyle* new_style) {
  ...
  if (probe_sink->HasInspectorCSSAgents()) {
    probe_sink->InspectorCSSAgents().ForEachAgent([&](InspectorCSSAgent* agent) {
      agent->DidUpdateComputedStyle(param_element, old_style, new_style);
    });
  }
}
...
void GetDisabledImageTypesImpl(ExecutionContext* param_execution_context, HashSet<String>* result) {
  ...
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->GetDisabledImageTypes(result);
    });
  }
}
...
void OnContentSecurityPolicyViolationImpl(ExecutionContext* context, const blink::ContentSecurityPolicyViolationType violationType) {
  ...
  if (probe_sink->HasInspectorDOMDebuggerAgents()) {
    probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) {
      agent->OnContentSecurityPolicyViolation(violationType);
    });
  }
}
...
void IsCacheDisabledImpl(ExecutionContext* param_execution_context, bool* is_cache_disabled) {
  ...
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->IsCacheDisabled(is_cache_disabled);
    });
  }
}

void ShouldApplyDevtoolsCookieSettingOverridesImpl(ExecutionContext* param_execution_context, bool* is_network_enabled) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorNetworkAgents()) {
    probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
      agent->ShouldApplyDevtoolsCookieSettingOverrides(is_network_enabled);
    });
  }
}
...
void PerformanceEntryAddedImpl(ExecutionContext* param_execution_context, PerformanceEntry* param_performance_entry) {
  ...
  if (probe_sink->HasInspectorPerformanceTimelineAgents()) {
    probe_sink->InspectorPerformanceTimelineAgents().ForEachAgent([&](InspectorPerformanceTimelineAgent* agent) {
      agent->PerformanceEntryAdded(param_execution_context, param_performance_entry);
    });
  }
}
...
void ApplyAutomationOverrideImpl(ExecutionContext* param_execution_context, bool& enabled) {
  ...
  if (probe_sink->HasInspectorEmulationAgents()) {
    probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) {
      agent->ApplyAutomationOverride(enabled);
    });
  }
}
...
void DidAddSpeculationRuleSetImpl(Document& param_document, const SpeculationRuleSet& rule_set) {
  ...
  if (probe_sink->HasInspectorPreloadAgents()) {
    probe_sink->InspectorPreloadAgents().ForEachAgent([&](InspectorPreloadAgent* agent) {
      agent->DidAddSpeculationRuleSet(param_document, rule_set);
    });
  }
}
...
void DidRemoveSpeculationRuleSetImpl(Document& param_document, const SpeculationRuleSet& rule_set) {
  ...
  if (probe_sink->HasInspectorPreloadAgents()) {
    probe_sink->InspectorPreloadAgents().ForEachAgent([&](InspectorPreloadAgent* agent) {
      agent->DidRemoveSpeculationRuleSet(rule_set);
    });
  }
}
...
void UpdateScrollableFlagImpl(Node* node, std::optional<bool> param_std) {
  ...
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->UpdateScrollableFlag(node, param_std);
    });
  }
}

void UpdateAdRelatedStateImpl(Node& node, std::optional<AdProvenance> ad_provenance) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(node);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->UpdateAdRelatedState(node, ad_provenance);
    });
  }
}

void WillHidePopoverImpl(HTMLElement* element, bool* force_open) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(element);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->WillHidePopover(element, force_open);
    });
  }
}

void UpdateAffectedByStartingStylesFlagImpl(Node* node, std::optional<bool> override_flag) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(node);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorDOMAgents()) {
    probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) {
      agent->UpdateAffectedByStartingStylesFlag(node, override_flag);
    });
  }
}

void WebMCPToolAddedImpl(Document* document, const ToolData& tool) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(document);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorWebMCPAgents()) {
    probe_sink->InspectorWebMCPAgents().ForEachAgent([&](InspectorWebMCPAgent* agent) {
      agent->WebMCPToolAdded(document, tool);
    });
  }
}

void WebMCPToolRemovedImpl(Document* document, const ToolData& tool) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(document);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorWebMCPAgents()) {
    probe_sink->InspectorWebMCPAgents().ForEachAgent([&](InspectorWebMCPAgent* agent) {
      agent->WebMCPToolRemoved(document, tool);
    });
  }
}

void WebMCPToolExecutedImpl(Document* document, const String& name, const String& input_arguments, const base::UnguessableToken& invocation_id) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(document);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorWebMCPAgents()) {
    probe_sink->InspectorWebMCPAgents().ForEachAgent([&](InspectorWebMCPAgent* agent) {
      agent->WebMCPToolExecuted(document, name, input_arguments, invocation_id);
    });
  }
}

void WebMCPToolRespondedImpl(Document* document, const String& result, const base::UnguessableToken& invocation_id) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(document);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorWebMCPAgents()) {
    probe_sink->InspectorWebMCPAgents().ForEachAgent([&](InspectorWebMCPAgent* agent) {
      agent->WebMCPToolResponded(document, result, invocation_id);
    });
  }
}

void WebMCPToolFailedImpl(Document* document, const ScriptToolError& result, const base::UnguessableToken& invocation_id, ScriptException exception) {
  CoreProbeSink* probe_sink = ToCoreProbeSink(document);
  if (!probe_sink)
    return;
  if (probe_sink->HasInspectorWebMCPAgents()) {
    probe_sink->InspectorWebMCPAgents().ForEachAgent([&](InspectorWebMCPAgent* agent) {
      agent->WebMCPToolFailed(document, result, invocation_id, exception);
    });
  }
}
