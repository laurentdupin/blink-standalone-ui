...
// [DeprecateAs]
bool has_deprecated;
if (!v8_dictionary->Has(current_context, v8_own_member_names[1].Get(isolate)).To(&has_deprecated)) {
  return;
}
if (has_deprecated) {
  Deprecation::CountDeprecation(execution_context, WebFeature::kAttributionReportingAPIAll);
}
}
