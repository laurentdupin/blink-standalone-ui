...
bindings::PerformAttributeSetCEReactionsReflectTypeString(info, html_names::kPosterAttr);
}
...
bindings::PerformAttributeSetCEReactionsReflectTypeBoolean(info, html_names::kPlaysinlineAttr);
}
...
bindings::PerformAttributeSetCEReactionsReflectTypeBoolean(info, html_names::kDisablepictureinpictureAttr);
}
...
// [HTMLConstructor]
V8HTMLConstructor::HtmlConstructor(info, *V8HTMLVideoElement::GetWrapperTypeInfo(), ElementType::kHTMLVideoElement);
}
...
static_assert(bindings::IsReturnTypeCompatible<VideoPlaybackQuality, std::remove_cvref_t<decltype(return_value)>>, "Return type from native call is incompatible to the type specified in IDL");
...
static_assert(bindings::IsReturnTypeCompatible<IDLPromise<PictureInPictureWindow>, std::remove_cvref_t<decltype(return_value)>>, "Return type from native call is incompatible to the type specified in IDL");
if (exception_state.HadException()) [[unlikely]] {
  return;
}
...
void V8HTMLVideoElement::Impl::InstallUnconditionalProperties(v8::Isolate* isolate, const DOMWrapperWorld& world, v8::Local<v8::ObjectTemplate> instance_template, v8::Local<v8::ObjectTemplate> prototype_template, v8::Local<v8::Template> interface_template) {
...
IDLMemberInstaller::InstallAttributes(isolate, world, instance_template, prototype_template, interface_template, signature, "HTMLVideoElement", kAttributeTable);
}
...
IDLMemberInstaller::InstallOperations(isolate, world, instance_template, prototype_template, interface_template, signature, "HTMLVideoElement", kOperationTable);
}
