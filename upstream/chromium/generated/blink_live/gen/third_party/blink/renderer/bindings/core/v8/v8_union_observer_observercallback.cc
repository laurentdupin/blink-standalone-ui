...
V8UnionObserverOrObserverCallback* V8UnionObserverOrObserverCallback::Create(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ExceptionState& exception_state) {
  if (v8_value->IsNullOrUndefined()) {
  auto&& blink_value = Observer::Create(isolate);
return MakeGarbageCollected<V8UnionObserverOrObserverCallback>(blink_value);
}
...
if (v8_value->IsObject()) {
  auto&& blink_value = NativeValueTraits<Observer>::NativeValue(isolate, v8_value, exception_state);
...
void V8UnionObserverOrObserverCallback::Trace(Visitor* visitor) const {
  visitor->Trace(member_observer_);
visitor->Trace(member_observer_callback_);
bindings::UnionBase::Trace(visitor);
}

void V8UnionObserverOrObserverCallback::Clear() {
  member_observer_.Clear();
