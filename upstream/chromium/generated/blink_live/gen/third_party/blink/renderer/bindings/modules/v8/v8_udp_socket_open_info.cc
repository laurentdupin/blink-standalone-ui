...
return dictionary;
 ...
 UDPSocketOpenInfo::UDPSocketOpenInfo(v8::Isolate* isolate) : SocketOpenInfo(isolate) {
...
void UDPSocketOpenInfo::Trace(Visitor* visitor) const {
  visitor->Trace(member_multicast_controller_);
SocketOpenInfo::Trace(visitor);
}

void UDPSocketOpenInfo::FillTemplateProperties(Vector<std::string_view>& properties) const {
...
if (hasMulticastController()) {
  values[0] = ToV8Traits<MulticastController>::ToV8(script_state, member_multicast_controller_.Get());
...
dictionary_from_v8_context.SetCurrentPropertyName("multicastController");
constexpr bool is_optional = false;
v8::Local<v8::Context> current_context = isolate->GetCurrentContext();
const auto& v8_own_member_names = GetV8OwnMemberNames(isolate);
if (!bindings::GetDictionaryMemberFromV8Object<MulticastController, is_optional>(isolate, current_context, v8_dictionary, v8_own_member_names[0].Get(isolate), has_multicast_controller_, member_multicast_controller_, class_like_name, exception_state)) {
  return;
}
}
