...
static_assert(std::derived_from<MessageEvent, Event>);
...
v8::Local<v8::Object> v8_receiver = info.This();
MessageEvent* blink_receiver = V8MessageEvent::ToWrappableUnsafe(isolate, v8_receiver);
auto&& return_value = blink_receiver->originForBindings();
...
static_assert(bindings::IsReturnTypeCompatible<IDLNullable<UserActivation>, std::remove_cvref_t<decltype(return_value)>>, "Return type from native call is incompatible to the type specified in IDL");
bindings::V8SetReturnValue(info, return_value, blink_receiver);
}
...
static_assert(bindings::IsReturnTypeCompatible<MessageEvent, std::remove_cvref_t<decltype(return_value)>>, "Return type from native call is incompatible to the type specified in IDL");
if (exception_state.HadException()) [[unlikely]] {
  return;
}
...
void V8MessageEvent::InstallUnconditionalProperties(v8::Isolate* isolate, const DOMWrapperWorld& world, v8::Local<v8::ObjectTemplate> instance_template, v8::Local<v8::ObjectTemplate> prototype_template, v8::Local<v8::Template> interface_template) {
...
IDLMemberInstaller::InstallAttributes(isolate, world, instance_template, prototype_template, interface_template, signature, "MessageEvent", kAttributeTable);
}
...
IDLMemberInstaller::InstallOperations(isolate, world, instance_template, prototype_template, interface_template, signature, "MessageEvent", kOperationTable);
}
