// third_party/blink/public/mojom/permissions/permission.mojom-forward.h is auto generated by mojom_bindings_generator.py, do not edit

// Copyright 2019 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef THIRD_PARTY_BLINK_PUBLIC_MOJOM_PERMISSIONS_PERMISSION_MOJOM_FORWARD_H_
#define THIRD_PARTY_BLINK_PUBLIC_MOJOM_PERMISSIONS_PERMISSION_MOJOM_FORWARD_H_

#include <stdint.h>

#include "mojo/public/cpp/bindings/struct_forward.h"

#include "mojo/public/cpp/bindings/deprecated_interface_types_forward.h"


#include "mojo/public/interfaces/bindings/native_struct.mojom-forward.h"






namespace blink::mojom {
class MidiPermissionDescriptorDataView;

class ClipboardPermissionDescriptorDataView;

class CameraDevicePermissionDescriptorDataView;

class TopLevelStorageAccessPermissionDescriptorDataView;

class FullscreenPermissionDescriptorDataView;

class PermissionDescriptorDataView;

class GeolocationEmbeddedPermissionRequestDescriptorDataView;

class InstallEmbeddedPermissionRequestDescriptorDataView;

class UserMediaEmbeddedPermissionRequestDescriptorDataView;

class EmbeddedPermissionRequestDescriptorDataView;

class PermissionDescriptorExtensionDataView;
class EmbeddedPermissionControlDescriptorExtensionDataView;

enum class PermissionName : int32_t;

enum class EmbeddedPermissionControlResult : int32_t;
class MidiPermissionDescriptor;
using MidiPermissionDescriptorPtr = mojo::InlinedStructPtr<MidiPermissionDescriptor>;

class ClipboardPermissionDescriptor;
using ClipboardPermissionDescriptorPtr = mojo::InlinedStructPtr<ClipboardPermissionDescriptor>;

class CameraDevicePermissionDescriptor;
using CameraDevicePermissionDescriptorPtr = mojo::InlinedStructPtr<CameraDevicePermissionDescriptor>;

class TopLevelStorageAccessPermissionDescriptor;
using TopLevelStorageAccessPermissionDescriptorPtr = mojo::StructPtr<TopLevelStorageAccessPermissionDescriptor>;

class FullscreenPermissionDescriptor;
using FullscreenPermissionDescriptorPtr = mojo::InlinedStructPtr<FullscreenPermissionDescriptor>;

class PermissionDescriptor;
using PermissionDescriptorPtr = mojo::StructPtr<PermissionDescriptor>;

class GeolocationEmbeddedPermissionRequestDescriptor;
using GeolocationEmbeddedPermissionRequestDescriptorPtr = mojo::InlinedStructPtr<GeolocationEmbeddedPermissionRequestDescriptor>;

class InstallEmbeddedPermissionRequestDescriptor;
using InstallEmbeddedPermissionRequestDescriptorPtr = mojo::InlinedStructPtr<InstallEmbeddedPermissionRequestDescriptor>;

class UserMediaEmbeddedPermissionRequestDescriptor;
using UserMediaEmbeddedPermissionRequestDescriptorPtr = mojo::InlinedStructPtr<UserMediaEmbeddedPermissionRequestDescriptor>;

class EmbeddedPermissionRequestDescriptor;
using EmbeddedPermissionRequestDescriptorPtr = mojo::StructPtr<EmbeddedPermissionRequestDescriptor>;

class PermissionDescriptorExtension;

using PermissionDescriptorExtensionPtr = mojo::StructPtr<PermissionDescriptorExtension>;

class EmbeddedPermissionControlDescriptorExtension;

using EmbeddedPermissionControlDescriptorExtensionPtr = mojo::StructPtr<EmbeddedPermissionControlDescriptorExtension>;

class PermissionObserver;

class EmbeddedPermissionControlClient;

class PermissionService;




}  // blink::mojom

#endif  // THIRD_PARTY_BLINK_PUBLIC_MOJOM_PERMISSIONS_PERMISSION_MOJOM_FORWARD_H_