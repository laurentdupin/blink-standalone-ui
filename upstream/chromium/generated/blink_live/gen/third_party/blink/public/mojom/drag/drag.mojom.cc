...
size_t AllowedDragOperations::Hash(size_t seed) const {
  seed = mojo::internal::Hash(seed, this->allow_copy);
  seed = mojo::internal::Hash(seed, this->allow_link);
  seed = mojo::internal::Hash(seed, this->allow_move);
  return seed;
}
...
DragItemBinary::DragItemBinary(
    ...
    ::mojo_base::mojom::FilePathPtr filename_extension_in,
...
void DragItem::set_file(::blink::mojom::DataTransferFilePtr file) {
  if (tag_ == Tag::kFile) {
    data_.file = std::move(file);
  } else {
    DestroyActive();
    tag_ = Tag::kFile;
    new (&data_.file) ::blink::mojom::DataTransferFilePtr(
        std::move(file));
  }
}
