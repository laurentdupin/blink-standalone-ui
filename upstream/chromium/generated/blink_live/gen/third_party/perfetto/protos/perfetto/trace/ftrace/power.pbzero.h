...
class GpuPowerStateFtraceEvent_Decoder : public ::protozero::TypedProtoDecoder</*MAX_FIELD_ID=*/2> {
 public:
  GpuPowerStateFtraceEvent_Decoder(const uint8_t* data, size_t len) : TypedProtoDecoder(data, len) {}
  explicit GpuPowerStateFtraceEvent_Decoder(const std::string& raw) : TypedProtoDecoder(reinterpret_cast<const uint8_t*>(raw.data()), raw.size()) {}
  explicit GpuPowerStateFtraceEvent_Decoder(const ::protozero::ConstBytes& raw) : TypedProtoDecoder(raw.data, raw.size) {}
  bool has_old_state() const { return at<1>().valid(); }
  int32_t old_state() const { return at<1>().as_int32(); }
  bool has_new_state() const { return at<2>().valid(); }
  int32_t new_state() const { return at<2>().as_int32(); }
};

class GpuPowerStateFtraceEvent : public ::protozero::Message {
 public:
  using Decoder = GpuPowerStateFtraceEvent_Decoder;
  enum : int32_t {
    kOldStateFieldNumber = 1,
    kNewStateFieldNumber = 2,
  };
  static constexpr const char* GetName() { return ".perfetto.protos.GpuPowerStateFtraceEvent"; }
