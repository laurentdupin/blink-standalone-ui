...
class PERFETTO_EXPORT_COMPONENT PerfEventConfig : public ::protozero::CppMessageObj {
 public:
  ...
  bool has_ignore_open_failure() const { return _has_field_[21]; }
  bool ignore_open_failure() const { return ignore_open_failure_; }
  void set_ignore_open_failure(bool value) { ignore_open_failure_ = value; _has_field_.set(21); }

  const std::vector<std::string>& cpuid() const { return cpuid_; }
  std::vector<std::string>* mutable_cpuid() { return &cpuid_; }
  int cpuid_size() const { return static_cast<int>(cpuid_.size()); }
  void clear_cpuid() { cpuid_.clear(); }
  void add_cpuid(std::string value) { cpuid_.emplace_back(value); }
  std::string* add_cpuid() { cpuid_.emplace_back(); return &cpuid_.back(); }
