...
TF_BUILTIN(StringPrototypeSubstr, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    tmp12 = CodeStubAssembler(state_).UintPtrLessThanOrEqual(TNode<UintPtrT>{tmp11}, TNode<UintPtrT>{tmp2});
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    {
      ...
      pos_stack.push_back({"src/builtins/string-substr.tq", 31});
      CodeStubAssembler(state_).FailAssert("Torque assert 'lengthLimit <= size' failed", pos_stack);
    }
  }
  ...
  if (block5.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).TaggedNotEqual(TNode<Object>{tmp10}, TNode<Union<Context, FixedArrayBase, FunctionTemplateInfo, Hole, JSReceiver, Map, Oddball, String, Symbol, WasmFuncRef, WasmNull, WeakCell>>{tmp13});
  ...
  if (block7.is_used()) {
    ca_.Bind(&block7);
    tmp15 = ClampToIndexRange_0(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp10}, TNode<UintPtrT>{tmp11});
  ...
  if (block11.is_used()) {
    ca_.Bind(&block11);
    tmp18 = CodeStubAssembler(state_).EmptyStringConstant();
  ...
  if (block12.is_used()) {
    ca_.Bind(&block12);
    tmp19 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb3_9}, TNode<UintPtrT>{phi_bb9_12});
    tmp20 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp1}, TNode<UintPtrT>{phi_bb3_9}, TNode<UintPtrT>{tmp19});
