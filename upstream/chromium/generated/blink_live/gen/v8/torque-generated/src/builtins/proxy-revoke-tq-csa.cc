...
TF_BUILTIN(ProxyRevoke, CodeStubAssembler) {
  ...
  if (block4.is_used()) {
    ...
    tmp8 = Is_JSProxy_JSProxy_0(state_, TNode<Context>{tmp0}, TNode<JSProxy>{ca_.UncheckedCast<JSProxy>(tmp4)});
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    {
      ...
      pos_stack.push_back({"src/builtins/proxy-revoke.tq", 29});
  ...
  if (block5.is_used()) {
    ...
    tmp13 = Null_0(state_);
    CodeStubAssembler(state_).StoreReference<Union<JSReceiver, Null>>(CodeStubAssembler::Reference{ca_.UncheckedCast<JSProxy>(tmp4), tmp12}, tmp13);
    tmp14 = Undefined_0(state_);
    CodeStubAssembler(state_).Return(tmp14);
  }
}

// https://crsrc.org/c/v8/src/builtins/proxy-revoke.tq?l=17&c=7
