...
TF_BUILTIN(RegExpPrototypeSourceGetter, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp4 = RegExpSource_0(state_, TNode<JSRegExp>{tmp0});
    CodeStubAssembler(state_).Return(tmp4);
  }

  if (block5.is_used()) {
    ca_.Bind(&block5);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kRegExpNonRegExp), "RegExp.prototype.source");
  }

  TNode<JSAny> tmp5;
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp5 = FromConstexpr_JSAny_constexpr_string_0(state_, "(?:)");
