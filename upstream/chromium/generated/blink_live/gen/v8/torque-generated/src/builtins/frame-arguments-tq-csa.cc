...
// https://crsrc.org/c/v8/src/builtins/frame-arguments.tq?l=45&c=1
TorqueStructFrameWithArgumentsInfo_0 GetFrameWithArgumentsInfo_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context) {
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/frame-arguments.tq:48:63");
  ...
  if (block4.is_used()) {
    ...
    tmp8 = Convert_bint_int32_0(state_, TNode<Int32T>{tmp7});
    tmp9 = LoadArgCFromFrame_0(state_, TNode<RawPtrT>{tmp1});
    tmp10 = Convert_bint_intptr_0(state_, TNode<IntPtrT>{tmp9});
    ca_.Goto(&block6);
  }

    ca_.Bind(&block6);
  return TorqueStructFrameWithArgumentsInfo_0{TNode<RawPtrT>{tmp1}, TNode<BInt>{tmp10}, TNode<BInt>{tmp8}};
}
