...
TF_BUILTIN(TypedArrayPrototypeToSorted, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    tmp4 = TypedArrayBuiltinsAssembler(state_).ValidateTypedArrayAndGetLength(TNode<Context>{parameter0}, TNode<JSAny>{parameter1}, kBuiltinNameToSorted_0(state_), CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kRead));
    ...
    tmp9 = CodeStubAssembler(state_).ArrayBufferMaxByteLength();
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/typed-array-to-sorted.tq:51:47");
  ...
  if (block12.is_used()) {
    ca_.Bind(&block12);
    tmp20 = TypedArraySortCommon_0(state_, TNode<Context>{parameter0}, TNode<JSTypedArray>{tmp6}, TNode<UintPtrT>{tmp4}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction, Undefined>>{tmp2}, false);
