...
TF_BUILTIN(StringPrototypeStartsWith, CodeStubAssembler) {
  ...
  if (block1.is_used()) {
    ca_.Bind(&block1);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kFirstArgumentNotRegExp), "String.prototype.startsWith");
  }
  ...
  if (block5.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).UintPtrSub(TNode<UintPtrT>{tmp8}, TNode<UintPtrT>{phi_bb5_11});
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp17 = CodeStubAssembler(state_).Signed(TNode<UintPtrT>{phi_bb5_11});
