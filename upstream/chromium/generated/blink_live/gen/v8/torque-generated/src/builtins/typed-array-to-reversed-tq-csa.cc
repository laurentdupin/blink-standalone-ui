...
// https://crsrc.org/c/v8/src/builtins/typed-array-to-reversed.tq?l=6&c=1
const char* kBuiltinNameToReversed_0(compiler::CodeAssemblerState* state_) {
  compiler::CodeAssembler ca_(state_);
  compiler::CodeAssemblerParameterizedLabel<> block0(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
    ca_.Goto(&block0);

    ca_.Bind(&block0);
  return "%TypedArray%.prototype.toReversed";}
...
TF_BUILTIN(TypedArrayPrototypeToReversed, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<UintPtrT, UintPtrT, UintPtrT> block7(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = TypedArrayBuiltinsAssembler(state_).ValidateTypedArrayAndGetLength(TNode<Context>{parameter0}, TNode<JSAny>{parameter1}, kBuiltinNameToReversed_0(state_), CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kRead));
...
tmp13 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(2)), tmp5, parameter0, tmp2, phi_bb1_12, tmp12));
    ...
    ca_.Branch(tmp15, &block6, std::vector<compiler::Node*>{phi_bb1_12, phi_bb1_12, phi_bb1_12}, &block7, std::vector<compiler::Node*>{phi_bb1_12, phi_bb1_12, phi_bb1_12});
  }

  TNode<UintPtrT> phi_bb7_12;
  TNode<UintPtrT> phi_bb7_20;
  TNode<UintPtrT> phi_bb7_24;
  if (block7.is_used()) {
    ca_.Bind(&block7, &phi_bb7_12, &phi_bb7_20, &phi_bb7_24);
    {
  ...
  TNode<UintPtrT> phi_bb6_20;
  TNode<UintPtrT> phi_bb6_24;
  TNode<UintPtrT> tmp16;
  TNode<UintPtrT> tmp17;
  if (block6.is_used()) {
    ca_.Bind(&block6, &phi_bb6_12, &phi_bb6_20, &phi_bb6_24);
    tmp16 = FromConstexpr_uintptr_constexpr_int31_0(state_, 1);
    tmp17 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb6_12}, TNode<UintPtrT>{tmp16});
