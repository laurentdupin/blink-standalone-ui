...
// https://crsrc.org/c/v8/src/builtins/typed-array-of.tq?l=8&c=1
...
TF_BUILTIN(TypedArrayOf, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<UintPtrT, UintPtrT, UintPtrT, BoolT> block14(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<UintPtrT, UintPtrT, UintPtrT> block11(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  compiler::CodeAssemblerParameterizedLabel<UintPtrT, UintPtrT, UintPtrT> block10(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block3.is_used()) {
    ...
    tmp4 = TypedArrayCreateByLength_0(state_, TNode<Context>{parameter0}, TNode<JSReceiver>{tmp1}, TNode<Number>{tmp3}, kBuiltinNameOf_0(state_), CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kWrite));
...
tmp13 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(3)), tmp8, parameter0, tmp4, phi_bb5_12, tmp12));
    tmp14 = kStoreSucceded_0(state_);
    tmp15 = CodeStubAssembler(state_).SmiEqual(TNode<Smi>{tmp13}, TNode<Smi>{tmp14});
    ca_.Branch(tmp15, &block12, std::vector<compiler::Node*>{phi_bb5_12, phi_bb5_12, phi_bb5_12}, &block13, std::vector<compiler::Node*>{phi_bb5_12, phi_bb5_12, phi_bb5_12});
  }

  TNode<UintPtrT> phi_bb12_12;
  TNode<UintPtrT> phi_bb12_19;
  TNode<UintPtrT> phi_bb12_23;
  TNode<BoolT> tmp16;
  if (block12.is_used()) {
    ca_.Bind(&block12, &phi_bb12_12, &phi_bb12_19, &phi_bb12_23);
    tmp16 = FromConstexpr_bool_constexpr_bool_0(state_, true);
    ca_.Goto(&block14, phi_bb12_12, phi_bb12_19, phi_bb12_23, tmp16);
  }

  TNode<UintPtrT> phi_bb13_12;
  TNode<UintPtrT> phi_bb13_19;
  TNode<UintPtrT> phi_bb13_23;
  TNode<Smi> tmp17;
  TNode<BoolT> tmp18;
  if (block13.is_used()) {
    ca_.Bind(&block13, &phi_bb13_12, &phi_bb13_19, &phi_bb13_23);
    tmp17 = kStoreFailureArrayDetachedOrImmutableOrOutOfBounds_0(state_);
    tmp18 = CodeStubAssembler(state_).SmiEqual(TNode<Smi>{tmp13}, TNode<Smi>{tmp17});
    ca_.Goto(&block14, phi_bb13_12, phi_bb13_19, phi_bb13_23, tmp18);
  }

  TNode<UintPtrT> phi_bb14_12;
  TNode<UintPtrT> phi_bb14_19;
  TNode<UintPtrT> phi_bb14_23;
  TNode<BoolT> phi_bb14_28;
  if (block14.is_used()) {
    ca_.Bind(&block14, &phi_bb14_12, &phi_bb14_19, &phi_bb14_23, &phi_bb14_28);
    ca_.Branch(phi_bb14_28, &block10, std::vector<compiler::Node*>{phi_bb14_12, phi_bb14_19, phi_bb14_23}, &block11, std::vector<compiler::Node*>{phi_bb14_12, phi_bb14_19, phi_bb14_23});
  }

  TNode<UintPtrT> phi_bb11_12;
  TNode<UintPtrT> phi_bb11_19;
  TNode<UintPtrT> phi_bb11_23;
  if (block11.is_used()) {
    ca_.Bind(&block11, &phi_bb11_12, &phi_bb11_19, &phi_bb11_23);
    {
  ...
  TNode<UintPtrT> phi_bb10_19;
  TNode<UintPtrT> phi_bb10_23;
  TNode<UintPtrT> tmp19;
  TNode<UintPtrT> tmp20;
  if (block10.is_used()) {
    ca_.Bind(&block10, &phi_bb10_12, &phi_bb10_19, &phi_bb10_23);
    tmp19 = FromConstexpr_uintptr_constexpr_int31_0(state_, 1);
    tmp20 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb10_12}, TNode<UintPtrT>{tmp19});
    ca_.Goto(&block7, tmp20);
  }
