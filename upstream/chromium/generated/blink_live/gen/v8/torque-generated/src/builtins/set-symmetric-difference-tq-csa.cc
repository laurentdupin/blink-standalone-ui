...
TF_BUILTIN(SetPrototypeSymmetricDifference, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    tmp13 = CodeStubAssembler(state_).CloneFixedArray(TNode<FixedArrayBase>{tmp12}, CastIfEnumClass<CodeStubAssembler::ExtractFixedArrayFlag>(CodeStubAssembler::ExtractFixedArrayFlag::kFixedArrays));
  ...
  if (block9.is_used()) {
    ca_.Bind(&block9);
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/set-symmetric-difference.tq:31:17");
  ...
  if (block51.is_used()) {
    ...
    tmp68 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{parameter0}, TNode<OrderedHashSet>{phi_bb51_14}, TNode<JSAny>{tmp67});
  ...
  if (block60.is_used()) {
    ...
    tmp76 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{parameter0}, TNode<OrderedHashSet>{tmp75}, TNode<JSAny>{tmp67});
  ...
  if (block66.is_used()) {
    ...
    tmp77 = CollectionsBuiltinsAssembler(state_).DeleteFromSetTable(TNode<Context>{parameter0}, TNode<OrderedHashSet>{phi_bb66_14}, TNode<JSAny>{tmp67}, &label78);
  ...
  if (block71.is_used()) {
    ...
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/set-symmetric-difference.tq:102:25");
    CodeStubAssembler(state_).Unreachable();
  }
  ...
  if (block20.is_used()) {
    ...
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/set-symmetric-difference.tq:125:3");
    CodeStubAssembler(state_).Unreachable();
  }
...
// https://crsrc.org/c/v8/src/builtins/set-symmetric-difference.tq?l=136&c=1
TorqueStructOrderedHashSetAndNumberOfElements_0 FastSymmetricDifference_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSAny> p_nextValue, TorqueStructStableJSSetBackingTableWitness_0 p_table, TorqueStructOrderedHashSetAndNumberOfElements_0 p_resultSetDataAndNumberOfElements, const char* p_methodName) {
  ...
  compiler::CodeAssemblerParameterizedLabel<OrderedHashSet, Smi> block7(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block0.is_used()) {
    ...
    tmp1 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{p_context}, TNode<OrderedHashSet>{p_resultSetDataAndNumberOfElements.setData}, TNode<JSAny>{tmp0});
    tmp2 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{p_context}, TNode<OrderedHashSet>{p_table.unstable}, TNode<JSAny>{tmp0});
    tmp3 = CodeStubAssembler(state_).Word32Equal(TNode<BoolT>{tmp1}, TNode<BoolT>{tmp2});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    {
      ...
      pos_stack.push_back({"src/builtins/set-symmetric-difference.tq", 152});
      CodeStubAssembler(state_).FailAssert("Torque assert 'inResult == table.HasKey(key)' failed", pos_stack);
    }
  }
  ...
  if (block5.is_used()) {
    ...
    tmp4 = CollectionsBuiltinsAssembler(state_).DeleteFromSetTable(TNode<Context>{p_context}, TNode<OrderedHashSet>{p_resultSetDataAndNumberOfElements.setData}, TNode<JSAny>{tmp0}, &label5);
    ca_.Goto(&block10);
    if (label5.is_used()) {
      ca_.Bind(&label5);
      ca_.Goto(&block11);
  ...
  if (block11.is_used()) {
    ca_.Bind(&block11);
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/set-symmetric-difference.tq:156:19");
    CodeStubAssembler(state_).Unreachable();
  }

  if (block10.is_used()) {
    ca_.Bind(&block10);
    ca_.Goto(&block7, p_resultSetDataAndNumberOfElements.setData, tmp4);
  }

  TNode<String> tmp6;
  TNode<OrderedHashSet> tmp7;
  TNode<Smi> tmp8;
  TNode<Smi> tmp9;
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp6 = FromConstexpr_String_constexpr_string_0(state_, p_methodName);
    tmp7 = CollectionsBuiltinsAssembler(state_).AddToSetTable(TNode<Context>{p_context}, TNode<OrderedHashSet>{p_resultSetDataAndNumberOfElements.setData}, TNode<JSAny>{tmp0}, TNode<String>{tmp6});
    tmp8 = FromConstexpr_Smi_constexpr_int31_0(state_, 1);
    tmp9 = CodeStubAssembler(state_).SmiAdd(TNode<Smi>{p_resultSetDataAndNumberOfElements.numberOfElements}, TNode<Smi>{tmp8});
    ca_.Goto(&block7, tmp7, tmp9);
  }
  ...
  if (block7.is_used()) {
    ...
    ca_.Bind(&block12);
  return TorqueStructOrderedHashSetAndNumberOfElements_0{TNode<OrderedHashSet>{phi_bb7_7}, TNode<Smi>{phi_bb7_8}};
}
