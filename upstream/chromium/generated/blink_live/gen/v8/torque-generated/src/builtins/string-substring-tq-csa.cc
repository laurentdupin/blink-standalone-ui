...
TF_BUILTIN(StringPrototypeSubstring, CodeStubAssembler) {
  ...
  if (block1.is_used()) {
    ca_.Bind(&block1);
    tmp7 = ClampToIndexRange_0(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp4}, TNode<UintPtrT>{tmp2});
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    tmp13 = ClampToIndexRange_0(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp10}, TNode<UintPtrT>{tmp2});
  ...
  if (block7.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).UintPtrLessThan(TNode<UintPtrT>{phi_bb7_11}, TNode<UintPtrT>{phi_bb3_9});
    ca_.Branch(tmp14, &block9, std::vector<compiler::Node*>{}, &block10, std::vector<compiler::Node*>{phi_bb3_9, phi_bb7_11});
  }

  if (block9.is_used()) {
    ca_.Bind(&block9);
    ca_.Goto(&block10, phi_bb7_11, phi_bb3_9);
  }
  ...
  TNode<String> tmp15;
  if (block10.is_used()) {
    ca_.Bind(&block10, &phi_bb10_9, &phi_bb10_11);
    tmp15 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp1}, TNode<UintPtrT>{phi_bb10_9}, TNode<UintPtrT>{phi_bb10_11});
