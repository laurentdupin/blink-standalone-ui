...
// https://crsrc.org/c/v8/src/objects/heap-number.tq?l=5&c=1
TNode<HeapNumber> Cast_HeapNumber_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_obj, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ...
    tmp0 = DownCastForTorqueClass_HeapNumber_0(state_, TNode<HeapObject>{p_obj}, &label1);
    ca_.Goto(&block3);
    if (label1.is_used()) {
      ca_.Bind(&label1);
      ca_.Goto(&block4);
...
TNode<HeapNumber> DownCastForTorqueClass_HeapNumber_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    if ((CodeStubAssembler(state_).ClassHasMapConstant<HeapNumber>())) {
      ca_.Goto(&block6);
    } else {
      ca_.Goto(&block7);
    }
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp2 = CodeStubAssembler(state_).GetClassMapConstant<HeapNumber>();
  ...
  if (block7.is_used()) {
    ...
    tmp6 = FromConstexpr_uint32_constexpr_uint32_0(state_, CastIfEnumClass<uint32_t>(static_cast<InstanceType>(130)));
