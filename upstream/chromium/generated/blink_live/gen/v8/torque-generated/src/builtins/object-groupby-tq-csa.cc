...
TF_BUILTIN(GroupByGeneric, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number> block5(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number> block4(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number> block9(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number> block8(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number, Number> block12(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number> block13(&ca_, compiler::CodeAssemblerLabel::kDeferred);
  compiler::CodeAssemblerParameterizedLabel<OrderedHashMap, Number, JSAny, JSAny, Union<JSMessageObject, TheHole>> block11(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block1.is_used()) {
    ...
    tmp5 = FromConstexpr_Number_constexpr_float64_0(state_, kMaxSafeInteger);
    tmp6 = NumberIsLessThan_0(state_, TNode<Number>{phi_bb1_10}, TNode<Number>{tmp5});
    ca_.Branch(tmp6, &block4, std::vector<compiler::Node*>{phi_bb1_6, phi_bb1_10}, &block5, std::vector<compiler::Node*>{phi_bb1_6, phi_bb1_10});
  }

  TNode<OrderedHashMap> phi_bb5_6;
  TNode<Number> phi_bb5_10;
  if (block5.is_used()) {
    ca_.Bind(&block5, &phi_bb5_6, &phi_bb5_10);
    {
      auto pos_stack = ca_.GetMacroSourcePositionStack();
      pos_stack.push_back({"src/builtins/object-groupby.tq", 55});
      CodeStubAssembler(state_).FailAssert("Torque assert 'k < kMaxSafeInteger' failed", pos_stack);
    }
  }

  TNode<OrderedHashMap> phi_bb4_6;
  TNode<Number> phi_bb4_10;
  TNode<JSReceiver> tmp7;
  if (block4.is_used()) {
    ca_.Bind(&block4, &phi_bb4_6, &phi_bb4_10);
    compiler::CodeAssemblerLabel label8(&ca_);
    tmp7 = IteratorBuiltinsAssembler(state_).IteratorStep(TNode<Context>{parameter0}, TorqueStructIteratorRecord{TNode<JSReceiver>{tmp1}, TNode<JSAny>{tmp2}}, TNode<Map>{tmp0}, &label8);
    ca_.Goto(&block8, phi_bb4_6, phi_bb4_10);
    if (label8.is_used()) {
      ca_.Bind(&label8);
      ca_.Goto(&block9, phi_bb4_6, phi_bb4_10);
    }
  }

  TNode<OrderedHashMap> phi_bb9_6;
  TNode<Number> phi_bb9_10;
  if (block9.is_used()) {
    ca_.Bind(&block9, &phi_bb9_6, &phi_bb9_10);
    CodeStubAssembler(state_).Return(phi_bb9_6);
  }

  TNode<OrderedHashMap> phi_bb8_6;
  ...
  TNode<JSAny> tmp14;
      TNode<JSAny> tmp16;
  TNode<OrderedHashMap> tmp17;
  TNode<Number> tmp18;
  TNode<Number> tmp19;
  if (block8.is_used()) {
    ca_.Bind(&block8, &phi_bb8_6, &phi_bb8_10);
    tmp9 = IteratorBuiltinsAssembler(state_).IteratorValue(TNode<Context>{parameter0}, TNode<JSReceiver>{tmp7}, TNode<Map>{tmp0});
    ...
    tmp11 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<JSAny>{parameter3}, TNode<JSAny>{tmp10}, TNode<JSAny>{tmp9}, TNode<JSAny>{phi_bb8_10});
    }
    if (catch12__label.is_used()) {
      compiler::CodeAssemblerLabel catch12_skip(&ca_);
      ca_.Goto(&catch12_skip);
      ca_.Bind(&catch12__label, &tmp13);
      ca_.Goto(&block12, phi_bb8_6, phi_bb8_10, phi_bb8_10);
    ...
    tmp14 = CoerceGroupKey_0(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp11}, TNode<Boolean>{parameter4});
    }
    if (catch15__label.is_used()) {
      compiler::CodeAssemblerLabel catch15_skip(&ca_);
      ca_.Goto(&catch15_skip);
      ca_.Bind(&catch15__label, &tmp16);
      ca_.Goto(&block13, phi_bb8_6, phi_bb8_10);
      ca_.Bind(&catch15_skip);
    }
    tmp17 = CollectionsBuiltinsAssembler(state_).AddValueToKeyedGroup(TNode<Context>{parameter0}, TNode<OrderedHashMap>{phi_bb8_6}, TNode<JSAny>{tmp14}, TNode<Object>{tmp9}, TNode<String>{parameter5});
    tmp18 = FromConstexpr_Number_constexpr_IntegerLiteral_0(state_, IntegerLiteral(false, 0x1ull));
    tmp19 = CodeStubAssembler(state_).NumberAdd(TNode<Number>{phi_bb8_10}, TNode<Number>{tmp18});
    ca_.Goto(&block3, tmp17, tmp19);
  }

  TNode<OrderedHashMap> phi_bb12_6;
  TNode<Number> phi_bb12_10;
  TNode<Number> phi_bb12_18;
  TNode<Union<JSMessageObject, TheHole>> tmp20;
  if (block12.is_used()) {
    ca_.Bind(&block12, &phi_bb12_6, &phi_bb12_10, &phi_bb12_18);
    tmp20 = GetAndResetPendingMessage_0(state_);
    ca_.Goto(&block11, phi_bb12_6, phi_bb12_10, ca_.Uninitialized<JSAny>(), tmp13, tmp20);
  }
  ...
  if (block13.is_used()) {
    ca_.Bind(&block13, &phi_bb13_6, &phi_bb13_10);
    tmp21 = GetAndResetPendingMessage_0(state_);
    ca_.Goto(&block11, phi_bb13_6, phi_bb13_10, tmp11, tmp16, tmp21);
  }

  TNode<OrderedHashMap> phi_bb11_6;
  TNode<Number> phi_bb11_10;
  TNode<JSAny> phi_bb11_13;
  TNode<JSAny> phi_bb11_14;
  TNode<Union<JSMessageObject, TheHole>> phi_bb11_15;
  if (block11.is_used()) {
    ca_.Bind(&block11, &phi_bb11_6, &phi_bb11_10, &phi_bb11_13, &phi_bb11_14, &phi_bb11_15);
    IteratorCloseOnException_0(state_, TNode<Context>{parameter0}, TNode<JSReceiver>{tmp1});
    CodeStubAssembler(state_).CallRuntime(Runtime::kReThrowWithMessage, parameter0, phi_bb11_14, phi_bb11_15);
  ...
  if (block2.is_used()) {
    ...
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/object-groupby.tq:91:3");
    CodeStubAssembler(state_).Unreachable();
  }
}
...
TNode<OrderedHashMap> GroupByImpl_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSAny> p_items, TNode<JSAny> p_callback, TNode<Boolean> p_coerceToProperty, const char* p_methodName) {
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{p_context}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kCalledNonCallable), TNode<Object>{p_callback});
  }
  ...
  if (block24.is_used()) {
    ...
    tmp30 = CollectionsBuiltinsAssembler(state_).AddValueToKeyedGroup(TNode<Context>{p_context}, TNode<OrderedHashMap>{phi_bb24_5}, TNode<JSAny>{tmp28}, TNode<Object>{phi_bb24_14}, TNode<String>{tmp29});
  ...
  if (block34.is_used()) {
    ...
    tmp41 = CollectionsBuiltinsAssembler(state_).AddValueToKeyedGroup(TNode<Context>{p_context}, TNode<OrderedHashMap>{phi_bb34_5}, TNode<JSAny>{tmp39}, TNode<Object>{tmp36}, TNode<String>{tmp40});
