...
TF_BUILTIN(StringPrototypeIndexOf, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    tmp6 = CodeStubAssembler(state_).ToString_Inline(TNode<Context>{parameter0}, TNode<JSAny>{tmp1});
  ...
  if (block1.is_used()) {
    ...
    tmp11 = ClampToIndexRange_0(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp3}, TNode<UintPtrT>{tmp10});
