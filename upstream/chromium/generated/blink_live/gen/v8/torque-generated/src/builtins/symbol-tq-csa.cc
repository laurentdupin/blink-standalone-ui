...
// https://crsrc.org/c/v8/src/builtins/symbol.tq?l=9&c=1
TNode<Symbol> ThisSymbolValue_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSAny> p_receiver, const char* p_method) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = CodeStubAssembler(state_).ToThisValue(TNode<Context>{p_context}, TNode<JSAny>{p_receiver}, CastIfEnumClass<PrimitiveType>(PrimitiveType::kSymbol), p_method);
...
// https://crsrc.org/c/v8/src/builtins/symbol.tq?l=12&c=10
TNode<Symbol> UnsafeCast_Symbol_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = Is_Symbol_Object_0(state_, TNode<Context>{p_context}, TNode<Object>{p_o});
