...
TF_BUILTIN(PromiseConstructor, CodeStubAssembler) {
  ...
  compiler::CodeAssemblerParameterizedLabel<JSAny, Union<JSMessageObject, TheHole>> block11(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
  ...
  if (block1.is_used()) {
    ca_.Bind(&block1);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kPromiseNewTargetUndefined));
  }
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kResolverNotAFunction), TNode<Object>{parameter3});
  }
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    tmp9 = CodeStubAssembler(state_).SmiConstant(CastIfEnumClass<int31_t>(v8::Isolate::kPromiseConstructorReturnedUndefined));
  ...
  TNode<Union<JSMessageObject, TheHole>> phi_bb11_12;
  TNode<Context> tmp29;
  TNode<IntPtrT> tmp30;
...
TNode<NativeContext> UnsafeCast_NativeContext_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = Is_NativeContext_Object_0(state_, TNode<Context>{p_context}, TNode<Object>{p_o});
