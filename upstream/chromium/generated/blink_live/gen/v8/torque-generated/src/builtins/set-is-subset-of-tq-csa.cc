...
TF_BUILTIN(SetPrototypeIsSubsetOf, CodeStubAssembler) {
  ...
  if (block23.is_used()) {
    ...
    tmp29 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{parameter0}, TNode<OrderedHashSet>{tmp19}, TNode<JSAny>{tmp26});
  ...
  if (block39.is_used()) {
    ...
    tmp44 = CollectionsBuiltinsAssembler(state_).TableHasKey(TNode<Context>{parameter0}, TNode<OrderedHashMap>{tmp34}, TNode<JSAny>{tmp41});
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).PrintErr("halting because of unreachable code at src/builtins/set-is-subset-of.tq:98:3");
