...
TF_BUILTIN(TypedArrayPrototypeFind, CodeStubAssembler) {
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kNotTypedArray), kBuiltinNameFind_0(state_));
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kTypedArrayValidateErrorOperation), kBuiltinNameFind_0(state_));
  }
  ...
  if (block11.is_used()) {
    ...
    tmp13 = FindAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructValidJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>>{tmp7}, TNode<JSAny>{tmp12});
