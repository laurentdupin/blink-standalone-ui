...
// https://crsrc.org/c/v8/src/builtins/typed-array-foreach.tq?l=8&c=1
...
// https://crsrc.org/c/v8/src/builtins/typed-array-foreach.tq?l=10&c=1
TNode<Undefined> ForEachAllElements_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TorqueStructValidJSTypedArrayAndLength_0 p_attachedArrayAndLength, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>> p_callbackfn, TNode<JSAny> p_thisArg) {
  compiler::CodeAssembler ca_(state_);
...
tmp9 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(1)), tmp2, TNode<Object>(), tmp8, phi_bb14_8));
  ...
  if (block6.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb6_8}, TNode<UintPtrT>{tmp13});
    ca_.Goto(&block4, phi_bb6_6, tmp14);
  }
...
TF_BUILTIN(TypedArrayPrototypeForEach, CodeStubAssembler) {
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kNotTypedArray), kBuiltinNameForEach_0(state_));
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kTypedArrayValidateErrorOperation), kBuiltinNameForEach_0(state_));
  }
  ...
  if (block11.is_used()) {
    ...
    tmp13 = ForEachAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructValidJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>>{tmp7}, TNode<JSAny>{tmp12});
