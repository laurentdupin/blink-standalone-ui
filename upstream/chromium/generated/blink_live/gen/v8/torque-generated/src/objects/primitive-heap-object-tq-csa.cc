...
// https://crsrc.org/c/v8/src/objects/primitive-heap-object.tq?l=5&c=1
TNode<PrimitiveHeapObject> Cast_PrimitiveHeapObject_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_obj, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  TNode<PrimitiveHeapObject> tmp0;
  if (block0.is_used()) {
    ca_.Bind(&block0);
    compiler::CodeAssemblerLabel label1(&ca_);
    tmp0 = DownCastForTorqueClass_PrimitiveHeapObject_0(state_, TNode<HeapObject>{p_obj}, &label1);
    ca_.Goto(&block3);
    if (label1.is_used()) {
      ca_.Bind(&label1);
      ca_.Goto(&block4);
  ...
  if (block3.is_used()) {
    ...
    ca_.Bind(&block5);
  return TNode<PrimitiveHeapObject>{tmp0};
}
...
TNode<PrimitiveHeapObject> DownCastForTorqueClass_PrimitiveHeapObject_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ...
    if (((CodeStubAssembler(state_).ConstexprInt31Equal(static_cast<InstanceType>(0), static_cast<InstanceType>(131))))) {
      ca_.Goto(&block3);
    } else {
      ca_.Goto(&block4);
    }
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    if ((CodeStubAssembler(state_).ClassHasMapConstant<PrimitiveHeapObject>())) {
      ca_.Goto(&block6);
    } else {
      ca_.Goto(&block7);
    }
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp2 = CodeStubAssembler(state_).GetClassMapConstant<PrimitiveHeapObject>();
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp8 = FromConstexpr_int32_constexpr_int32_0(state_, (CodeStubAssembler(state_).ConstexprUint32Sub(static_cast<InstanceType>(131), static_cast<InstanceType>(0))));
  ...
  TNode<PrimitiveHeapObject> tmp20;
  if (block5.is_used()) {
    ca_.Bind(&block5);
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block15);
  return TNode<PrimitiveHeapObject>{tmp20};
}
