...
tmp9 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(1)), tmp2, TNode<Object>(), tmp8, phi_bb14_8));
  ...
  if (block18.is_used()) {
    ...
    tmp16 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb18_8}, TNode<UintPtrT>{tmp15});
    ca_.Goto(&block4, phi_bb6_6, tmp16);
  }
...
TF_BUILTIN(TypedArrayPrototypeSome, CodeStubAssembler) {
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kNotTypedArray), kBuiltinNameSome_0(state_));
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kTypedArrayValidateErrorOperation), kBuiltinNameSome_0(state_));
  }
  ...
  if (block11.is_used()) {
    ...
    tmp13 = SomeAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructValidJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>>{tmp7}, TNode<JSAny>{tmp12});
