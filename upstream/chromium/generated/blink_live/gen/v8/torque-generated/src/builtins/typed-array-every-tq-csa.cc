...
// https://crsrc.org/c/v8/src/builtins/typed-array-every.tq?l=8&c=1
...
// https://crsrc.org/c/v8/src/builtins/typed-array-every.tq?l=11&c=1
TNode<Boolean> EveryAllElements_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TorqueStructValidJSTypedArrayAndLength_0 p_attachedArrayAndLength, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>> p_callbackfn, TNode<JSAny> p_thisArg) {
  compiler::CodeAssembler ca_(state_);
...
tmp9 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(1)), tmp2, TNode<Object>(), tmp8, phi_bb14_8));
  ...
  if (block6.is_used()) {
    ...
    tmp12 = CodeStubAssembler(state_).Call(TNode<Context>{p_context}, TNode<JSAny>{p_callbackfn}, TNode<JSAny>{p_thisArg}, TNode<JSAny>{phi_bb6_9}, TNode<JSAny>{tmp11}, TNode<JSAny>{tmp0});
    tmp13 = ToBoolean_0(state_, TNode<JSAny>{tmp12});
    tmp14 = CodeStubAssembler(state_).Word32BinaryNot(TNode<BoolT>{tmp13});
    ca_.Branch(tmp14, &block17, std::vector<compiler::Node*>{phi_bb6_8}, &block18, std::vector<compiler::Node*>{phi_bb6_8});
  }
  ...
  if (block18.is_used()) {
    ...
    tmp17 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb18_8}, TNode<UintPtrT>{tmp16});
    ca_.Goto(&block4, phi_bb6_6, tmp17);
  }
...
TF_BUILTIN(TypedArrayPrototypeEvery, CodeStubAssembler) {
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kNotTypedArray), kBuiltinNameEvery_0(state_));
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kTypedArrayValidateErrorOperation), kBuiltinNameEvery_0(state_));
  }
  ...
  if (block11.is_used()) {
    ...
    tmp13 = EveryAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructValidJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>>{tmp7}, TNode<JSAny>{tmp12});
    arguments.PopAndReturn(tmp13);
  }
}

// https://crsrc.org/c/v8/src/builtins/typed-array-every.tq?l=61&c=33
