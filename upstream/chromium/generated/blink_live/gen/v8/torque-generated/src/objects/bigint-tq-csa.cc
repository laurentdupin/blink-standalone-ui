...
// https://crsrc.org/c/v8/src/objects/bigint.tq?l=7&c=1
...
// https://crsrc.org/c/v8/src/objects/bigint.tq?l=15&c=1
TNode<BigInt> Convert_BigInt_MutableBigInt_0(compiler::CodeAssemblerState* state_, TNode<BigInt> p_i) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = IsCanonicalized_0(state_, TNode<BigInt>{p_i});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    {
      ...
      pos_stack.push_back({"src/objects/bigint.tq", 16});
      CodeStubAssembler(state_).FailAssert("Torque assert 'bigint::IsCanonicalized(i)' failed", pos_stack);
    }
  }
  ...
  if (block2.is_used()) {
    ca_.Bind(&block2);
    tmp1 = Convert_BigIntBase_MutableBigInt_0(state_, TNode<BigInt>{p_i});
...
TNode<BigInt> DownCastForTorqueClass_BigIntBase_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block7.is_used()) {
    ...
    tmp6 = FromConstexpr_uint32_constexpr_uint32_0(state_, CastIfEnumClass<uint32_t>(static_cast<InstanceType>(129)));
...
// https://crsrc.org/c/v8/src/objects/bigint.tq?l=17&c=31
