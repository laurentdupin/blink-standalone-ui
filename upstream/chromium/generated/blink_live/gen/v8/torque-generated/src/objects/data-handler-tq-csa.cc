...
TNode<DataHandler> Cast_DataHandler_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_obj, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ...
    tmp0 = DownCastForTorqueClass_DataHandler_0(state_, TNode<HeapObject>{p_obj}, &label1);
    ca_.Goto(&block3);
    if (label1.is_used()) {
      ca_.Bind(&label1);
      ca_.Goto(&block4);
...
TNode<Union<Code, Smi>> LoadDataHandlerSmiHandler_0(compiler::CodeAssemblerState* state_, TNode<DataHandler> p_o) {
  ...
  TNode<IntPtrT> tmp0;
  TNode<Union<Code, Smi>> tmp1;
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = FromConstexpr_intptr_constexpr_int31_0(state_, 4);
    tmp1 = CodeStubAssembler(state_).LoadReference<Union<Code, Smi>>(CodeStubAssembler::Reference{p_o, tmp0});
    ca_.Goto(&block2);
  }

    ca_.Bind(&block2);
  return TNode<Union<Code, Smi>>{tmp1};
}
...
void StoreDataHandlerSmiHandler_0(compiler::CodeAssemblerState* state_, TNode<DataHandler> p_o, TNode<Union<Code, Smi>> p_v) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = FromConstexpr_intptr_constexpr_int31_0(state_, 4);
    CodeStubAssembler(state_).StoreReference<Union<Code, Smi>>(CodeStubAssembler::Reference{p_o, tmp0}, p_v);
    ca_.Goto(&block2);
  }
...
TNode<DataHandler> DownCastForTorqueClass_DataHandler_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ...
    if (((CodeStubAssembler(state_).ConstexprInt31Equal(static_cast<InstanceType>(138), static_cast<InstanceType>(139))))) {
      ca_.Goto(&block3);
    } else {
      ca_.Goto(&block4);
    }
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    if ((CodeStubAssembler(state_).ClassHasMapConstant<DataHandler>())) {
      ca_.Goto(&block6);
    } else {
      ca_.Goto(&block7);
    }
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp2 = CodeStubAssembler(state_).GetClassMapConstant<DataHandler>();
  ...
  if (block4.is_used()) {
    ca_.Bind(&block4);
    tmp8 = FromConstexpr_int32_constexpr_int32_0(state_, (CodeStubAssembler(state_).ConstexprUint32Sub(static_cast<InstanceType>(139), static_cast<InstanceType>(138))));
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block15);
  return TNode<DataHandler>{tmp20};
}
