...
// https://crsrc.org/c/v8/src/builtins/string-iterator.tq?l=7&c=1
...
TF_BUILTIN(StringIteratorPrototypeNext, CodeStubAssembler) {
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp13 = StringBuiltinsAssembler(state_).LoadSurrogatePairAt(TNode<String>{tmp4}, TNode<IntPtrT>{tmp8}, TNode<IntPtrT>{tmp7}, CastIfEnumClass<UnicodeEncoding>(UnicodeEncoding::UTF16));
...
// https://crsrc.org/c/v8/src/builtins/string-iterator.tq?l=31&c=20
