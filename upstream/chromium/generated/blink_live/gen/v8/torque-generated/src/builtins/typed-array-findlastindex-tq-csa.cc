...
// https://crsrc.org/c/v8/src/builtins/typed-array-findlastindex.tq?l=8&c=1
...
// https://crsrc.org/c/v8/src/builtins/typed-array-findlastindex.tq?l=12&c=1
TNode<Number> FindLastIndexAllElements_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TorqueStructValidJSTypedArrayAndLength_0 p_attachedArrayAndLength, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>> p_predicate, TNode<JSAny> p_thisArg) {
  compiler::CodeAssembler ca_(state_);
...
tmp11 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(1)), tmp2, TNode<Object>(), tmp10, tmp4));
  ...
  return TNode<Number>{phi_bb18_5};
}
...
TF_BUILTIN(TypedArrayPrototypeFindLastIndex, CodeStubAssembler) {
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kNotTypedArray), kBuiltinNameFindLastIndex_0(state_));
  }
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kTypedArrayValidateErrorOperation), kBuiltinNameFindLastIndex_0(state_));
  }
  ...
  if (block11.is_used()) {
    ...
    tmp13 = FindLastIndexAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructValidJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<Union<JSBoundFunction, JSFunction, JSObject, JSProxy, JSWrappedFunction>>{tmp7}, TNode<JSAny>{tmp12});
