...
TF_BUILTIN(RegExpSplit, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    tmp7 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp>{parameter1}, CastIfEnumClass<JSRegExp::Flag>(JSRegExp::Flag::kSticky));
...
TF_BUILTIN(RegExpPrototypeSplit, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    CodeStubAssembler(state_).ThrowIfNotJSReceiver(TNode<Context>{parameter0}, TNode<JSAny>{parameter1}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kIncompatibleMethodReceiver), "RegExp.prototype.@@split");
