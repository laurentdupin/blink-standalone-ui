...
// https://crsrc.org/c/v8/src/builtins/internal-coverage.tq?l=11&c=1
...
// https://crsrc.org/c/v8/src/builtins/internal-coverage.tq?l=20&c=1
void IncrementBlockCount_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<CoverageInfo> p_coverageInfo, TNode<Smi> p_slot) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = Convert_int32_Smi_0(state_, TNode<Smi>{p_slot});
    tmp1 = FromConstexpr_intptr_constexpr_int31_0(state_, 4);
    tmp2 = CodeStubAssembler(state_).LoadReference<Int32T>(CodeStubAssembler::Reference{p_coverageInfo, tmp1});
    tmp3 = CodeStubAssembler(state_).Int32LessThan(TNode<Int32T>{tmp0}, TNode<Int32T>{tmp2});
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    {
      ...
      pos_stack.push_back({"src/builtins/internal-coverage.tq", 22});
      CodeStubAssembler(state_).FailAssert("Torque assert 'Convert<int32>(slot) < coverageInfo.slot_count' failed", pos_stack);
    }
  }
  ...
  if (block2.is_used()) {
    ca_.Bind(&block2);
    std::tie(tmp4, tmp5, tmp6) = FieldSliceCoverageInfoSlots_0(state_, TNode<CoverageInfo>{p_coverageInfo}).Flatten();
    tmp7 = Convert_intptr_Smi_0(state_, TNode<Smi>{p_slot});
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp11 = TimesSizeOf_CoverageInfoSlot_0(state_, TNode<IntPtrT>{tmp7});
    tmp12 = CodeStubAssembler(state_).IntPtrAdd(TNode<IntPtrT>{tmp5}, TNode<IntPtrT>{tmp11});
    std::tie(tmp13, tmp14) = NewReference_CoverageInfoSlot_0(state_, TNode<Union<HeapObject, TaggedIndex>>{tmp4}, TNode<IntPtrT>{tmp12}).Flatten();
    ...
    tmp17 = CodeStubAssembler(state_).LoadReference<Int32T>(CodeStubAssembler::Reference{tmp13, tmp16});
    tmp18 = FromConstexpr_int32_constexpr_int31_0(state_, 1);
    tmp19 = CodeStubAssembler(state_).Int32Add(TNode<Int32T>{tmp17}, TNode<Int32T>{tmp18});
    CodeStubAssembler(state_).StoreReference<Int32T>(CodeStubAssembler::Reference{tmp13, tmp16}, tmp19);
    ca_.Goto(&block12);
  }
...
// https://crsrc.org/c/v8/src/builtins/internal-coverage.tq?l=15&c=24
