...
TF_BUILTIN(TypedArrayPrototypeAt, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = TypedArrayBuiltinsAssembler(state_).ValidateTypedArrayAndGetLength(TNode<Context>{parameter0}, TNode<JSAny>{parameter1}, "%TypedArray%.prototype.at", CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kRead));
  ...
  if (block3.is_used()) {
    ...
    tmp7 = NumberIsLessThan_0(state_, TNode<Number>{phi_bb3_5}, TNode<Number>{tmp6});
  ...
  if (block8.is_used()) {
    ca_.Bind(&block8);
    tmp9 = NumberIsGreaterThanOrEqual_0(state_, TNode<Number>{phi_bb3_5}, TNode<Number>{tmp1});
