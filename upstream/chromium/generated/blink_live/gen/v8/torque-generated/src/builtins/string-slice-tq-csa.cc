...
TF_BUILTIN(StringPrototypeSlice, CodeStubAssembler) {
  ...
  if (block5.is_used()) {
    ca_.Bind(&block5);
    tmp13 = ConvertAndClampRelativeIndex_1(state_, TNode<Context>{parameter0}, TNode<JSAny>{tmp10}, TNode<UintPtrT>{tmp2});
  ...
  if (block7.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).UintPtrLessThanOrEqual(TNode<UintPtrT>{phi_bb7_11}, TNode<UintPtrT>{phi_bb3_9});
  ...
  if (block10.is_used()) {
    ca_.Bind(&block10);
    tmp16 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp1}, TNode<UintPtrT>{phi_bb3_9}, TNode<UintPtrT>{phi_bb7_11});
