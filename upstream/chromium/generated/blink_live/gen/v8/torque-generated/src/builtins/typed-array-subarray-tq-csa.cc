...
TF_BUILTIN(TypedArrayPrototypeSubArray, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    tmp4 = CodeStubAssembler(state_).LoadJSTypedArrayLengthAndValidate(TNode<JSTypedArray>{tmp0}, CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kRead), &label5);
    ca_.Goto(&block7);
    if (label5.is_used()) {
      ca_.Bind(&label5);
      ca_.Goto(&block8);
  ...
  if (block26.is_used()) {
    ...
    CodeStubAssembler(state_).ThrowRangeError(TNode<Context>{parameter0}, CastIfEnumClass<MessageTemplate>(MessageTemplate::kInvalidArrayBufferLength));
  }
  ...
  if (block27.is_used()) {
    ...
    tmp36 = TypedArraySpeciesCreateByBuffer_0(state_, TNode<Context>{parameter0}, "%TypedArray%.prototype.subarray", TNode<JSTypedArray>{tmp0}, TNode<JSArrayBuffer>{tmp3}, TNode<UintPtrT>{tmp35}, TNode<Union<HeapNumber, Smi, Undefined>>{phi_bb27_13}, CastIfEnumClass<TypedArrayAccessMode>(TypedArrayAccessMode::kRead));
