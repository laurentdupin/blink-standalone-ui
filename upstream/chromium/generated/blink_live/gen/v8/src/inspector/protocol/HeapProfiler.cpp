...
struct startSamplingParams : public v8_crdtp::DeserializableProtocolObject<startSamplingParams> {
    ...
    std::optional<double> stackDepth;
...
V8_CRDTP_BEGIN_DESERIALIZER(startSamplingParams)
    ...
    V8_CRDTP_DESERIALIZE_FIELD_OPT("stackDepth", stackDepth),
V8_CRDTP_END_DESERIALIZER()
...
void DomainDispatcherImpl::startSampling(const v8_crdtp::Dispatchable& dispatchable)
{
    ...
    std::unique_ptr<DomainDispatcher::WeakPtr> weak = weakPtr();
    DispatchResponse response = m_backend->startSampling(std::move(params.samplingInterval), std::move(params.stackDepth), std::move(params.includeObjectsCollectedByMajorGC), std::move(params.includeObjectsCollectedByMinorGC));
    if (response.IsFallThrough()) {
