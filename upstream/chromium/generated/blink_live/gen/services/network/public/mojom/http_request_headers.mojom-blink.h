...
class BLINK_PLATFORM_EXPORT HttpRequestHeadersUpdateParams {
 public:
  template <typename T>
  using EnableIfSame = std::enable_if_t<std::is_same<HttpRequestHeadersUpdateParams, T>::value>;
  using DataView = HttpRequestHeadersUpdateParamsDataView;
  using Data_ = internal::HttpRequestHeadersUpdateParams_Data;

  template <typename... Args>
  static HttpRequestHeadersUpdateParamsPtr New(Args&&... args) {
    return HttpRequestHeadersUpdateParamsPtr(
        std::in_place, std::forward<Args>(args)...);
  }

  template <typename U>
  static HttpRequestHeadersUpdateParamsPtr From(const U& u) {
    return mojo::TypeConverter<HttpRequestHeadersUpdateParamsPtr, U>::Convert(u);
  }

  template <typename U>
  U To() const {
    return mojo::TypeConverter<U, HttpRequestHeadersUpdateParams>::Convert(*this);
  }
  ...
  HttpRequestHeadersUpdateParams(
      ::blink::Vector<::blink::String> removed_headers,
      HttpRequestHeadersPtr modified_headers,
      HttpRequestHeadersPtr modified_cors_exempt_headers);

HttpRequestHeadersUpdateParams(const HttpRequestHeadersUpdateParams&) = delete;
HttpRequestHeadersUpdateParams& operator=(const HttpRequestHeadersUpdateParams&) = delete;
  ...
  // bindings generator does not need to know whether Clone() or copy
  // constructor/assignment are available for members.
  template <typename StructPtrType = HttpRequestHeadersUpdateParamsPtr>
  HttpRequestHeadersUpdateParamsPtr Clone() const;

  // Equals() is a template so it is only instantiated if it is used. Thus, the
  // bindings generator does not need to know whether Equals() or == operator
  // are available for members.
  template <typename T, HttpRequestHeadersUpdateParams::EnableIfSame<T>* = nullptr>
  bool Equals(const T& other) const;
  ...
  static ::blink::Vector<uint8_t> Serialize(UserType* input) {
    return mojo::internal::SerializeImpl<
        HttpRequestHeadersUpdateParams::DataView, ::blink::Vector<uint8_t>, send_validation>(input);
  }

  template <typename UserType>
  static ::blink::Vector<uint8_t> Serialize(UserType* input) {
    return mojo::internal::SerializeImpl<
        HttpRequestHeadersUpdateParams::DataView, ::blink::Vector<uint8_t>>(input);
  }

  template <typename UserType>
  static mojo::Message SerializeAsMessage(UserType* input) {
    return mojo::internal::SerializeAsMessageImpl<
        HttpRequestHeadersUpdateParams::DataView>(input);
  }
  ...
  static mojo::Message WrapAsMessage(UserType input) {
    return mojo::Message(std::make_unique<
        internal::HttpRequestHeadersUpdateParams_UnserializedMessageContext<
            UserType, HttpRequestHeadersUpdateParams::DataView>>(0, 0, std::move(input)),
        MOJO_CREATE_MESSAGE_FLAG_NONE);
  }
  ...
  static bool Deserialize(const void* data,
                          ...
                          UserType* output) {
    mojo::Message message;
    return mojo::internal::DeserializeImpl<HttpRequestHeadersUpdateParams::DataView>(
        message, data, data_num_bytes, output, Validate);
  }
  ...
  static bool Deserialize(base::span<const uint8_t> input,
                          UserType* output) {
    return HttpRequestHeadersUpdateParams::Deserialize(
        input.empty() ? nullptr : input.data(), input.size(), output);
  }
  ...
  static bool DeserializeFromMessage(mojo::Message input,
    ...
    auto context = input.TakeUnserializedContext<
        internal::HttpRequestHeadersUpdateParams_UnserializedMessageContext<
            UserType, HttpRequestHeadersUpdateParams::DataView>>();
    if (context) {
      *output = std::move(context->TakeData());
      return true;
    }
...
template <typename StructPtrType>
HttpRequestHeadersUpdateParamsPtr HttpRequestHeadersUpdateParams::Clone() const {
  return New(
      mojo::Clone(removed_headers),
      mojo::Clone(modified_headers),
      mojo::Clone(modified_cors_exempt_headers)
  );
}

template <typename T, HttpRequestHeadersUpdateParams::EnableIfSame<T>*>
bool HttpRequestHeadersUpdateParams::Equals(const T& other_struct) const {
  if (!mojo::Equals(this->removed_headers, other_struct.removed_headers))
    return false;
  if (!mojo::Equals(this->modified_headers, other_struct.modified_headers))
    return false;
  if (!mojo::Equals(this->modified_cors_exempt_headers, other_struct.modified_cors_exempt_headers))
    return false;
  return true;
}
...
template <typename T, HttpRequestHeadersUpdateParams::EnableIfSame<T>*>
bool operator<(const T& lhs, const T& rhs) {
  if (lhs.removed_headers < rhs.removed_headers)
    return true;
  if (rhs.removed_headers < lhs.removed_headers)
    return false;
  if (lhs.modified_headers < rhs.modified_headers)
    return true;
  if (rhs.modified_headers < lhs.modified_headers)
    return false;
  if (lhs.modified_cors_exempt_headers < rhs.modified_cors_exempt_headers)
    return true;
  if (rhs.modified_cors_exempt_headers < lhs.modified_cors_exempt_headers)
    return false;
  return false;
}
...
template <>
struct BLINK_PLATFORM_EXPORT StructTraits<::network::mojom::blink::HttpRequestHeadersUpdateParams::DataView,
                                         ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr> {
  static bool IsNull(const ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr& input) { return !input; }
  static void SetToNull(::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr* output) { output->reset(); }

  static const decltype(::network::mojom::blink::HttpRequestHeadersUpdateParams::removed_headers)& removed_headers(
      const ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr& input) {
    return input->removed_headers;
  }

  static const decltype(::network::mojom::blink::HttpRequestHeadersUpdateParams::modified_headers)& modified_headers(
      const ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr& input) {
    return input->modified_headers;
  }

  static const decltype(::network::mojom::blink::HttpRequestHeadersUpdateParams::modified_cors_exempt_headers)& modified_cors_exempt_headers(
      const ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr& input) {
    return input->modified_cors_exempt_headers;
  }

  static bool Read(::network::mojom::blink::HttpRequestHeadersUpdateParams::DataView input, ::network::mojom::blink::HttpRequestHeadersUpdateParamsPtr* output);
};
