...
void TransferableDirectory::set_path(::mojo_base::mojom::blink::FilePathPtr path) {
  if (tag_ == Tag::kPath) {
    data_.path = std::move(path);
  } else {
    DestroyActive();
    tag_ = Tag::kPath;
    new (&data_.path) ::mojo_base::mojom::blink::FilePathPtr(
        std::move(path));
  }
}
...
bool UnionTraits<::network::mojom::blink::TransferableDirectory::DataView, ::network::mojom::blink::TransferableDirectoryPtr>::Read(
  ...
  switch (input.tag()) {
    ...
    case Tag::kPath: {
      ::mojo_base::mojom::blink::FilePathPtr result_path{};
