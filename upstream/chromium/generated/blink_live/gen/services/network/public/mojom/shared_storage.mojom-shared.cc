...
// static
bool SharedStorageBatchUpdateMethodsArgument_Data::Validate(
    const void* data,
    mojo::internal::ValidationContext* validation_context) {
  if (!data)
    return true;
  ...
  // NOTE: The memory backing |object| may be smaller than |sizeof(*object)| if
  // the message comes from an older version.
  [[maybe_unused]] const SharedStorageBatchUpdateMethodsArgument_Data* object =
      static_cast<const SharedStorageBatchUpdateMethodsArgument_Data*>(data);
...
SharedStorageBatchUpdateMethodsArgument_Data::SharedStorageBatchUpdateMethodsArgument_Data()
    : header_({sizeof(*this), 0}) {}
