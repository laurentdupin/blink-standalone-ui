...
class  P2PSocketManager_GetHostAddress_Params_Data {
 public:
  ...
  uint8_t address_family_$flag : 1;
  uint8_t enable_mdns : 1;
  uint8_t pad2_[3];
  int32_t address_family_$value;
...
class P2PSocketManager_GetHostAddress_ParamsDataView {
 public:
  ...
  [[nodiscard]] bool ReadAddressFamily(UserType* output) const {
    if (!data_->address_family_$flag) {
      *output = std::nullopt;
      return true;
    }

    return mojo::internal::Deserialize<::network::mojom::AddressFamily>(
        data_->address_family_$value, &output->emplace());
  }
  std::optional<::network::mojom::AddressFamily> address_family() const {
    if (!data_->address_family_$flag) {
      return std::nullopt;
    }
    return ::mojo::internal::ToKnownEnumValueHelper(
          static_cast<::network::mojom::AddressFamily>(data_->address_family_$value));
  }
