...
NOINLINE static const char* IPAddressChangeTypeToStringHelper(IPAddressChangeType value) {
  // Defined in a helper function to ensure that Clang generates a lookup table.
  switch(value) {
    case IPAddressChangeType::IP_ADDRESS_CHANGE_NONE:
      return "IP_ADDRESS_CHANGE_NONE";
    case IPAddressChangeType::IP_ADDRESS_CHANGE_NORMAL:
      return "IP_ADDRESS_CHANGE_NORMAL";
    case IPAddressChangeType::IP_ADDRESS_CHANGE_IPV6_TEMPADDR:
      return "{IP_ADDRESS_CHANGE_IPV6_TEMPADDR, IP_ADDRESS_CHANGE_LAST}";
    default:
      return nullptr;
  }
}

std::string IPAddressChangeTypeToString(IPAddressChangeType value) {
  const char *str = IPAddressChangeTypeToStringHelper(value);
  if (!str) {
    return base::StringPrintf("Unknown IPAddressChangeType value: %i", static_cast<int32_t>(value));
  }
  return str;
}

std::ostream& operator<<(std::ostream& os, IPAddressChangeType value) {
  return os << IPAddressChangeTypeToString(value);
}
...
// static
void TraceFormatTraits<::network::mojom::IPAddressChangeType>::WriteIntoTrace(
   perfetto::TracedValue context, ::network::mojom::IPAddressChangeType value) {
  return std::move(context).WriteString(::network::mojom::IPAddressChangeTypeToString(value));
}
