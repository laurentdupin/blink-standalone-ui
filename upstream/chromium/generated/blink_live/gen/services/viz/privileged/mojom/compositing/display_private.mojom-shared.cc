...
// static
bool DisplayClient_CreateLayeredWindowUpdater_Params_Data::Validate(
    const void* data,
    mojo::internal::ValidationContext* validation_context) {
  if (!data)
    return true;
...
// static
bool DisplayClient_AddChildWindowToBrowser_Params_Data::Validate(
    const void* data,
    mojo::internal::ValidationContext* validation_context) {
  if (!data)
    return true;
  ...
  if (!mojo::internal::ValidatePointerNonNullable(
          object->child_window, 1, validation_context)) {
    return false;
  }
