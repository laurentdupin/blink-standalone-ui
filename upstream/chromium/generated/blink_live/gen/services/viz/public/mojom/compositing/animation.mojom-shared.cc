...
bool AnimationKeyframeModel_Data::Validate(
  ...
  if (!mojo::internal::ValidatePointerNonNullable(
          object->start_delay, 13, validation_context)) {
    return false;
  }
  if (!mojo::internal::ValidateStruct(object->start_delay, validation_context))
    return false;

  if (!mojo::internal::ValidateStruct(object->hold_time, validation_context))
    return false;
