...
JSONReader::Result DecodeJSONWithoutRust(std::string_view json,
                                         int options,
                                         size_t max_depth) {
  return base::unexpected(JSONReader::Error{
      .message = "JSONReader is disabled in the no-Rust standalone probe",
      .line = 1,
      .column = 1,
  });
}
...
std::optional<Value> JSONReader::Read(std::string_view json,
                                      ...
                                      size_t max_depth) {
#if defined(STANDALONE_RENDERER_NO_RUST)
  JSONReader::Result result = DecodeJSONWithoutRust(json, options, max_depth);
#else
...
JSONReader::Result JSONReader::ReadAndReturnValueWithError(
    ...
    int options) {
#if defined(STANDALONE_RENDERER_NO_RUST)
  return DecodeJSONWithoutRust(json, options, internal::kAbsoluteMaxDepth);
#else
