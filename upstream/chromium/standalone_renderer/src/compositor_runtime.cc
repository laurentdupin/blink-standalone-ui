...
class StandaloneCompositorRuntimeImpl final : public StandaloneCompositorRuntime {
 public:
  ...
  CompositorFrameResult AdvanceFrame(const FrameInput& input) override {
    ...
    if (input.request_d3d12_gpu_frame) {
      probe::StandaloneBlinkLiveFrameBridgeRequestD3D12GpuFrameForStandaloneRenderer();
    } else if (input.request_vulkan_gpu_frame) {
  ...
  std::string RunBorrowedVkImageBackingSmokeForTesting() override {
    ScopedStandaloneResourceProviderContext scoped_resources(
        resource_provider_context_id_);
    ScopedTypefaceResourceRegistryContext scoped_typefaces(
        typeface_registry_context_id_);
  ...
  ExternalGpuTargetCopyResult RenderExternalVkImageToTarget(
      const ExternalVulkanImageTarget& vulkan_image) override {
    ScopedStandaloneResourceProviderContext scoped_resources(
        resource_provider_context_id_);
    ScopedTypefaceResourceRegistryContext scoped_typefaces(
  ...
  ExternalGpuTargetCopyResult RenderBackdropMaskToExternalVkImage(
      const ExternalVulkanImageTarget& vulkan_image) override {
    ScopedStandaloneResourceProviderContext scoped_resources(
        resource_provider_context_id_);
    ScopedTypefaceResourceRegistryContext scoped_typefaces(
  ...
  std::string RunExternalD3D12RenderCopyForTesting(
      void* d3d12_resource,
  ...
  ExternalGpuTargetCopyResult RenderExternalD3D12ToTarget(
      void* d3d12_resource,
      void* shared_handle,
      int width,
      int height) override {
  ...
  ExternalGpuTargetCopyResult RenderBackdropMaskToExternalD3D12Target(
      void* d3d12_resource,
      void* shared_handle,
      int width,
      int height) override {
