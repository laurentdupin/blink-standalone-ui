...
void PrintUsage() {
  std::fprintf(
      ...
      "[--gpu-vulkan-backdrop-mask-prototype-smoke] "
      "[--gpu-external-vulkan-runtime-target-smoke] "
      "[--gpu-borrowed-d3d12-render-copy-smoke] "
      "[--gpu-d3d12-backdrop-mask-prototype-smoke] "
      ...
      "[--c-api-vulkan-external-target-async-backdrop-mask-smoke] "
      "[--c-api-vulkan-external-target-backdrop-mask-resize-bounce-smoke] "
      "[--c-api-vulkan-external-target-backdrop-mask-dsf-resize-smoke] "
      ...
      "[--c-api-d3d12-external-target-async-backdrop-mask-smoke] "
...
int RunCApiExternalGpuTargetSmoke(uint32_t backend,
                                  ...
                                  bool exercise_backdrop_mask_resize_bounce =
                                      false,
                                  bool exercise_dsf_resize_sequence = false,
                                  bool exercise_async_render_sequence = false,
                                  bool exercise_async_resize_stale_sequence =
                                      false,
                                  int async_dirty_timing_iterations = 0,
                                  ...
                                  bool dedicated_cancel_before_completion =
                                      false,
                                  bool dedicated_cancel_resize_backdrop_mask =
                                      false) {
  ...
  uint32_t active_logical_height =
      exercise_dsf_resize_sequence ? 720 : height;
  float active_device_scale_factor =
      exercise_dsf_resize_sequence
          ? static_cast<float>(active_width) /
                static_cast<float>(active_logical_width)
          : 1.0f;
  blink_standalone_renderer_config_t config = {};
  config.width = active_logical_width;
  config.height = active_logical_height;
  config.device_scale_factor = active_device_scale_factor;
  config.no_script_profile = 1;
  blink_standalone_renderer_t* renderer = nullptr;
  blink_standalone_status_code_t status =
  ...
  target.common.logical_width = active_logical_width;
  target.common.logical_height = active_logical_height;
  target.common.physical_width = width;
  target.common.physical_height = height;
  target.common.device_scale_factor = active_device_scale_factor;
  ...
  Microsoft::WRL::ComPtr<ID3D12Resource> d3d12_mask_resource;
  Microsoft::WRL::ComPtr<ID3D12Resource1> d3d12_resource_alias;
  HANDLE d3d12_shared_handle = nullptr;
  HANDLE d3d12_mask_shared_handle = nullptr;
  auto cleanup_d3d12_targets = [&] {
    if (d3d12_shared_handle) {
      CloseHandle(d3d12_shared_handle);
      d3d12_shared_handle = nullptr;
    }
    if (d3d12_mask_shared_handle) {
      CloseHandle(d3d12_mask_shared_handle);
      d3d12_mask_shared_handle = nullptr;
    }
    d3d12_resource.Reset();
    d3d12_mask_resource.Reset();
    d3d12_resource_alias.Reset();
    target.d3d12.d3d12_resource = nullptr;
    target.d3d12.shared_handle = nullptr;
    mask_target.d3d12.d3d12_resource = nullptr;
    mask_target.d3d12.shared_handle = nullptr;
  };
#endif
  ...
  std::unique_ptr<gpu::VulkanImage> vulkan_mask_image;
  ...
  auto destroy_vulkan_target = [&] {
    ...
    if (vulkan_mask_image) {
      vulkan_mask_image->Destroy();
      vulkan_mask_image.reset();
    }
  ...
  if (backend == BLINK_STANDALONE_GPU_BACKEND_VULKAN) {
    ...
    create_vulkan_target = [&](uint32_t target_width,
      ...
      if (exercise_gpu_backdrop_mask) {
        vulkan_mask_image = gpu::VulkanImage::Create(
            vulkan_owner_queue.get(),
            gfx::Size(static_cast<int>(target_width),
                      static_cast<int>(target_height)),
            VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
                ...
                VK_IMAGE_USAGE_TRANSFER_DST_BIT);
        if (!vulkan_mask_image ||
            vulkan_mask_image->image() == VK_NULL_HANDLE) {
          return false;
        }
        mask_target.common.logical_width = active_logical_width;
        mask_target.common.logical_height = active_logical_height;
        mask_target.common.physical_width = target_width;
        mask_target.common.physical_height = target_height;
        mask_target.common.device_scale_factor = active_device_scale_factor;
        mask_target.vulkan.vk_image = vulkan_mask_image->image();
        mask_target.vulkan.vk_device = vulkan_owner_queue->GetVulkanDevice();
        mask_target.vulkan.vk_physical_device =
            vulkan_owner_queue->GetVulkanPhysicalDevice();
        mask_target.vulkan.vk_device_memory =
            vulkan_mask_image->device_memory();
        mask_target.vulkan.vk_format = VK_FORMAT_R8G8B8A8_UNORM;
        mask_target.vulkan.width = target_width;
        mask_target.vulkan.height = target_height;
        mask_target.vulkan.current_layout = VK_IMAGE_LAYOUT_UNDEFINED;
        mask_target.vulkan.required_final_layout =
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        mask_target.vulkan.queue_family_index =
            vulkan_owner_queue->GetVulkanQueueIndex();
        mask_target.vulkan.allocation_size = vulkan_mask_image->device_size();
        mask_target.vulkan.memory_type_index =
            vulkan_mask_image->memory_type_index();
        mask_target.vulkan.image_tiling = vulkan_mask_image->image_tiling();
        mask_target.vulkan.image_usage_flags = vulkan_mask_image->usage();
        mask_target.vulkan.sample_count = VK_SAMPLE_COUNT_1_BIT;
        mask_target.vulkan.level_count = 1;
      }
      return true;
    };
    ...
    create_d3d12_target = [&](uint32_t target_width, uint32_t target_height) {
      ...
      if (exercise_gpu_backdrop_mask) {
        create_hr = d3d12_device->CreateCommittedResource(
            &heap_properties, D3D12_HEAP_FLAG_SHARED, &resource_desc,
            D3D12_RESOURCE_STATE_COMMON, nullptr,
            IID_PPV_ARGS(&d3d12_mask_resource));
        if (FAILED(create_hr) || !d3d12_mask_resource) {
          return false;
        }
        create_hr = d3d12_device->CreateSharedHandle(
            d3d12_mask_resource.Get(), nullptr, GENERIC_ALL, nullptr,
            &d3d12_mask_shared_handle);
        if (FAILED(create_hr) || !d3d12_mask_shared_handle) {
          d3d12_mask_resource.Reset();
          return false;
        }
        mask_target.common.logical_width = active_logical_width;
        ...
        mask_target.common.physical_height = target_height;
        mask_target.common.device_scale_factor = active_device_scale_factor;
        mask_target.d3d12.d3d12_device = d3d12_device.Get();
        mask_target.d3d12.d3d12_command_queue = d3d12_queue.Get();
        mask_target.d3d12.d3d12_resource = d3d12_mask_resource.Get();
        mask_target.d3d12.shared_handle = d3d12_mask_shared_handle;
        mask_target.d3d12.dxgi_format = DXGI_FORMAT_R8G8B8A8_UNORM;
        mask_target.d3d12.width = target_width;
        mask_target.d3d12.height = target_height;
    ...
    if (status != BLINK_STANDALONE_STATUS_OK) {
      ...
      cleanup_d3d12_targets();
      blink_standalone_renderer_destroy(renderer);
      if (status == BLINK_STANDALONE_STATUS_UNSUPPORTED) {
        return 0;
      }
      return 1;
    }
  ...
  if (!skip_initial_gpu_target_render) {
  ...
  if (!skip_initial_gpu_target_render &&
      (status != BLINK_STANDALONE_STATUS_OK || result.target_written == 0 ||
  ...
  uint32_t backdrop_mask_pixels = 0;
  uint32_t backdrop_mask_distinct_ids = 0;
  uint32_t backdrop_effect_count = 0;
  auto verify_target_pixels = [&](const char* stage,
    ...
    if (backend == BLINK_STANDALONE_GPU_BACKEND_D3D12) {
      ...
      const uint32_t scaled_box_sample_x = std::min<uint32_t>(
          active_width - 1,
          static_cast<uint32_t>(std::lround(20.0f *
                                            active_device_scale_factor)));
      const uint32_t scaled_box_sample_y = std::min<uint32_t>(
          active_height - 1,
          static_cast<uint32_t>(std::lround(20.0f *
                                            active_device_scale_factor)));
      observed_background = pixel_at(4, 4);
      observed_box = pixel_at(scaled_box_sample_x, scaled_box_sample_y);
  ...
  auto verify_backdrop_mask_pixels = [&](const char* stage) {
    ...
    if (backend == BLINK_STANDALONE_GPU_BACKEND_D3D12) {
      if (!ReadbackD3D12TextureForSmoke(d3d12_device.Get(), d3d12_queue.Get(),
                                        d3d12_mask_resource.Get(),
                                        active_width, active_height, &pixels,
                                        &failure)) {
        std::fprintf(stderr,
                     "%s: failed %s d3d12_mask_readback=0 failure=%s\n",
                     label, stage, failure.c_str());
        return false;
      }
      backdrop_mask_pixels = 0;
      uint32_t seen_ids[256] = {};
      for (uint32_t pixel : pixels) {
        const uint32_t id = (pixel >> 16) & 0xffu;
        const uint32_t coverage = (pixel >> 8) & 0xffu;
        if (id != 0 && coverage != 0) {
          ++backdrop_mask_pixels;
          seen_ids[id] = 1;
        }
      }
      backdrop_mask_distinct_ids = 0;
    ...
    if (backend == BLINK_STANDALONE_GPU_BACKEND_VULKAN) {
      if (!ReadbackVulkanImageForSmoke(vulkan_owner_queue.get(),
                                       vulkan_mask_image.get(), active_width,
                                       active_height, &pixels, &failure)) {
        std::fprintf(stderr,
                     "%s: failed %s vulkan_mask_readback=0 failure=%s\n",
                     label, stage, failure.c_str());
        return false;
      }
      backdrop_mask_pixels = 0;
      uint32_t seen_ids[256] = {};
      for (uint32_t rgba : pixels) {
        const uint32_t id = rgba & 0xffu;
        const uint32_t coverage = (rgba >> 8) & 0xffu;
        if (id != 0 && coverage != 0) {
          ++backdrop_mask_pixels;
          seen_ids[id] = 1;
        }
  ...
  if (!skip_initial_gpu_target_render &&
      !verify_target_pixels("initial", expected_background, expected_box,
                            require_full_nontransparent, /*quiet=*/false)) {
    return cleanup_and_fail();
  }
  if (expect_backdrop_filter_metadata) {
    const size_t backdrop_region_count =
        blink_standalone_renderer_backdrop_filter_region_count(renderer);
    if (backdrop_region_count == 0) {
      std::fprintf(stderr,
                   "%s: expected backdrop metadata after gpu target render, "
                   "got regions=0 hit_count=%zu\n",
                   label,
                   blink_standalone_renderer_hit_metadata_count(renderer));
      return cleanup_and_fail();
    }
    ...
    if (blink_standalone_renderer_get_backdrop_filter_region(renderer, 0,
                                                             &region) ==
        BLINK_STANDALONE_STATUS_OK) {
      for (uint32_t i = 0; i < region.filter_op_count; ++i) {
        if (region.filter_ops[i].type ==
                BLINK_STANDALONE_BACKDROP_FILTER_OP_BLUR &&
            region.filter_ops[i].amount > 0.0f) {
          saw_blur = true;
        }
        if (region.filter_ops[i].type ==
                BLINK_STANDALONE_BACKDROP_FILTER_OP_SATURATE &&
            region.filter_ops[i].amount > 1.0f) {
          saw_saturate = true;
        }
      }
    }
    if (!saw_blur || !saw_saturate) {
      std::fprintf(stderr,
                   "%s: backdrop metadata missing expected operations "
                   "regions=%zu saw_blur=%d saw_saturate=%d\n",
                   label, backdrop_region_count, saw_blur ? 1 : 0,
                   saw_saturate ? 1 : 0);
      return cleanup_and_fail();
    }
  }
  if (exercise_async_render_sequence) {
    constexpr uint32_t kAsyncBox = 0xff00a050u;
    ...
    if (status != BLINK_STANDALONE_STATUS_OK) {
      std::fprintf(stderr,
                   "%s: async style mutation failed status=%d error=%s\n",
                   label, status, blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    blink_standalone_update_result_t async_update = {};
    status = blink_standalone_renderer_update(renderer, 2.000, &async_update);
    if (status != BLINK_STANDALONE_STATUS_OK ||
        async_update.needs_output == 0) {
      std::fprintf(stderr,
                   "%s: async update failed status=%d needs_output=%u "
                   "error=%s\n",
                   label, status, async_update.needs_output,
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    target.common.generation++;
    ...
    target.d3d12.current_state = target.d3d12.required_final_state;
#endif
    mask_target.common.generation = target.common.generation + 1;
    mask_target.vulkan.current_layout =
        mask_target.vulkan.required_final_layout;
#if BUILDFLAG(IS_WIN)
    mask_target.d3d12.current_state = mask_target.d3d12.required_final_state;
#endif
    blink_standalone_gpu_async_render_request_t async_request = {};
    async_request.backend = backend;
    async_request.flags = exercise_gpu_backdrop_mask
                              ? BLINK_STANDALONE_GPU_ASYNC_BACKDROP_MASK_REQUIRED
                              : 0;
    async_request.request_generation = target.common.generation;
    async_request.mask_encoding =
        BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE;
    async_request.main_target = target;
    if (exercise_gpu_backdrop_mask) {
      async_request.backdrop_mask_target = mask_target;
    }
    blink_standalone_gpu_async_render_result_t async_result = {};
    double submit_ms = 0.0;
    constexpr int kMaxAsyncSubmitRetries = 8;
    int submit_retry_count = 0;
    for (; submit_retry_count <= kMaxAsyncSubmitRetries; ++submit_retry_count) {
      const auto submit_start = std::chrono::steady_clock::now();
      status = blink_standalone_renderer_submit_gpu_frame_async(
          renderer, &async_request, &async_result);
      const auto submit_end = std::chrono::steady_clock::now();
      submit_ms += std::chrono::duration<double, std::milli>(
                       submit_end - submit_start)
                       .count();
      if (!(status == BLINK_STANDALONE_STATUS_PENDING &&
            async_result.request_id == 0)) {
        break;
      }
      blink_standalone_update_result_t pending_update = {};
      status = blink_standalone_renderer_update(
          renderer, 2.100 + static_cast<double>(submit_retry_count) * 0.016,
          &pending_update);
      if (status != BLINK_STANDALONE_STATUS_OK) {
        break;
      }
    ...
    if ((status != BLINK_STANDALONE_STATUS_OK &&
        ...
        async_result.request_id == 0) {
      std::fprintf(stderr,
                   "%s: async submit failed status=%d result_status=%u "
                   "state=%u request_id=%llu submit_retries=%d error=%s\n",
                   label, status, async_result.status, async_result.state,
                   static_cast<unsigned long long>(async_result.request_id),
                   submit_retry_count,
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    if (submit_ms > 100.0) {
      std::fprintf(stderr,
                   "%s: async submit exceeded budget elapsed_ms=%.3f "
                   "state=%u\n",
                   label, submit_ms, async_result.state);
      return cleanup_and_fail();
    }
    const uint64_t async_request_id = async_result.request_id;
    constexpr int kMaxAsyncPolls = 120;
    int poll_count = 0;
    const auto poll_start = std::chrono::steady_clock::now();
    while (async_result.state == BLINK_STANDALONE_GPU_ASYNC_STATE_PENDING ||
           async_result.state == BLINK_STANDALONE_GPU_ASYNC_STATE_SUBMITTED) {
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
      status = blink_standalone_renderer_poll_gpu_frame_async(
          renderer, async_request_id, &async_result);
      ++poll_count;
      if (status != BLINK_STANDALONE_STATUS_OK &&
          status != BLINK_STANDALONE_STATUS_PENDING) {
        break;
      }
      if (poll_count > kMaxAsyncPolls) {
        break;
      }
    }
    const auto poll_end = std::chrono::steady_clock::now();
    const double completion_ms =
        std::chrono::duration<double, std::milli>(poll_end - poll_start)
            .count();
    if (status != BLINK_STANDALONE_STATUS_OK ||
        async_result.state != BLINK_STANDALONE_GPU_ASYNC_STATE_COMPLETED ||
        async_result.main_target_written == 0 ||
        async_result.request_id != async_request_id ||
        async_result.main_target_generation != target.common.generation ||
        async_result.physical_width != active_width ||
        async_result.physical_height != active_height) {
      std::fprintf(stderr,
                   "%s: async completion failed status=%d result_status=%u "
                   "state=%u request_id=%llu written=%u size=%ux%u polls=%d "
                   "error=%s\n",
                   label, status, async_result.status, async_result.state,
                   static_cast<unsigned long long>(async_result.request_id),
                   async_result.main_target_written,
                   async_result.physical_width, async_result.physical_height,
                   poll_count, blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    if (exercise_gpu_backdrop_mask &&
        (async_result.backdrop_mask_written == 0 ||
         async_result.effect_count == 0 ||
         async_result.backdrop_mask_generation !=
             mask_target.common.generation)) {
      std::fprintf(stderr,
                   "%s: async backdrop result missing mask/effects "
                   "mask_written=%u effects=%u mask_generation=%llu\n",
                   label, async_result.backdrop_mask_written,
                   async_result.effect_count,
                   static_cast<unsigned long long>(
                       async_result.backdrop_mask_generation));
      return cleanup_and_fail();
    }
    if (!verify_target_pixels("async", expected_background, kAsyncBox,
                              require_full_nontransparent, /*quiet=*/false)) {
      return cleanup_and_fail();
    }
    if (exercise_gpu_backdrop_mask &&
        !verify_backdrop_mask_pixels("async-mask")) {
      return cleanup_and_fail();
    }
    std::printf(
        "%s: async request_id=%llu submit_ms=%.3f completion_ms=%.3f "
        "polls=%d backdrop=%u effects=%u\n",
        label, static_cast<unsigned long long>(async_request_id), submit_ms,
        completion_ms, poll_count, exercise_gpu_backdrop_mask ? 1u : 0u,
        async_result.effect_count);
  }
  if (exercise_async_resize_stale_sequence) {
    status = blink_standalone_renderer_set_element_style(
        renderer, "box",
        "position:absolute;left:16px;top:12px;width:80px;height:32px;"
        "background:#00a050");
    if (status != BLINK_STANDALONE_STATUS_OK) {
      std::fprintf(stderr,
                   "%s: async stale style mutation failed status=%d error=%s\n",
                   label, status, blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    blink_standalone_update_result_t stale_update = {};
    status = blink_standalone_renderer_update(renderer, 3.000, &stale_update);
    if (status != BLINK_STANDALONE_STATUS_OK ||
        stale_update.needs_output == 0) {
      std::fprintf(stderr,
                   "%s: async stale update failed status=%d needs_output=%u "
                   "error=%s\n",
                   label, status, stale_update.needs_output,
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    target.common.generation++;
    ...
    target.d3d12.current_state = target.d3d12.required_final_state;
#endif
    blink_standalone_gpu_async_render_request_t stale_request = {};
    stale_request.backend = backend;
    stale_request.request_generation = target.common.generation;
    stale_request.main_target = target;
    blink_standalone_gpu_async_render_result_t stale_result = {};
    status = blink_standalone_renderer_submit_gpu_frame_async(
        renderer, &stale_request, &stale_result);
    if ((status != BLINK_STANDALONE_STATUS_OK &&
         status != BLINK_STANDALONE_STATUS_PENDING) ||
        stale_result.request_id == 0) {
      std::fprintf(stderr,
                   "%s: async stale submit failed status=%d result_status=%u "
                   "state=%u request_id=%llu error=%s\n",
                   label, status, stale_result.status, stale_result.state,
                   static_cast<unsigned long long>(stale_result.request_id),
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    const uint64_t stale_request_id = stale_result.request_id;
    const int resized_logical_width =
        std::max(1u, active_logical_width / 2u);
    const int resized_logical_height =
        std::max(1u, active_logical_height / 2u);
    status = blink_standalone_renderer_set_viewport(
        renderer, resized_logical_width, resized_logical_height,
        active_device_scale_factor);
    if (status != BLINK_STANDALONE_STATUS_OK) {
      std::fprintf(stderr,
                   "%s: async stale viewport failed status=%d error=%s\n",
                   label, status, blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    status = blink_standalone_renderer_poll_gpu_frame_async(
        renderer, stale_request_id, &stale_result);
    if (status != BLINK_STANDALONE_STATUS_OK ||
        stale_result.state != BLINK_STANDALONE_GPU_ASYNC_STATE_STALE ||
        stale_result.main_target_written != 0 ||
        stale_result.backdrop_mask_written != 0) {
      std::fprintf(stderr,
                   "%s: async stale poll did not reject old generation "
                   "status=%d result_status=%u state=%u main=%u mask=%u "
                   "request_id=%llu error=%s\n",
                   label, status, stale_result.status, stale_result.state,
                   stale_result.main_target_written,
                   stale_result.backdrop_mask_written,
                   static_cast<unsigned long long>(stale_result.request_id),
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    std::printf("%s: async stale request_id=%llu state=%u\n", label,
                static_cast<unsigned long long>(stale_request_id),
                stale_result.state);
  }
  auto resize_active_async_target = [&](uint32_t target_width,
                                        uint32_t target_height,
    ...
    active_logical_width = logical_width ? logical_width : target_width;
    active_logical_height = logical_height ? logical_height : target_height;
    active_device_scale_factor =
        device_scale_factor > 0.0f ? device_scale_factor : 1.0f;
    status = blink_standalone_renderer_set_viewport(
        renderer, static_cast<int>(active_logical_width),
        static_cast<int>(active_logical_height), active_device_scale_factor);
    if (status != BLINK_STANDALONE_STATUS_OK) {
      std::fprintf(stderr, "%s: async resize viewport failed status=%d "
                           "error=%s\n",
                   label, status, blink_standalone_renderer_last_error(renderer));
      return false;
    }
    ...
    target.common.device_scale_factor = active_device_scale_factor;
    target.common.generation++;
    mask_target.common = target.common;
    mask_target.common.generation++;
    if (backend == BLINK_STANDALONE_GPU_BACKEND_VULKAN) {
      if (!create_vulkan_target || !create_vulkan_target(active_width,
                                                         active_height)) {
        std::fprintf(stderr, "%s: async resize Vulkan target %ux%u failed\n",
...
#if BUILDFLAG(IS_WIN)
    if (backend == BLINK_STANDALONE_GPU_BACKEND_D3D12) {
      if (!create_d3d12_target || !create_d3d12_target(active_width,
                                                       active_height)) {
        std::fprintf(stderr, "%s: async resize D3D12 target %ux%u failed\n",
                     label, active_width, active_height);
        return false;
      }
    }
  ...
  struct AsyncDirtyTimingStats {
    int frames = 0;
    int accepted_requests = 0;
    int completed_requests = 0;
    int no_demand_frames = 0;
    int source_pending_retries = 0;
    int source_ready_count = 0;
    int poll_iterations = 0;
    double set_text_ms = 0.0;
    double update_ms = 0.0;
    double tick_ms = 0.0;
    double submit_ms = 0.0;
    double poll_call_ms = 0.0;
    double completion_wait_ms = 0.0;
    double total_dirty_ms = 0.0;
    double max_tick_ms = 0.0;
    double max_submit_ms = 0.0;
    double max_completion_wait_ms = 0.0;
  ...
  auto run_async_dirty_frame = [&](int iteration,
                                   bool mutate,
                                   double timestamp,
                                   AsyncDirtyTimingStats* stats,
                                   bool cooperative_source_tick,
                                   bool cancel_before_completion = false) {
    const auto frame_start = std::chrono::steady_clock::now();
    if (mutate) {
      const std::string text = "fps " + std::to_string(iteration);
      const auto set_text_start = std::chrono::steady_clock::now();
      status = blink_standalone_renderer_set_element_text(
          renderer, "counter", text.c_str());
      const auto set_text_end = std::chrono::steady_clock::now();
      stats->set_text_ms += std::chrono::duration<double, std::milli>(
                                set_text_end - set_text_start)
                                .count();
      if (status != BLINK_STANDALONE_STATUS_OK) {
        std::fprintf(stderr, "%s: async dirty set_text failed frame=%d "
                             "status=%d error=%s\n",
    ...
    if (use_dedicated_thread_gpu_frame_api) {
      ...
      if (exercise_gpu_backdrop_mask) {
        ...
        dedicated_request.render_request.flags |=
            BLINK_STANDALONE_GPU_ASYNC_BACKDROP_MASK_REQUIRED;
        dedicated_request.render_request.mask_encoding =
            BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE;
        dedicated_request.render_request.backdrop_mask_target = mask_target;
      }
      ...
      if (status != BLINK_STANDALONE_STATUS_OK ||
          ...
          (exercise_gpu_backdrop_mask &&
           (dedicated_result.render_result.backdrop_mask_written == 0 ||
            dedicated_result.render_result.backdrop_mask_generation !=
                mask_target.common.generation ||
            dedicated_result.render_result.effect_count == 0))) {
        std::fprintf(stderr,
                     "%s: dedicated gpu frame completion failed frame=%d "
                     "status=%d command_state=%u render_status=%u "
                     "render_state=%u command_id=%llu main=%u mask=%u "
                     "effects=%u generation=%llu mask_generation=%llu "
                     "size=%ux%u caller_polls=%d renderer_polls=%u "
                     "command_error=%s error=%s\n",
                     ...
                     dedicated_result.render_result.backdrop_mask_written,
                     dedicated_result.render_result.effect_count,
                     static_cast<unsigned long long>(
                         dedicated_result.render_result.main_target_generation),
                     static_cast<unsigned long long>(
                         dedicated_result.render_result.backdrop_mask_generation),
                     ...
                     dedicated_result.error_message
                         ? dedicated_result.error_message
                         : "",
                     blink_standalone_renderer_last_error(renderer));
        return false;
      ...
      if (exercise_gpu_backdrop_mask &&
          !verify_backdrop_mask_pixels("dedicated-backdrop-mask")) {
        return false;
      }
      if (exercise_gpu_backdrop_mask) {
        backdrop_effect_count = dedicated_result.render_result.effect_count;
      }
      ++stats->completed_requests;
    ...
    status = blink_standalone_renderer_update(renderer, timestamp,
                                              &dirty_update);
    ...
    ++stats->frames;
    if (status != BLINK_STANDALONE_STATUS_OK) {
      std::fprintf(stderr, "%s: async dirty update failed frame=%d status=%d "
                           "error=%s\n",
                   label, iteration, status,
                   blink_standalone_renderer_last_error(renderer));
      return false;
    }
    if (dirty_update.needs_output == 0) {
      ++stats->no_demand_frames;
      return true;
    }
    target.common.generation++;
    ...
    target.d3d12.current_state = target.d3d12.required_final_state;
#endif
    blink_standalone_gpu_async_render_request_t request = {};
    request.backend = backend;
    request.request_generation = target.common.generation;
    request.main_target = target;
    blink_standalone_gpu_async_render_result_t async_result = {};
    constexpr int kMaxSourceRetries = 8;
    int source_retries = 0;
    if (cooperative_source_tick) {
      blink_standalone_gpu_source_frame_tick_result_t tick_result = {};
      for (; source_retries <= kMaxSourceRetries; ++source_retries) {
        blink_standalone_gpu_source_frame_tick_request_t tick_request = {};
        tick_request.backend = backend;
        tick_request.request_generation = target.common.generation;
        tick_request.timeline_time_seconds =
            timestamp + static_cast<double>(source_retries) * 0.016;
        tick_request.logical_width = active_logical_width;
        tick_request.logical_height = active_logical_height;
        tick_request.physical_width = active_width;
        tick_request.physical_height = active_height;
        tick_request.device_scale_factor = active_device_scale_factor;
        tick_request.max_work_budget_ms = 4.0;
        const auto tick_start = std::chrono::steady_clock::now();
        status = blink_standalone_renderer_tick_gpu_source_frame_async(
            renderer, &tick_request, &tick_result);
        const auto tick_end = std::chrono::steady_clock::now();
        const double tick_sample =
            std::chrono::duration<double, std::milli>(tick_end - tick_start)
                .count();
        stats->tick_ms += tick_sample;
        stats->max_tick_ms = std::max(stats->max_tick_ms, tick_sample);
        stats->tick_samples.push_back(tick_sample);
        if (status == BLINK_STANDALONE_STATUS_OK &&
            tick_result.state ==
                BLINK_STANDALONE_GPU_SOURCE_FRAME_STATE_SOURCE_READY) {
          ++stats->source_ready_count;
        ...
        if (status != BLINK_STANDALONE_STATUS_PENDING ||
            tick_result.state !=
                BLINK_STANDALONE_GPU_SOURCE_FRAME_STATE_PENDING) {
          std::fprintf(stderr,
                       "%s: cooperative source tick failed frame=%d status=%d "
                       "result_status=%u state=%u generation=%llu error=%s\n",
                       label, iteration, status, tick_result.status,
                       tick_result.state,
                       static_cast<unsigned long long>(
                           tick_result.source_frame_generation),
                       blink_standalone_renderer_last_error(renderer));
          return false;
        }
        ++stats->source_pending_retries;
        blink_standalone_update_result_t pending_update = {};
        status = blink_standalone_renderer_update(
            renderer, timestamp + 0.001 + static_cast<double>(source_retries) *
                                        0.016,
            &pending_update);
        if (status != BLINK_STANDALONE_STATUS_OK) {
          break;
      ...
      if (source_retries > kMaxSourceRetries) {
        std::fprintf(stderr,
                     "%s: cooperative source tick exhausted retries frame=%d "
                     "error=%s\n",
                     label, iteration,
                     blink_standalone_renderer_last_error(renderer));
        return false;
      ...
      const auto submit_start = std::chrono::steady_clock::now();
      status = blink_standalone_renderer_submit_gpu_frame_async(
          renderer, &request, &async_result);
      const auto submit_end = std::chrono::steady_clock::now();
      const double submit_sample =
          std::chrono::duration<double, std::milli>(submit_end - submit_start)
              .count();
      stats->submit_ms += submit_sample;
      stats->max_submit_ms = std::max(stats->max_submit_ms, submit_sample);
      stats->submit_samples.push_back(submit_sample);
    } else {
      for (; source_retries <= kMaxSourceRetries; ++source_retries) {
        const auto submit_start = std::chrono::steady_clock::now();
        status = blink_standalone_renderer_submit_gpu_frame_async(
    ...
    if ((status != BLINK_STANDALONE_STATUS_OK &&
        ...
        async_result.request_id == 0) {
      std::fprintf(stderr,
                   "%s: async dirty submit failed frame=%d status=%d "
                   "result_status=%u state=%u request_id=%llu "
                   "source_retries=%d error=%s\n",
                   label, iteration, status, async_result.status,
                   async_result.state,
                   static_cast<unsigned long long>(async_result.request_id),
                   source_retries,
                   blink_standalone_renderer_last_error(renderer));
    ...
    ++stats->accepted_requests;
    const uint64_t request_id = async_result.request_id;
    const auto completion_start = std::chrono::steady_clock::now();
    int poll_count = 0;
    constexpr int kMaxPolls = 240;
    while (async_result.state == BLINK_STANDALONE_GPU_ASYNC_STATE_PENDING ||
           async_result.state == BLINK_STANDALONE_GPU_ASYNC_STATE_SUBMITTED) {
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
      const auto poll_call_start = std::chrono::steady_clock::now();
      status = blink_standalone_renderer_poll_gpu_frame_async(
          renderer, request_id, &async_result);
      const auto poll_call_end = std::chrono::steady_clock::now();
      stats->poll_call_ms += std::chrono::duration<double, std::milli>(
                                  poll_call_end - poll_call_start)
                                  .count();
    ...
    if (status != BLINK_STANDALONE_STATUS_OK ||
        ...
        async_result.physical_height != active_height) {
      std::fprintf(stderr,
                   "%s: async dirty completion failed frame=%d status=%d "
                   "result_status=%u state=%u request_id=%llu main=%u "
                   "generation=%llu size=%ux%u polls=%d error=%s\n",
                   label, iteration, status, async_result.status,
                   async_result.state,
                   static_cast<unsigned long long>(async_result.request_id),
                   async_result.main_target_written,
                   static_cast<unsigned long long>(
                       async_result.main_target_generation),
                   async_result.physical_width, async_result.physical_height,
                   poll_count, blink_standalone_renderer_last_error(renderer));
      return false;
    }
  ...
  auto print_async_dirty_stats = [&](const char* stats_label,
                                     const AsyncDirtyTimingStats& stats) {
    const int completed = std::max(1, stats.completed_requests);
    const int frames = std::max(1, stats.frames);
    const int submits = std::max(1, static_cast<int>(stats.submit_samples.size()));
    std::printf(
        "%s: %s frames=%d accepted=%d completed=%d no_demand=%d "
        "source_ready=%d source_pending_retries=%d poll_iterations=%d "
        "set_text_avg_ms=%.3f update_avg_ms=%.3f tick_avg_ms=%.3f "
        "tick_p95_ms=%.3f tick_max_ms=%.3f submit_avg_ms=%.3f "
        "submit_p95_ms=%.3f submit_max_ms=%.3f poll_call_avg_ms=%.3f "
        "completion_avg_ms=%.3f completion_p95_ms=%.3f "
        "completion_max_ms=%.3f total_dirty_avg_ms=%.3f\n",
        label, stats_label, stats.frames, stats.accepted_requests,
        stats.completed_requests, stats.no_demand_frames,
        stats.source_ready_count, stats.source_pending_retries,
        stats.poll_iterations,
        stats.set_text_ms / frames, stats.update_ms / frames,
        stats.tick_ms / std::max(1, static_cast<int>(stats.tick_samples.size())),
        percentile(stats.tick_samples, 0.95), stats.max_tick_ms,
        stats.submit_ms / submits, percentile(stats.submit_samples, 0.95),
        stats.max_submit_ms,
        stats.poll_call_ms / std::max(1, stats.poll_iterations),
        stats.completion_wait_ms / completed,
        percentile(stats.completion_samples, 0.95),
        stats.max_completion_wait_ms, stats.total_dirty_ms / completed);
  };
  if (dedicated_first_frame_no_mutation) {
    AsyncDirtyTimingStats stats;
    if (!run_async_dirty_frame(0, /*mutate=*/false, 2.000, &stats,
                               /*cooperative_source_tick=*/false,
                               dedicated_cancel_before_completion)) {
      return cleanup_and_fail();
    }
  ...
  if (dedicated_cancel_resize_backdrop_mask) {
    AsyncDirtyTimingStats stats;
    if (!run_async_dirty_frame(0, /*mutate=*/true, 3.000, &stats,
                               /*cooperative_source_tick=*/false,
                               /*cancel_before_completion=*/true)) {
      return cleanup_and_fail();
    }
    if (!resize_active_async_target(1280, 720, 1280, 720, 1.0f)) {
      return cleanup_and_fail();
    }
    if (!run_async_dirty_frame(1, /*mutate=*/true, 3.100, &stats,
                               /*cooperative_source_tick=*/false)) {
      return cleanup_and_fail();
    }
    if (!resize_active_async_target(2560, 1440, 1280, 720, 2.0f)) {
      return cleanup_and_fail();
    }
    if (!run_async_dirty_frame(2, /*mutate=*/true, 3.200, &stats,
                               /*cooperative_source_tick=*/false)) {
      return cleanup_and_fail();
    }
    print_async_dirty_stats("dedicated_cancel_resize_backdrop_mask", stats);
    if (!verify_target_pixels("dedicated-cancel-resize-backdrop",
                              expected_background, expected_box,
                              require_full_nontransparent, /*quiet=*/true)) {
      return cleanup_and_fail();
    }
  }
  if (async_dirty_timing_iterations > 0) {
    AsyncDirtyTimingStats stats;
    for (int i = 0; i < async_dirty_timing_iterations; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/true,
                                 10.0 + static_cast<double>(i) * 0.016,
                                 &stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_dirty_timing", stats);
    if (!verify_target_pixels("async-dirty-final", expected_background,
                              (async_dirty_timing_iterations % 2) == 0
                                  ? expected_box
                                  : expected_box,
                              require_full_nontransparent, /*quiet=*/true)) {
      return cleanup_and_fail();
  ...
  if (cooperative_async_dirty_timing_iterations > 0) {
    AsyncDirtyTimingStats stats;
    for (int i = 0; i < cooperative_async_dirty_timing_iterations; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/true,
                                 11.0 + static_cast<double>(i) * 0.016,
                                 &stats,
                                 /*cooperative_source_tick=*/true)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_source_tick_timing", stats);
    if (!verify_target_pixels("async-source-tick-final", expected_background,
                              expected_box, require_full_nontransparent,
                              /*quiet=*/true)) {
      return cleanup_and_fail();
    }
  }
  if (exercise_async_profile_churn_sequence) {
    AsyncDirtyTimingStats static_stats;
    for (int i = 0; i < 18; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/false,
                                 20.0 + static_cast<double>(i) * 0.016,
                                 &static_stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_churn_static", static_stats);

    AsyncDirtyTimingStats ten_hz_stats;
    for (int i = 0; i < 48; ++i) {
      if (!run_async_dirty_frame(i, (i % 6) == 0,
                                 21.0 + static_cast<double>(i) * 0.016,
                                 &ten_hz_stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_churn_10hz", ten_hz_stats);

    blink_standalone_gpu_async_render_result_t cancel_result = {};
    status = blink_standalone_renderer_set_element_text(renderer, "counter",
                                                        "cancel-before-resize");
    if (status != BLINK_STANDALONE_STATUS_OK) {
      return cleanup_and_fail();
    }
    blink_standalone_update_result_t cancel_update = {};
    status = blink_standalone_renderer_update(renderer, 22.0, &cancel_update);
    if (status != BLINK_STANDALONE_STATUS_OK) {
      return cleanup_and_fail();
    }
    target.common.generation++;
    ...
    target.d3d12.current_state = target.d3d12.required_final_state;
#endif
    blink_standalone_gpu_async_render_request_t cancel_request = {};
    cancel_request.backend = backend;
    cancel_request.request_generation = target.common.generation;
    cancel_request.main_target = target;
    status = blink_standalone_renderer_submit_gpu_frame_async(
        renderer, &cancel_request, &cancel_result);
    if ((status == BLINK_STANDALONE_STATUS_OK ||
         status == BLINK_STANDALONE_STATUS_PENDING) &&
        cancel_result.request_id != 0) {
      status = blink_standalone_renderer_cancel_gpu_frame_async(
          renderer, cancel_result.request_id, &cancel_result);
      if (status != BLINK_STANDALONE_STATUS_OK ||
          cancel_result.state != BLINK_STANDALONE_GPU_ASYNC_STATE_CANCELLED) {
        std::fprintf(stderr,
                     "%s: async churn cancel failed status=%d state=%u "
                     "request_id=%llu error=%s\n",
                     label, status, cancel_result.state,
                     static_cast<unsigned long long>(cancel_result.request_id),
                     blink_standalone_renderer_last_error(renderer));
        return cleanup_and_fail();
      }
      std::printf("%s: async_churn_cancel request_id=%llu state=%u\n", label,
                  static_cast<unsigned long long>(cancel_result.request_id),
                  cancel_result.state);
    }
    if (!resize_active_async_target(640, 360)) {
      return cleanup_and_fail();
    }
    AsyncDirtyTimingStats every_640_stats;
    for (int i = 0; i < 96; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/true,
                                 23.0 + static_cast<double>(i) * 0.016,
                                 &every_640_stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_churn_every_640x360", every_640_stats);

    if (!resize_active_async_target(320, 180)) {
      return cleanup_and_fail();
    }
    AsyncDirtyTimingStats every_320_stats;
    for (int i = 0; i < 72; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/true,
                                 25.0 + static_cast<double>(i) * 0.016,
                                 &every_320_stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_churn_every_320x180", every_320_stats);

    if (!resize_active_async_target(width, height)) {
      return cleanup_and_fail();
    }
    AsyncDirtyTimingStats final_stats;
    for (int i = 0; i < 18; ++i) {
      if (!run_async_dirty_frame(i, /*mutate=*/true,
                                 27.0 + static_cast<double>(i) * 0.016,
                                 &final_stats,
                                 /*cooperative_source_tick=*/false)) {
        return cleanup_and_fail();
      }
    }
    print_async_dirty_stats("async_churn_final_size", final_stats);
  }
  if (exercise_gpu_backdrop_mask && !dedicated_cancel_resize_backdrop_mask) {
    if (backend == BLINK_STANDALONE_GPU_BACKEND_VULKAN) {
      target.vulkan.current_layout = target.vulkan.required_final_layout;
      mask_target.vulkan.current_layout = VK_IMAGE_LAYOUT_UNDEFINED;
    }
    ...
    target.common.generation++;
    mask_target.common.generation++;
    blink_standalone_gpu_backdrop_render_request_t backdrop_request = {};
    backdrop_request.backend = backend;
    backdrop_request.flags = BLINK_STANDALONE_GPU_BACKDROP_MASK_REQUIRED;
    backdrop_request.mask_encoding =
        BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE;
    backdrop_request.main_target = target;
    backdrop_request.backdrop_mask_target = mask_target;
    blink_standalone_gpu_backdrop_render_result_t backdrop_result = {};
    status = blink_standalone_renderer_render_gpu_backdrop_frame(
        renderer, &backdrop_request, &backdrop_result);
    if (status != BLINK_STANDALONE_STATUS_OK ||
        backdrop_result.status != BLINK_STANDALONE_STATUS_OK ||
        backdrop_result.main_target_written == 0 ||
        backdrop_result.backdrop_mask_written == 0 ||
        backdrop_result.backend != backend ||
        backdrop_result.width != active_width ||
        backdrop_result.height != active_height ||
        backdrop_result.mask_encoding !=
            BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE ||
        backdrop_result.effect_count == 0 ||
        backdrop_result.frame_generation == 0 ||
        backdrop_result.main_target_generation != target.common.generation ||
        backdrop_result.backdrop_mask_generation !=
            mask_target.common.generation) {
      std::fprintf(stderr,
                   "%s: gpu backdrop public render failed status=%d "
                   "result_status=%u main=%u mask=%u backend=%u size=%ux%u "
                   "encoding=%u effects=%u generation=%llu main_gen=%llu "
                   "mask_gen=%llu error=%s\n",
                   label, status, backdrop_result.status,
                   backdrop_result.main_target_written,
                   backdrop_result.backdrop_mask_written,
                   backdrop_result.backend, backdrop_result.width,
                   backdrop_result.height, backdrop_result.mask_encoding,
                   backdrop_result.effect_count,
                   static_cast<unsigned long long>(
                       backdrop_result.frame_generation),
                   static_cast<unsigned long long>(
                       backdrop_result.main_target_generation),
                   static_cast<unsigned long long>(
                       backdrop_result.backdrop_mask_generation),
                   blink_standalone_renderer_last_error(renderer));
      return cleanup_and_fail();
    }
    backdrop_effect_count = backdrop_result.effect_count;
    if (blink_standalone_renderer_gpu_backdrop_effect_count(renderer) !=
        backdrop_effect_count) {
      std::fprintf(stderr,
                   "%s: gpu backdrop effect count mismatch result=%u "
                   "accessor=%zu\n",
                   label, backdrop_effect_count,
    ...
    bool saw_blur = false;
    bool saw_saturate = false;
    for (uint32_t i = 0; i < backdrop_effect_count; ++i) {
      blink_standalone_gpu_backdrop_effect_t effect = {};
      status = blink_standalone_renderer_get_gpu_backdrop_effect(renderer, i,
                                                                 &effect);
      if (status != BLINK_STANDALONE_STATUS_OK || effect.id == 0 ||
          effect.generation != backdrop_result.frame_generation ||
          effect.filter_op_count == 0) {
        std::fprintf(stderr,
                     "%s: invalid gpu backdrop effect index=%u status=%d "
                     "id=%u generation=%llu expected_generation=%llu "
                     "ops=%u\n",
                     label, i, status, effect.id,
                     static_cast<unsigned long long>(effect.generation),
                     static_cast<unsigned long long>(
                         backdrop_result.frame_generation),
                     effect.filter_op_count);
        return cleanup_and_fail();
      }
      for (uint32_t op = 0; op < effect.filter_op_count; ++op) {
        if (effect.filter_ops[op].type ==
                BLINK_STANDALONE_BACKDROP_FILTER_OP_BLUR &&
            effect.filter_ops[op].amount > 0.0f) {
          saw_blur = true;
        }
        if (effect.filter_ops[op].type ==
                BLINK_STANDALONE_BACKDROP_FILTER_OP_SATURATE &&
            effect.filter_ops[op].amount > 1.0f) {
          saw_saturate = true;
        }
      }
    }
    if (!saw_blur || !saw_saturate) {
      std::fprintf(stderr,
                   "%s: gpu backdrop effects missing expected operations "
                   "effects=%u saw_blur=%d saw_saturate=%d\n",
                   label, backdrop_effect_count, saw_blur ? 1 : 0,
                   saw_saturate ? 1 : 0);
      return cleanup_and_fail();
    }
    if (!verify_backdrop_mask_pixels("gpu-backdrop-mask")) {
      std::fprintf(stderr,
                   "%s: gpu backdrop mask had no encoded coverage "
                   "mask_pixels=%u distinct_ids=%u\n",
                   label, backdrop_mask_pixels, backdrop_mask_distinct_ids);
      return cleanup_and_fail();
    }
  }
  if (exercise_clean_skip_sequence) {
    constexpr int kCleanSkipIterations = 4;
    for (int i = 0; i < kCleanSkipIterations; ++i) {
      blink_standalone_update_result_t clean_update = {};
      status = blink_standalone_renderer_update(
          renderer, 1.000 + static_cast<double>(i) * 0.016, &clean_update);
      if (status != BLINK_STANDALONE_STATUS_OK ||
          clean_update.needs_output != 0) {
        std::fprintf(stderr,
                     "%s: clean skip update %d failed status=%d "
                     "needs_output=%u advanced=%u skipped=%u error=%s\n",
                     label, i, status, clean_update.needs_output,
                     clean_update.frame_advanced,
                     clean_update.frame_skipped_due_to_no_demand,
                     blink_standalone_renderer_last_error(renderer));
        return cleanup_and_fail();
      }
      target.common.flags |= BLINK_STANDALONE_GPU_TARGET_SKIP_IF_CLEAN;
      mask_target.common.flags |= BLINK_STANDALONE_GPU_TARGET_SKIP_IF_CLEAN;
      target.common.generation++;
      mask_target.common.generation++;
      if (exercise_gpu_backdrop_mask) {
        blink_standalone_gpu_backdrop_render_request_t clean_request = {};
        clean_request.backend = backend;
        clean_request.flags = BLINK_STANDALONE_GPU_BACKDROP_MASK_REQUIRED;
        clean_request.mask_encoding =
            BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE;
        clean_request.main_target = target;
        clean_request.backdrop_mask_target = mask_target;
        blink_standalone_gpu_backdrop_render_result_t clean_result = {};
        status = blink_standalone_renderer_render_gpu_backdrop_frame(
            renderer, &clean_request, &clean_result);
        if (status != BLINK_STANDALONE_STATUS_OK ||
            clean_result.status != BLINK_STANDALONE_STATUS_OK ||
            clean_result.main_target_written != 0 ||
            clean_result.backdrop_mask_written != 0 ||
            clean_result.needs_output != 0 ||
            clean_result.frame_skipped_due_to_no_demand == 0 ||
            clean_result.width != active_width ||
            clean_result.height != active_height) {
          std::fprintf(stderr,
                       "%s: clean backdrop render %d did not no-op "
                       "status=%d result_status=%u main=%u mask=%u "
                       "needs_output=%u skipped=%u size=%ux%u error=%s\n",
                       label, i, status, clean_result.status,
                       clean_result.main_target_written,
                       clean_result.backdrop_mask_written,
                       clean_result.needs_output,
                       clean_result.frame_skipped_due_to_no_demand,
                       clean_result.width, clean_result.height,
                       blink_standalone_renderer_last_error(renderer));
          return cleanup_and_fail();
        }
      } else {
        blink_standalone_gpu_render_result_t clean_result = {};
        status = blink_standalone_renderer_render_to_gpu_target(
            renderer, &target, &clean_result);
        if (status != BLINK_STANDALONE_STATUS_OK ||
            clean_result.status != BLINK_STANDALONE_STATUS_OK ||
            clean_result.target_written != 0 ||
            clean_result.needs_output != 0 ||
            clean_result.frame_skipped_due_to_no_demand == 0 ||
            clean_result.width != active_width ||
            clean_result.height != active_height) {
          std::fprintf(stderr,
                       "%s: clean gpu render %d did not no-op "
                       "status=%d result_status=%u written=%u "
                       "needs_output=%u skipped=%u size=%ux%u error=%s\n",
                       label, i, status, clean_result.status,
                       clean_result.target_written, clean_result.needs_output,
                       clean_result.frame_skipped_due_to_no_demand,
                       clean_result.width, clean_result.height,
                       blink_standalone_renderer_last_error(renderer));
          return cleanup_and_fail();
  ...
  if (exercise_resize_sequence) {
    ...
    } else if (exercise_dsf_resize_sequence) {
      resize_steps = {{1280, 720}, {2560, 1440}, {1280, 720}, {2560, 1440}};
    ...
    for (const ResizeStep& step : resize_steps) {
      ...
      if (exercise_dsf_resize_sequence) {
        active_logical_width = 1280;
        active_logical_height = 720;
        active_device_scale_factor =
            static_cast<float>(active_width) /
            static_cast<float>(active_logical_width);
      } else {
        active_logical_width = active_width;
        active_logical_height = active_height;
        active_device_scale_factor = 1.0f;
      }
      status = blink_standalone_renderer_set_viewport(
          renderer, static_cast<int>(active_logical_width),
          static_cast<int>(active_logical_height), active_device_scale_factor);
      if (status != BLINK_STANDALONE_STATUS_OK) {
        std::fprintf(stderr,
                     "%s: resize set_viewport logical=%ux%u dsf=%.3f "
                     "physical=%ux%u failed status=%d error=%s\n",
                     label, active_logical_width, active_logical_height,
                     active_device_scale_factor, active_width, active_height,
                     status,
                     blink_standalone_renderer_last_error(renderer));
        return cleanup_and_fail();
      }
  ...
  std::printf(
      ...
      "backdrop_mask_pixels=%u backdrop_mask_ids=%u backdrop_effects=%u\n",
      ...
      backdrop_mask_pixels, backdrop_mask_distinct_ids, backdrop_effect_count);
#if BUILDFLAG(IS_WIN)
  cleanup_d3d12_targets();
#endif
  blink_standalone_renderer_destroy(renderer);
...
int RunCApiVulkanExternalTargetAsyncSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
      ...
      /*exercise_async_render_sequence=*/true);
}

int RunCApiVulkanExternalTargetAsyncDirtyTimingSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_dirty_timing_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
      ...
      /*async_dirty_timing_iterations=*/60);
}

int RunCApiVulkanExternalTargetAsyncSourceTickSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_source_tick_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiVulkanDedicatedThreadBackdropMaskCancelResizeSmoke() {
  const char* extra_css =
      "#counter{position:absolute;left:24px;top:18px;"
      "font:32px sans-serif;color:white}"
      ...
      "#stripe{position:absolute;left:0;top:260px;width:720px;height:120px;"
      "background:linear-gradient(90deg,#144a80,#237a57,#d06329);}";
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_dedicated_thread_backdrop_mask_cancel_resize_smoke",
      /*require_external_target=*/true,
      ...
      extra_css,
      /*extra_body=*/
      "<div id='counter'>fps 0</div><div id='backdrop'></div>"
      "<div id='backdrop2'></div><div id='stripe'></div>",
      /*require_full_nontransparent=*/true,
      /*exercise_update_output_sequence=*/false,
      /*expect_invalid_vulkan_metadata=*/false,
      /*repeated_update_output_iterations=*/0,
      ...
      /*use_dedicated_thread_gpu_frame_api=*/true,
      /*skip_initial_gpu_target_render=*/false,
      /*dedicated_first_frame_no_mutation=*/false,
      /*dedicated_cancel_before_completion=*/false,
      /*dedicated_cancel_resize_backdrop_mask=*/true);
}

int RunCApiVulkanExternalTargetAsyncProfileChurnSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_profile_churn_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
      ...
      /*exercise_async_resize_stale_sequence=*/false,
      /*async_dirty_timing_iterations=*/0,
      /*exercise_async_profile_churn_sequence=*/true);
}

int RunCApiVulkanExternalTargetAsyncResizeStaleSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_resize_stale_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
      ...
      /*exercise_dsf_resize_sequence=*/false,
      /*exercise_async_render_sequence=*/false,
      /*exercise_async_resize_stale_sequence=*/true);
}
...
int RunCApiVulkanExternalTargetFilterBackdropSmoke() {
  const char* extra_css =
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_filter_backdrop_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunCApiVulkanExternalTargetBackdropMaskSmoke() {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(10px) saturate(170%);"
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_backdrop_mask_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunCApiVulkanExternalTargetAsyncBackdropMaskSmoke() {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(10px) saturate(170%);"
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_async_backdrop_mask_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunCApiExternalTargetBackdropMaskDsfResizeSmoke(uint32_t backend,
                                                    const char* label) {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      ...
      "backdrop-filter:blur(4px) brightness(1.2);"
      "-webkit-backdrop-filter:blur(4px) brightness(1.2);}"
      "#stripe{position:absolute;left:0;top:260px;width:1280px;height:120px;"
      "background:linear-gradient(90deg,#144a80,#237a57,#d06329);}";
  return RunCApiExternalGpuTargetSmoke(
      backend,
      label,
      ...
      /*exercise_clean_skip_sequence=*/false,
      /*exercise_backdrop_mask_resize_bounce=*/false,
      /*exercise_dsf_resize_sequence=*/true);
}

int RunCApiVulkanExternalTargetBackdropMaskDsfResizeSmoke() {
  return RunCApiExternalTargetBackdropMaskDsfResizeSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_backdrop_mask_dsf_resize_smoke");
}
...
int RunCApiVulkanExternalTargetCleanSkipSmoke() {
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_VULKAN,
      "c_api_vulkan_external_target_clean_skip_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
      ...
      /*expect_backdrop_filter_metadata=*/true,
      /*exercise_gpu_backdrop_mask=*/true,
      /*exercise_clean_skip_sequence=*/true);
}

int RunCApiGpuBackdropMaskRequiredInvalidSmoke() {
  blink_standalone_renderer_config_t config = {};
  config.width = 320;
  config.height = 200;
  config.device_scale_factor = 1.0f;
  ...
  if (status != BLINK_STANDALONE_STATUS_OK || !renderer) {
    std::fprintf(stderr,
                 "c_api_gpu_backdrop_mask_required_invalid_smoke: create "
                 "failed status=%d\n",
                 status);
    return 1;
  }
  const char* html =
      ...
      "#box{position:absolute;left:12px;top:12px;width:80px;height:32px;"
      "background:#d06329}"
      "#backdrop{position:absolute;left:80px;top:60px;width:160px;"
      "height:96px;background:rgba(255,255,255,.2);"
      "backdrop-filter:blur(8px) saturate(160%);"
      "-webkit-backdrop-filter:blur(8px) saturate(160%);}"
      "</style><div id='box'></div><div id='backdrop'></div>";
  status = blink_standalone_renderer_set_document_html(renderer, html, "", "");
  if (status != BLINK_STANDALONE_STATUS_OK) {
    std::fprintf(stderr,
                 "c_api_gpu_backdrop_mask_required_invalid_smoke: set html "
  ...
  request.flags = BLINK_STANDALONE_GPU_BACKDROP_MASK_REQUIRED;
  request.mask_encoding =
      BLINK_STANDALONE_GPU_BACKDROP_MASK_ENCODING_RGBA8_ID_COVERAGE;
  request.main_target.common.backend = BLINK_STANDALONE_GPU_BACKEND_VULKAN;
  request.main_target.common.flags =
      BLINK_STANDALONE_GPU_TARGET_INTERNAL_TEST_STANDIN;
  request.main_target.common.logical_width = 320;
  request.main_target.common.logical_height = 200;
  request.main_target.common.physical_width = 320;
  request.main_target.common.physical_height = 200;
  request.main_target.common.device_scale_factor = 1.0f;
  request.main_target.common.pixel_format =
      BLINK_STANDALONE_PIXEL_FORMAT_RGBA8;
  request.main_target.common.alpha_mode =
      BLINK_STANDALONE_ALPHA_MODE_PREMULTIPLIED;
  request.main_target.common.color_space = BLINK_STANDALONE_COLOR_SPACE_SRGB;
  request.main_target.common.generation = 1;
  request.backdrop_mask_target.common.backend =
      BLINK_STANDALONE_GPU_BACKEND_VULKAN;
  request.backdrop_mask_target.common.logical_width = 320;
  request.backdrop_mask_target.common.logical_height = 200;
  request.backdrop_mask_target.common.physical_width = 320;
  request.backdrop_mask_target.common.physical_height = 200;
  request.backdrop_mask_target.common.device_scale_factor = 1.0f;
  request.backdrop_mask_target.common.pixel_format =
      BLINK_STANDALONE_PIXEL_FORMAT_RGBA8;
  request.backdrop_mask_target.common.alpha_mode =
      BLINK_STANDALONE_ALPHA_MODE_PREMULTIPLIED;
  request.backdrop_mask_target.common.color_space =
      BLINK_STANDALONE_COLOR_SPACE_SRGB;
  request.backdrop_mask_target.common.generation = 2;
  blink_standalone_gpu_backdrop_render_result_t result = {};
  status = blink_standalone_renderer_render_gpu_backdrop_frame(renderer,
                                                               &request,
                                                               &result);
  const char* error = blink_standalone_renderer_last_error(renderer);
  const bool rejected =
      status == BLINK_STANDALONE_STATUS_INVALID_ARGUMENT &&
      result.main_target_written != 0 && result.backdrop_mask_written == 0 &&
      error && std::string(error).find("backdrop mask") != std::string::npos;
  if (!rejected) {
    std::fprintf(stderr,
                 "c_api_gpu_backdrop_mask_required_invalid_smoke: expected "
                 "invalid mask rejection status=%d result_status=%u main=%u "
                 "mask=%u error=%s\n",
                 status, result.status, result.main_target_written,
                 result.backdrop_mask_written, error ? error : "");
    blink_standalone_renderer_destroy(renderer);
    return 1;
  }
  std::printf(
      "c_api_gpu_backdrop_mask_required_invalid_smoke: ok status=%d "
      "main_written=%u mask_written=%u error=%s\n",
      status, result.main_target_written, result.backdrop_mask_written,
      error ? error : "");
  blink_standalone_renderer_destroy(renderer);
  return 0;
}
...
int RunCApiD3D12ExternalTargetAsyncSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_async_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiD3D12ExternalTargetAsyncDirtyTimingSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_async_dirty_timing_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiD3D12ExternalTargetAsyncSourceTickSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_async_source_tick_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiD3D12DedicatedThreadFirstFrameSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_dedicated_thread_first_frame_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiD3D12ExternalTargetAsyncResizeStaleSmoke() {
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_async_resize_stale_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff123456u,
      /*expected_box=*/0xffd06329u,
      /*background_css=*/"#123456",
...
int RunCApiD3D12ExternalTargetBackdropMaskSmoke() {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(10px) saturate(170%);"
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_backdrop_mask_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunCApiD3D12ExternalTargetAsyncBackdropMaskSmoke() {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(10px) saturate(170%);"
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_async_backdrop_mask_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunCApiD3D12ExternalTargetBackdropMaskDsfResizeSmoke() {
  return RunCApiExternalTargetBackdropMaskDsfResizeSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_backdrop_mask_dsf_resize_smoke");
}

int RunCApiD3D12ExternalTargetCleanSkipSmoke() {
  const char* extra_css =
      "#backdrop{position:absolute;left:80px;top:64px;width:240px;"
      "height:140px;border-radius:24px;background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(12px) saturate(170%);"
  ...
  return RunCApiExternalGpuTargetSmoke(
      BLINK_STANDALONE_GPU_BACKEND_D3D12,
      "c_api_d3d12_external_target_clean_skip_smoke",
      /*require_external_target=*/true,
      /*expected_background=*/0xff144a80u,
      /*expected_box=*/0xff237a57u,
      /*background_css=*/"#144a80",
...
int RunGpuVulkanBackdropMaskPrototypeSmoke() {
  html_css_renderer::CompositorRuntimeCreateInfo create_info;
  create_info.renderer.viewport = {256.0f, 160.0f};
  create_info.renderer.device_scale_factor = 1.0f;
  create_info.renderer.html =
      "<!doctype html><style>"
      "html,body{margin:0;width:100%;height:100%;background:#123456;}"
      "#box{position:absolute;left:16px;top:12px;width:80px;height:32px;"
      "background:#d06329;}"
      ".panel{position:absolute;width:72px;height:52px;"
      "background:rgba(255,255,255,.20);"
      "backdrop-filter:blur(8px) saturate(160%);"
      "-webkit-backdrop-filter:blur(8px) saturate(160%);}"
      "#a{left:24px;top:72px;border-radius:12px;}"
      "#b{left:132px;top:84px;backdrop-filter:brightness(1.35);"
      "-webkit-backdrop-filter:brightness(1.35);}"
      "</style><div id='box'></div><div id='a' class='panel'></div>"
  ...
  if (!runtime || !runtime->Initialize(&diagnostics)) {
    std::fprintf(
        stderr,
        "gpu_vulkan_backdrop_mask_prototype_smoke: failed runtime "
        "initialization\n");
    for (const std::string& diagnostic : diagnostics) {
      std::fprintf(stderr, "diagnostic: %s\n", diagnostic.c_str());
    }
  ...
  if (!frame.gpu_frame.shared_image_available ||
      ...
      !frame.gpu_frame.shared_context_state_is_vulkan ||
      !frame.viz_display_created || !frame.skia_renderer_gpu_path_reached ||
      frame.gpu_frame.is_software || frame.backdrop_filter_regions.size() < 2) {
    std::fprintf(
        stderr,
        "gpu_vulkan_backdrop_mask_prototype_smoke: failed setup "
        "shared_image=%d vk_context_provider=%d is_vulkan=%d "
        "viz_display=%d skia_gpu=%d software=%d backdrop_regions=%zu "
        "failure=%s\n",
        frame.gpu_frame.shared_image_available ? 1 : 0,
        frame.gpu_frame.vk_context_provider_available ? 1 : 0,
        frame.gpu_frame.shared_context_state_is_vulkan ? 1 : 0,
        ...
        frame.skia_renderer_gpu_path_reached ? 1 : 0,
        frame.gpu_frame.is_software ? 1 : 0,
        frame.backdrop_filter_regions.size(), frame.gpu_frame_failure.c_str());
    for (const std::string& diagnostic : frame.diagnostics) {
      std::fprintf(stderr, "diagnostic: %s\n", diagnostic.c_str());
    }
    return 1;
  ...
  const std::string main_target_line =
      runtime->RunBorrowedVkImageRenderCopySmokeForTesting();
  if (main_target_line.find("gpu_borrowed_vkimage_render_copy_smoke: ok") !=
      0) {
    std::fprintf(stderr, "%s\n", main_target_line.c_str());
    return 1;
  }
  const std::string mask_line =
      runtime->RunVulkanBackdropMaskPrototypeForTesting();
  if (mask_line.find("gpu_vulkan_backdrop_mask_prototype_smoke: ok") != 0) {
    std::fprintf(stderr, "%s\n", mask_line.c_str());
    return 1;
  }
  std::printf("%s main_target=\"%s\"\n", mask_line.c_str(),
              main_target_line.c_str());
  return 0;
}

int RunGpuD3D12BackdropMaskPrototypeSmoke() {
#if defined(_WIN32) && defined(BLINK_STANDALONE_HAVE_DAWN_D3D12)
  html_css_renderer::CompositorRuntimeCreateInfo create_info;
  create_info.renderer.viewport = {256.0f, 160.0f};
  create_info.renderer.device_scale_factor = 1.0f;
  create_info.renderer.html =
  ...
  if (!runtime || !runtime->Initialize(&diagnostics)) {
    std::fprintf(
        stderr,
        "gpu_d3d12_backdrop_mask_prototype_smoke: failed runtime "
        "initialization\n");
    for (const std::string& diagnostic : diagnostics) {
      std::fprintf(stderr, "diagnostic: %s\n", diagnostic.c_str());
    }
  ...
  html_css_renderer::CompositorFrameResult frame =
      runtime->AdvanceFrame(input);
  if (!frame.viz_display_created || !frame.skia_renderer_gpu_path_reached ||
      frame.gpu_frame.is_software || frame.backdrop_filter_regions.size() < 2) {
    std::fprintf(
        stderr,
        "gpu_d3d12_backdrop_mask_prototype_smoke: failed setup "
        "viz_display=%d skia_gpu=%d software=%d backdrop_regions=%zu "
        "failure=%s\n",
        frame.viz_display_created ? 1 : 0,
        frame.skia_renderer_gpu_path_reached ? 1 : 0,
  ...
  const std::string main_target_line =
      runtime->RunBorrowedD3D12RenderCopySmokeForTesting();
  if (main_target_line.find("gpu_borrowed_d3d12_render_copy_smoke: ok") !=
      0) {
    std::fprintf(stderr, "%s\n", main_target_line.c_str());
    return 1;
  }
  const std::string mask_line =
      runtime->RunD3D12BackdropMaskPrototypeForTesting();
  if (mask_line.find("gpu_d3d12_backdrop_mask_prototype_smoke: ok") != 0) {
    std::fprintf(stderr, "%s\n", mask_line.c_str());
    return 1;
  }
  std::printf("%s main_target=\"%s\"\n", mask_line.c_str(),
...
#else
  std::printf(
      "gpu_d3d12_backdrop_mask_prototype_smoke: blocked "
      "failure=D3D12/Dawn external targets are not enabled in this build\n");
  return 0;
#endif
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--c-api-smoke" ||
        ...
        arg ==
            "--c-api-vulkan-dedicated-thread-backdrop-mask-cancel-resize-smoke" ||
        ...
        arg == "--c-api-vulkan-external-target-async-backdrop-mask-smoke" ||
        arg ==
            "--c-api-vulkan-external-target-backdrop-mask-resize-bounce-smoke" ||
        arg ==
            "--c-api-vulkan-external-target-backdrop-mask-dsf-resize-smoke" ||
        ...
        arg ==
            "--c-api-d3d12-external-target-backdrop-mask-resize-smoke" ||
        arg == "--c-api-d3d12-external-target-async-backdrop-mask-smoke" ||
        arg ==
            "--c-api-d3d12-external-target-backdrop-mask-dsf-resize-smoke" ||
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (std::string(argv[i]) ==
        "--c-api-vulkan-dedicated-thread-backdrop-mask-cancel-resize-smoke") {
      return RunCApiVulkanDedicatedThreadBackdropMaskCancelResizeSmoke();
    }
    if (std::string(argv[i]) ==
        "--c-api-d3d12-dedicated-thread-gpu-frame-smoke") {
  ...
  bool gpu_vulkan_backdrop_mask_prototype_smoke = false;
  bool gpu_external_vulkan_runtime_target_smoke = false;
  bool gpu_borrowed_d3d12_render_copy_smoke = false;
  bool gpu_d3d12_backdrop_mask_prototype_smoke = false;
  ...
  bool c_api_vulkan_dedicated_thread_backdrop_mask_cancel_resize_smoke =
      false;
  ...
  bool c_api_vulkan_external_target_async_backdrop_mask_smoke = false;
  bool c_api_vulkan_external_target_backdrop_mask_resize_bounce_smoke = false;
  bool c_api_vulkan_external_target_backdrop_mask_dsf_resize_smoke = false;
  ...
  bool c_api_d3d12_external_target_async_backdrop_mask_smoke = false;
  bool c_api_d3d12_external_target_backdrop_mask_resize_smoke = false;
  bool c_api_d3d12_external_target_backdrop_mask_dsf_resize_smoke = false;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
    ...
    } else if (arg == "--gpu-vulkan-backdrop-mask-prototype-smoke") {
      gpu_vulkan_backdrop_mask_prototype_smoke = true;
    ...
    } else if (arg == "--gpu-d3d12-backdrop-mask-prototype-smoke") {
      gpu_d3d12_backdrop_mask_prototype_smoke = true;
    ...
    } else if (arg == "--c-api-vulkan-external-target-async-smoke") {
      c_api_vulkan_external_target_async_smoke = true;
    } else if (
        arg == "--c-api-vulkan-external-target-async-dirty-timing-smoke") {
      c_api_vulkan_external_target_async_dirty_timing_smoke = true;
    } else if (
        arg == "--c-api-vulkan-external-target-async-source-tick-smoke") {
      ...
      c_api_vulkan_dedicated_thread_gpu_frame_smoke = true;
        ...
        arg ==
        "--c-api-vulkan-dedicated-thread-backdrop-mask-cancel-resize-smoke") {
      c_api_vulkan_dedicated_thread_backdrop_mask_cancel_resize_smoke = true;
    } else if (
        arg == "--c-api-vulkan-external-target-async-profile-churn-smoke") {
      c_api_vulkan_external_target_async_profile_churn_smoke = true;
    } else if (
        arg == "--c-api-vulkan-external-target-async-resize-stale-smoke") {
      c_api_vulkan_external_target_async_resize_stale_smoke = true;
      ...
      c_api_vulkan_external_target_full_viewport_button_hit_metadata_smoke =
          true;
    } else if (arg == "--c-api-vulkan-external-target-filter-backdrop-smoke") {
      ...
      c_api_vulkan_external_target_backdrop_mask_smoke = true;
        ...
        arg == "--c-api-vulkan-external-target-async-backdrop-mask-smoke") {
      c_api_vulkan_external_target_async_backdrop_mask_smoke = true;
    } else if (
        arg ==
        "--c-api-vulkan-external-target-backdrop-mask-resize-bounce-smoke") {
      c_api_vulkan_external_target_backdrop_mask_resize_bounce_smoke = true;
    } else if (
        arg == "--c-api-vulkan-external-target-backdrop-mask-dsf-resize-smoke") {
      c_api_vulkan_external_target_backdrop_mask_dsf_resize_smoke = true;
    ...
    } else if (arg == "--c-api-d3d12-external-target-async-smoke") {
      c_api_d3d12_external_target_async_smoke = true;
    } else if (
        arg == "--c-api-d3d12-external-target-async-dirty-timing-smoke") {
      c_api_d3d12_external_target_async_dirty_timing_smoke = true;
    } else if (
        arg == "--c-api-d3d12-external-target-async-source-tick-smoke") {
      ...
      c_api_d3d12_external_target_backdrop_mask_smoke = true;
        ...
        arg == "--c-api-d3d12-external-target-async-backdrop-mask-smoke") {
      c_api_d3d12_external_target_async_backdrop_mask_smoke = true;
    } else if (
        arg ==
        "--c-api-d3d12-external-target-backdrop-mask-resize-smoke") {
      c_api_d3d12_external_target_backdrop_mask_resize_smoke = true;
    } else if (
        arg == "--c-api-d3d12-external-target-backdrop-mask-dsf-resize-smoke") {
      c_api_d3d12_external_target_backdrop_mask_dsf_resize_smoke = true;
  ...
  if (gpu_vulkan_backdrop_mask_prototype_smoke) {
    return RunGpuVulkanBackdropMaskPrototypeSmoke();
  }
  ...
  if (gpu_d3d12_backdrop_mask_prototype_smoke) {
    return RunGpuD3D12BackdropMaskPrototypeSmoke();
  }
  ...
  if (c_api_vulkan_external_target_async_smoke) {
    return RunCApiVulkanExternalTargetAsyncSmoke();
  }

  if (c_api_vulkan_external_target_async_dirty_timing_smoke) {
    return RunCApiVulkanExternalTargetAsyncDirtyTimingSmoke();
  }

  if (c_api_vulkan_external_target_async_source_tick_smoke) {
    return RunCApiVulkanExternalTargetAsyncSourceTickSmoke();
  }
  ...
  if (c_api_vulkan_dedicated_thread_backdrop_mask_cancel_resize_smoke) {
    return RunCApiVulkanDedicatedThreadBackdropMaskCancelResizeSmoke();
  }
  ...
  if (c_api_vulkan_external_target_async_resize_stale_smoke) {
    return RunCApiVulkanExternalTargetAsyncResizeStaleSmoke();
  }
  ...
  if (c_api_vulkan_external_target_filter_backdrop_smoke) {
    return RunCApiVulkanExternalTargetFilterBackdropSmoke();
  }

  if (c_api_vulkan_external_target_backdrop_mask_smoke) {
    return RunCApiVulkanExternalTargetBackdropMaskSmoke();
  }

  if (c_api_vulkan_external_target_async_backdrop_mask_smoke) {
    return RunCApiVulkanExternalTargetAsyncBackdropMaskSmoke();
  }
  ...
  if (c_api_vulkan_external_target_backdrop_mask_dsf_resize_smoke) {
    return RunCApiVulkanExternalTargetBackdropMaskDsfResizeSmoke();
  }
  ...
  if (c_api_d3d12_external_target_async_smoke) {
    return RunCApiD3D12ExternalTargetAsyncSmoke();
  }

  if (c_api_d3d12_external_target_async_dirty_timing_smoke) {
    return RunCApiD3D12ExternalTargetAsyncDirtyTimingSmoke();
  }
  ...
  if (c_api_d3d12_external_target_async_resize_stale_smoke) {
    return RunCApiD3D12ExternalTargetAsyncResizeStaleSmoke();
  }
  ...
  if (c_api_d3d12_external_target_async_backdrop_mask_smoke) {
    return RunCApiD3D12ExternalTargetAsyncBackdropMaskSmoke();
  }
  ...
  if (c_api_d3d12_external_target_backdrop_mask_dsf_resize_smoke) {
    return RunCApiD3D12ExternalTargetBackdropMaskDsfResizeSmoke();
  }

  if (c_api_d3d12_external_target_clean_skip_smoke) {
    return RunCApiD3D12ExternalTargetCleanSkipSmoke();
  }
