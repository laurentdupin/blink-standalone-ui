...
gles2::ProgramCache* GpuChannelManager::program_cache() {
  ...
  // The standalone renderer does not wire Chromium's GPU disk/program cache
  // service yet. Keep shader program caching disabled rather than pulling the
  // disk-cache proto/runtime into the first Viz/Vulkan checkpoint.
  return nullptr;
#else
