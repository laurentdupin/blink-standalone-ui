...
void TraceStandaloneServiceUtilsStage(const char* stage) {
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!std::getenv("HTML_CSS_RENDERER_TRACE_GPU_INIT"))
    return;
  fprintf(stderr, "service_utils.stage=%s\n", stage ? stage : "");
  fflush(stderr);
#endif
}

void TraceStandaloneServiceUtilsPointer(const char* label, const void* ptr) {
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!std::getenv("HTML_CSS_RENDERER_TRACE_GPU_INIT"))
    return;
  fprintf(stderr, "service_utils.ptr=%s %p\n", label ? label : "", ptr);
  fflush(stderr);
#endif
}
...
bool MSAAIsSlow(const GpuDriverBugWorkarounds& workarounds) {
  TraceStandaloneServiceUtilsStage("MSAAIsSlow begin");
  TraceStandaloneServiceUtilsPointer("workarounds", &workarounds);
  ...
  TraceStandaloneServiceUtilsStage("MSAAIsSlow before field read");
  bool affected_by_experiment =
      workarounds.msaa_is_slow && !workarounds.msaa_is_slow_2;
  TraceStandaloneServiceUtilsStage("MSAAIsSlow after field read");

  if (affected_by_experiment) {
    TraceStandaloneServiceUtilsStage("MSAAIsSlow before FeatureList query");
    const bool enabled =
        base::FeatureList::IsEnabled(features::kEnableMSAAOnNewIntelGPUs);
    TraceStandaloneServiceUtilsStage("MSAAIsSlow after FeatureList query");
    return !enabled;
  }
  TraceStandaloneServiceUtilsStage("MSAAIsSlow returning field");
  return workarounds.msaa_is_slow;
}
