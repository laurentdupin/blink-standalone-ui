...
void GpuPersistentCache::InitializeCache(
    ...
    scoped_refptr<RefCountedGpuProcessShmCount> use_shader_cache_shm_count) {
#if defined(HTML_CSS_RENDERER_STANDALONE)
  // The standalone SDL/Vulkan checkpoint keeps shader/program cache entries in
  // memory. Disk persistent cache would pull Chromium's SQL/SQLite runtime and
  // is not required for first compositor presentation.
  (void)pending_backend;
  (void)use_shader_cache_shm_count;
  return;
#else
...
size_t GpuPersistentCache::FindKey(std::string_view key) {
  ...
  LoadImpl(key, std::move(buffer_provider));
...
size_t GpuPersistentCache::LoadData(std::string_view key,
  ...
  auto buffer_provider = [dest, &discovered_size](size_t content_size) {
    ...
    if (dest.size() >= content_size) {
      return base::span<uint8_t>(dest.data(), content_size);
    }
    return base::span<uint8_t>();
  };
