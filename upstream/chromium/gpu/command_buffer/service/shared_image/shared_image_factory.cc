...
#if BUILDFLAG(USE_DAWN) && !defined(HTML_CSS_RENDERER_STANDALONE)
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
const char* SharedImageBackingTypeToString(SharedImageBackingType type) {
  switch (type) {
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    DVLOG(1) << "SharedImageFactory registered "
             << SharedImageBackingTypeToString(wrapped_sk_image_factory->GetBackingType());
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    DVLOG(1) << "SharedImageFactory registered "
             << SharedImageBackingTypeToString(
                    gl_texture_backing_factory->GetBackingType());
#endif
...
#endif  // BUILDFLAG(USE_DAWN) && !defined(HTML_CSS_RENDERER_STANDALONE)
}
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  shared_image_caps.shared_image_swap_chain = false;
#else
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
      DVLOG(1) << "SharedImageFactory skipped "
               << SharedImageBackingTypeToString(factory->GetBackingType())
               << " for stream=" << static_cast<int>(*stream)
               << " usage=" << CreateLabelForSharedImageUsage(usage)
               << " format=" << format.ToString()
               << " size=" << size.ToString();
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
      DVLOG(1) << "SharedImageFactory selected "
               << SharedImageBackingTypeToString(factory->GetBackingType())
               << " for usage=" << CreateLabelForSharedImageUsage(usage)
               << " format=" << format.ToString()
               << " size=" << size.ToString();
#endif
      return factory.get();
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    DVLOG(1) << "SharedImageFactory rejected "
             << SharedImageBackingTypeToString(factory->GetBackingType())
             << " for usage=" << CreateLabelForSharedImageUsage(usage)
             << " format=" << format.ToString()
             << " size=" << size.ToString()
