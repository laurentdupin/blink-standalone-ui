...
void TraceStandaloneSharedContextStage(const char* stage) {
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!std::getenv("HTML_CSS_RENDERER_TRACE_GPU_INIT"))
    return;
  fprintf(stderr, "shared_context_state.stage=%s\n", stage ? stage : "");
  fflush(stderr);
#endif
}

void TraceStandaloneSharedContextPointer(const char* label, const void* ptr) {
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!std::getenv("HTML_CSS_RENDERER_TRACE_GPU_INIT"))
    return;
  fprintf(stderr, "shared_context_state.ptr=%s %p\n", label ? label : "", ptr);
  fflush(stderr);
#endif
}
...
bool SharedContextState::InitializeSkia(
  ...
  TraceStandaloneSharedContextStage("InitializeSkia begin");
  TraceStandaloneSharedContextPointer("InitializeSkia workarounds",
                                      &workarounds);
  ...
  if (gr_context_type_ == GrContextType::kGraphiteDawn) {
    TraceStandaloneSharedContextStage("InitializeSkia graphite dawn");
    return InitializeGraphite(gpu_preferences, workarounds,
                              use_shader_cache_shm_count);
  }

  TraceStandaloneSharedContextStage("InitializeSkia before ganesh");
...
bool SharedContextState::InitializeGanesh(
    ...
    scoped_refptr<GpuPersistentCache> persistent_cache,
                          GpuProcessShmCount* use_shader_cache_shm_count,
                          gl::ProgressReporter* progress_reporter) {
  TraceStandaloneSharedContextStage("InitializeGanesh begin");
  TraceStandaloneSharedContextPointer("InitializeGanesh entry workarounds",
                                      &workarounds);
  progress_reporter_ = progress_reporter;
  gr_shader_cache_ = gr_cache;
  persistent_cache_ = std::move(persistent_cache);
  use_shader_cache_shm_count_ = use_shader_cache_shm_count;
  TraceStandaloneSharedContextStage("InitializeGanesh after member setup");

  size_t max_resource_cache_bytes;
  size_t glyph_cache_max_texture_bytes;
  TraceStandaloneSharedContextStage("InitializeGanesh before cache limits");
  DetermineGrCacheLimitsFromAvailableMemory(&max_resource_cache_bytes,
                                            &glyph_cache_max_texture_bytes);
  TraceStandaloneSharedContextStage("InitializeGanesh after cache limits");
  TraceStandaloneSharedContextPointer("after cache limits workarounds",
                                      &workarounds);
  ...
  TraceStandaloneSharedContextStage("InitializeGanesh before default options");
  GrContextOptions options;
  GetDefaultGrContextOptions(&options);
  TraceStandaloneSharedContextStage("InitializeGanesh after default options");
  TraceStandaloneSharedContextPointer("after default options workarounds",
                                      &workarounds);

  TraceStandaloneSharedContextStage("InitializeGanesh before MSAAIsSlow");
  TraceStandaloneSharedContextPointer("workarounds", &workarounds);
  const bool msaa_is_slow = gles2::MSAAIsSlow(workarounds);
  TraceStandaloneSharedContextStage("InitializeGanesh after MSAAIsSlow");
  options.fAllowMSAAOnNewIntel = !msaa_is_slow;
  TraceStandaloneSharedContextStage("InitializeGanesh after AllowMSAAOnNewIntel");
  // Limit MSAA sample counts to 4 on Intel Android devices for performance.
  TraceStandaloneSharedContextStage("InitializeGanesh before msaa fields");
  if(workarounds.msaa_is_slow && !workarounds.msaa_is_slow_2)
    options.fInternalMultisampleCount = 4;
  TraceStandaloneSharedContextStage("InitializeGanesh after msaa fields");
  TraceStandaloneSharedContextStage("InitializeGanesh before reduce ops");
  options.fReduceOpsTaskSplitting = GrContextOptions::Enable::kNo;
  TraceStandaloneSharedContextStage("InitializeGanesh after reduce ops");
  TraceStandaloneSharedContextStage("InitializeGanesh before persistent cache option");
  if (persistent_cache_) {
    options.fPersistentCache = persistent_cache_.get();
  } else {
    options.fPersistentCache = gr_cache;
  }
  TraceStandaloneSharedContextStage("InitializeGanesh after persistent cache option");
  TraceStandaloneSharedContextStage("InitializeGanesh before shader handler option");
  options.fShaderErrorHandler = this;
  TraceStandaloneSharedContextStage("InitializeGanesh after shader handler option");
  TraceStandaloneSharedContextStage("InitializeGanesh before force max texture");
  if (gpu_preferences.force_max_texture_size)
    options.fMaxTextureSizeOverride = gpu_preferences.force_max_texture_size;
  TraceStandaloneSharedContextStage("InitializeGanesh after force max texture");
  TraceStandaloneSharedContextStage("InitializeGanesh after workaround options");

  if (gr_context_options_provider_) {
    TraceStandaloneSharedContextStage(
        "InitializeGanesh before custom options provider");
    gr_context_options_provider_->SetCustomGrContextOptions(options);
    TraceStandaloneSharedContextStage(
        "InitializeGanesh after custom options provider");
  }

  if (gr_context_type_ == GrContextType::kGL) {
    TraceStandaloneSharedContextStage("InitializeGanesh GL begin");
    DCHECK(context_->IsCurrent(nullptr));
    TraceStandaloneSharedContextStage("InitializeGanesh before GrGLInterface");
    sk_sp<GrGLInterface> gr_gl_interface(gl::init::CreateGrGLInterface(
        *context_->GetVersionInfo(), progress_reporter));
    TraceStandaloneSharedContextStage("InitializeGanesh after GrGLInterface");
    if (!gr_gl_interface) {
    ...
    TraceStandaloneSharedContextStage("InitializeGanesh before cache bind");
    BindGLContextToShaderCache(feature_info_, gr_cache);
    BindCacheToCurrentOpenGLContext(persistent_cache_.get());
    TraceStandaloneSharedContextStage("InitializeGanesh after cache bind");

    TraceStandaloneSharedContextStage("InitializeGanesh before ToIntSet");
    const auto workaround_ints = workarounds.ToIntSet();
    TraceStandaloneSharedContextStage("InitializeGanesh after ToIntSet");
    TraceStandaloneSharedContextStage("InitializeGanesh before driver workarounds");
    options.fDriverBugWorkarounds =
        GrDriverBugWorkarounds(workaround_ints);
    TraceStandaloneSharedContextStage("InitializeGanesh after driver workarounds");
    TraceStandaloneSharedContextStage("InitializeGanesh before avoid stencil");
    options.fAvoidStencilBuffers = workarounds.avoid_stencil_buffers;
    TraceStandaloneSharedContextStage("InitializeGanesh after avoid stencil");
    TraceStandaloneSharedContextStage("InitializeGanesh before disk cache option");
    if (workarounds.disable_program_disk_cache) {
      options.fShaderCacheStrategy =
          GrContextOptions::ShaderCacheStrategy::kBackendSource;
    }
    TraceStandaloneSharedContextStage("InitializeGanesh after disk cache option");
    TraceStandaloneSharedContextStage("InitializeGanesh before external image option");
    options.fPreferExternalImagesOverES3 = true;
    TraceStandaloneSharedContextStage("InitializeGanesh after external image option");
    ...
    if (gl::GetGLImplementation() == gl::kGLImplementationStubGL) {
      ...
      TraceStandaloneSharedContextStage("InitializeGanesh before MakeMock");
      owned_gr_context_ = GrDirectContext::MakeMock(&mock_options, options);
      TraceStandaloneSharedContextStage("InitializeGanesh after MakeMock");
      DCHECK(owned_gr_context_);
    } else {
      TraceStandaloneSharedContextStage("InitializeGanesh before MakeGL");
      owned_gr_context_ =
          GrDirectContexts::MakeGL(std::move(gr_gl_interface), options);
      TraceStandaloneSharedContextStage("InitializeGanesh after MakeGL");
    }

    gr_context_ = owned_gr_context_.get();
  } else {
    TraceStandaloneSharedContextStage("InitializeGanesh Vulkan begin");
  ...
  if (!gr_context_) {
    TraceStandaloneSharedContextStage("InitializeGanesh no GrContext");
    LOG(ERROR) << "GrContext creation failed.";
    return false;
  }

  TraceStandaloneSharedContextStage("InitializeGanesh before cache limit");
  gr_context_->setResourceCacheLimit(max_resource_cache_bytes);
  TraceStandaloneSharedContextStage("InitializeGanesh before transfer cache");
  transfer_cache_ = std::make_unique<ServiceTransferCache>(
      gpu_preferences,
      base::BindRepeating(&SharedContextState::ScheduleSkiaCleanup,
                          base::Unretained(this)));
  TraceStandaloneSharedContextStage("InitializeGanesh before cache controller");
  gr_cache_controller_ = std::make_unique<raster::GrCacheController>(this);
  TraceStandaloneSharedContextStage("InitializeGanesh done");
  return true;
}
