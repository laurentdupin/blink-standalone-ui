...
#define GPU_DRIVER_BUG_WORKAROUNDS(GPU_OP)                                  \
  ...
  GPU_OP(CHECK_EGL_FENCE_BEFORE_WAIT,                                       \
         check_egl_fence_before_wait)                                       \
  GPU_OP(CHECK_YCBCR_STUDIO_G22_LEFT_P709_FOR_NV12_SUPPORT,                 \
  ...
  GPU_OP(DISABLE_ACCELERATED_AV1_ENCODE,                                    \
         disable_accelerated_av1_encode)                                    \
  GPU_OP(DISABLE_ACCELERATED_H264_DECODE,                                   \
         disable_accelerated_h264_decode)                                   \
  GPU_OP(DISABLE_ACCELERATED_H264_ENCODE,                                   \
         disable_accelerated_h264_encode)                                   \
  GPU_OP(DISABLE_ACCELERATED_HEVC_DECODE,                                   \
         disable_accelerated_hevc_decode)                                   \
  GPU_OP(DISABLE_ACCELERATED_VP8_DECODE,                                    \
         disable_accelerated_vp8_decode)                                    \
  GPU_OP(DISABLE_ACCELERATED_VP8_ENCODE,                                    \
         disable_accelerated_vp8_encode)                                    \
  GPU_OP(DISABLE_ACCELERATED_VP9_DECODE,                                    \
         disable_accelerated_vp9_decode)                                    \
  GPU_OP(DISABLE_ACCELERATED_VP9_ENCODE,                                    \
         disable_accelerated_vp9_encode)                                    \
  GPU_OP(DISABLE_ACCELERATED_VP9_PROFILE2_DECODE,                           \
         disable_accelerated_vp9_profile2_decode)                           \
  GPU_OP(DISABLE_ANDROID_ZERO_COPY_VIDEO_CAPTURE,                           \
         disable_android_zero_copy_video_capture)                           \
  ...
  GPU_OP(DISABLE_D3D11,                                                     \
         disable_d3d11)                                                     \
  GPU_OP(DISABLE_D3D11_UPDATE_SUBRESOURCE1,                                 \
  ...
  GPU_OP(DISABLE_D3D12_AV1_ENCODING,                                        \
         disable_d3d12_av1_encoding)                                        \
  GPU_OP(DISABLE_D3D12_AV1_MULTI_REF_ENCODING,                              \
         disable_d3d12_av1_multi_ref_encoding)                              \
  GPU_OP(DISABLE_D3D12_H264_ENCODER_NON_REFERENCE_FRAMES,                   \
         disable_d3d12_h264_encoder_non_reference_frames)                   \
  GPU_OP(DISABLE_D3D12_H264_ENCODING,                                       \
         disable_d3d12_h264_encoding)                                       \
  GPU_OP(DISABLE_D3D12_HEVC_ENCODING,                                       \
         disable_d3d12_hevc_encoding)                                       \
  GPU_OP(DISABLE_D3D12_VIDEO_ENCODER,                                       \
         disable_d3d12_video_encoder)                                       \
  GPU_OP(DISABLE_DCOMP_TEXTURE,                                             \
         disable_dcomp_texture)                                             \
  GPU_OP(DISABLE_DECODE_INTO_ARRAY_TEXTURE,                                 \
  ...
  GPU_OP(DISABLE_DIRECT_COMPOSITION_LETTERBOX_VIDEO_OPTIMIZATION,           \
         disable_direct_composition_letterbox_video_optimization)           \
  GPU_OP(DISABLE_DIRECT_COMPOSITION_SW_VIDEO_OVERLAYS,                      \
         disable_direct_composition_sw_video_overlays)                      \
  GPU_OP(DISABLE_DIRECT_COMPOSITION_VIDEO_OVERLAYS,                         \
  ...
  GPU_OP(DISABLE_DYNAMIC_VIDEO_ENCODE_FRAMERATE_UPDATE,                     \
         disable_dynamic_video_encode_framerate_update)                     \
  GPU_OP(DISABLE_EGL_EXT_IMAGE_DMA_BUF_IMPORT_MODIFIERS,                    \
         disable_egl_ext_image_dma_buf_import_modifiers)                    \
  GPU_OP(DISABLE_GL_RGB_FORMAT,                                             \
         disable_gl_rgb_format)                                             \
  GPU_OP(DISABLE_H264_ACCELERATOR_SW_BRC,                                   \
         disable_h264_accelerator_sw_brc)                                   \
  GPU_OP(DISABLE_HEVC_HMFT_CBR_ENCODING,                                    \
         disable_hevc_hmft_cbr_encoding)                                    \
  GPU_OP(DISABLE_IMAGEBITMAP_FROM_VIDEO_USING_GPU,                          \
  ...
  GPU_OP(DISABLE_NV12_DYNAMIC_TEXTURES,                                     \
         disable_nv12_dynamic_textures)                                     \
  GPU_OP(DISABLE_NV12_UPLOAD,                                               \
         disable_nv12_upload)                                               \
  ...
  GPU_OP(DISABLE_RENDERING_TO_RGB_EXTERNAL_TEXTURE,                         \
         disable_rendering_to_rgb_external_texture)                         \
  GPU_OP(DISABLE_RGB_TO_YUV_CONVERSION,                                     \
         disable_rgb_to_yuv_conversion)                                     \
  GPU_OP(DISABLE_SHARING_NV12_FROM_D3D11_TO_D3D12,                          \
         disable_sharing_nv12_from_d3d11_to_d3d12)                          \
  GPU_OP(DISABLE_SKIA_REDUCE_OPS_TASK_SPLITTING,                            \
  ...
  GPU_OP(DISABLE_VAAPI,                                                     \
         disable_vaapi)                                                     \
  GPU_OP(DISABLE_VIDEO_OVERLAY_IF_MOVING,                                   \
         disable_video_overlay_if_moving)                                   \
  GPU_OP(DISABLE_VP9_HMFT_TEMPORAL_ENCODING,                                \
         disable_vp9_hmft_temporal_encoding)                                \
  GPU_OP(DISABLE_VP_AUTO_HDR,                                               \
         disable_vp_auto_hdr)                                               \
  GPU_OP(DISABLE_VP_SCALING,                                                \
         disable_vp_scaling)                                                \
  GPU_OP(DISABLE_VP_SUPER_RESOLUTION,                                       \
  ...
  GPU_OP(FLUSH_BEFORE_CREATE_FENCE,                                         \
  ...
  GPU_OP(FORCE_DCOMP_TRIPLE_BUFFER_VIDEO_SWAP_CHAIN,                        \
         force_dcomp_triple_buffer_video_swap_chain)                        \
  GPU_OP(FORCE_ENABLE_COLOR_BUFFER_FLOAT,                                   \
  ...
  GPU_OP(FORCE_LOW_POWER_GPU,                                               \
         force_low_power_gpu)                                               \
  GPU_OP(FORCE_NV12_OVERLAY_SUPPORT,                                        \
         force_nv12_overlay_support)                                        \
  GPU_OP(FORCE_PHYSICAL_GPU_FOR_TESTING,                                    \
         force_physical_gpu_for_testing)                                    \
  GPU_OP(FORCE_RGB10A2_OVERLAY_SUPPORT,                                     \
  ...
  GPU_OP(LIMIT_D3D12_AV1_PROFILE_TO_MAIN,                                   \
  ...
  GPU_OP(USE_COPYTEXIMAGE2D_INSTEAD_OF_READPIXELS_ON_MULTISAMPLED_TEXTURES, \
         use_copyteximage2d_instead_of_readpixels_on_multisampled_textures) \
  GPU_OP(USE_EQAA_STORAGE_SAMPLES_2,                                        \
         use_eqaa_storage_samples_2)                                        \
  GPU_OP(USE_FIRST_VALID_REF_FOR_AV1_INVALID_REF,                           \
  ...
  GPU_OP(VALIDATE_MAX_PER_STAGE_UNIFORM_BLOCKS_AT_COMPILE_TIME,             \
  ...
  GPU_OP(WEBGL_OR_CAPS_MAX_TEXTURE_SIZE_LIMIT_8192,                         \
