...
void SetStandaloneDocumentBodyForStandaloneRenderer(Document* document,
                                                    HTMLElement* body) {
  g_standalone_body_document = document;
  g_standalone_body_element = body;
}

Document* GetStandaloneDocumentForStandaloneRenderer() {
  return g_standalone_body_document.Get();
}
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  g_standalone_style_engine = style_engine_.Get();
  g_standalone_style_engine_raw = style_engine_.Get();
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  bool needs_slot_assignment = false;
  bool needs_layout_tree_update = true;
#else
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  const bool standalone_needs_layout_tree_update = true;
#else
  const bool standalone_needs_layout_tree_update = needs_layout_tree_update;
#endif
  if (!standalone_needs_layout_tree_update) {
    advance_to_style_clean();
    return;
  }
...
#else
  bool should_record_stats = false;
#endif
#if !defined(HTML_CSS_RENDERER_STANDALONE)
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!layout_view_->Layer()) {
    auto mutable_layout_view = layout_view_->GetMutableForPainting();
    mutable_layout_view.EnsureId();
    mutable_layout_view.FirstFragment().SetLayer(
        MakeGarbageCollected<PaintLayer>(layout_view_));
    layout_view_->SetHasLayer(true);
  }
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (g_standalone_body_document.Get() == this &&
      g_standalone_body_element.Get()) {
    return g_standalone_body_element.Get();
  }

  static const AtomicString* html_local = new AtomicString("html");
  static const AtomicString* body_local = new AtomicString("body");
  if (!documentElement() ||
      documentElement()->localName() != *html_local) {
    return nullptr;
  }
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  exception_state.ThrowSecurityError("document.domain is disabled.");
  return;
#else
