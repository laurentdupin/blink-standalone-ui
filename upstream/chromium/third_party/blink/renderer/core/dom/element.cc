...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    // Scroll-target-group membership is a browser/navigation-facing service in
    // the standalone cut. The reduced HTMLElementFactory may represent anchors
    // as generic HTML elements until the full generated factory is imported.
    return;
#else
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  StringBuilder standalone_content;
  unsigned standalone_total_length = 0;
  bool standalone_found_text = false;
  for (Node* child = firstChild(); child; child = child->nextSibling()) {
    if (child->getNodeType() != Node::kTextNode) {
      continue;
    }
    const String text = child->textContent();
    if (text.length() >
        std::numeric_limits<unsigned>::max() - standalone_total_length) {
      return g_empty_string;
    }
    standalone_total_length += text.length();
    standalone_content.Append(text);
    standalone_found_text = true;
  }
  if (standalone_found_text) {
    return standalone_content.ReleaseString();
  }
#endif
