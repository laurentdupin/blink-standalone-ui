...
void LayoutObject::AddChild(LayoutObject* new_child,
  ...
  if (
    ...
    defined(HTML_CSS_RENDERER_STANDALONE)
      false &&
#endif
      RequiresAnonymousTableWrappers(new_child)) {
    ...
    defined(HTML_CSS_RENDERER_STANDALONE)
      true ||
#endif
      !LayoutTextCombine::IsSupportedMode(
...
Element* LayoutObject::ScrollParent(const Element* base) const {
  ...
  for (LayoutObject* ancestor = ContainingBlock(); ancestor;
    ...
    //    scrollingElement for the element窶冱 document if it is not
