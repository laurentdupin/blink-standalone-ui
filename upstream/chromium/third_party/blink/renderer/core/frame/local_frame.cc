...
LocalFrame* TraceStandaloneLocalFrameThis(const char* stage, LocalFrame* frame) {
  TraceStandaloneLocalFrameStage(stage);
  return frame;
}

std::unique_ptr<FrameScheduler> TraceStandaloneFrameScheduler(
    std::unique_ptr<FrameScheduler> scheduler) {
  TraceStandaloneLocalFrameStage("after CreateFrameScheduler");
  return scheduler;
}

template <typename T>
T* TraceStandaloneGC(const char* stage, T* value) {
  TraceStandaloneLocalFrameStage(stage);
  return value;
}

scoped_refptr<InspectorTaskRunner> TraceStandaloneInspectorTaskRunner(
    scoped_refptr<InspectorTaskRunner> runner) {
  TraceStandaloneLocalFrameStage("after InspectorTaskRunner");
  return runner;
}

InterfaceRegistry* TraceStandaloneInterfaceRegistry(InterfaceRegistry* registry) {
  TraceStandaloneLocalFrameStage("after InterfaceRegistry");
  return registry;
}
...
void LocalFrame::SetDOMWindow(LocalDOMWindow* dom_window) {
  TraceStandaloneLocalFrameStage("SetDOMWindow begin");
  DCHECK(dom_window);
  if (DomWindow()) {
    TraceStandaloneLocalFrameStage("SetDOMWindow before old reset");
    DomWindow()->Reset();
  ...
  TraceStandaloneLocalFrameStage("SetDOMWindow before ClearForNavigation");
  GetWindowProxyManager()->ClearForNavigation();
  TraceStandaloneLocalFrameStage("SetDOMWindow before assign");
  dom_window_ = dom_window;
  TraceStandaloneLocalFrameStage("SetDOMWindow before window Initialize");
  dom_window->Initialize();
  TraceStandaloneLocalFrameStage("SetDOMWindow before scheduler agent id");
  GetFrameScheduler()->SetAgentClusterId(GetAgentClusterId());
  TraceStandaloneLocalFrameStage("SetDOMWindow done");
}
...
LocalFrame::LocalFrame(
    ...
    : Frame(client,
      ...
      loader_(TraceStandaloneLocalFrameThis("before FrameLoader", this)),
      editor_(TraceStandaloneGC("after Editor",
                                MakeGarbageCollected<Editor>(*this))),
      selection_(TraceStandaloneGC(
          "after FrameSelection",
          MakeGarbageCollected<FrameSelection>(*this))),
      event_handler_(TraceStandaloneGC(
          "after EventHandler",
          MakeGarbageCollected<EventHandler>(*this))),
      console_(TraceStandaloneGC("after FrameConsole",
                                 MakeGarbageCollected<FrameConsole>(*this))),
      ...
      inspector_task_runner_(TraceStandaloneInspectorTaskRunner(
          InspectorTaskRunner::Create(
              GetTaskRunner(TaskType::kInternalInspector)))),
      interface_registry_(TraceStandaloneInterfaceRegistry(
          interface_registry ? interface_registry
                             : InterfaceRegistry::GetEmptyInterfaceRegistry())),
      v8_local_compile_hints_producer_(
          TraceStandaloneGC(
              "after V8LocalCompileHintsProducer",
              MakeGarbageCollected<
                  v8_compile_hints::V8LocalCompileHintsProducer>(this))),
  ...
  TraceStandaloneLocalFrameStage("LocalFrame ctor body");
#if !defined(HTML_CSS_RENDERER_STANDALONE)
  TRACE_EVENT("navigation", "LocalFrame");
#endif
  TraceStandaloneLocalFrameStage("after TRACE_EVENT");
  auto frame_tracking_result =
      GetLocalFramesMap().insert(FrameToken::Hasher()(GetFrameToken()), this);
  TraceStandaloneLocalFrameStage("after frame tracking insert");
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  return *static_cast<mojom::blink::LocalFrameHost*>(nullptr);
#else
