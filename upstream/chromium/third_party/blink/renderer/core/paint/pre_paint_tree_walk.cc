...
#if defined(HTML_CSS_RENDERER_STANDALONE)
void TraceStandalonePrePaintStage(const char* stage) {
}
#else
void TraceStandalonePrePaintStage(const char*) {}
#endif
...
void PrePaintTreeWalk::WalkTree(LocalFrameView& root_frame_view) {
  TraceStandalonePrePaintStage("WalkTree begin");
  if (root_frame_view.ShouldThrottleRendering()) {
    // Skip the throttled frame. Will update it when it becomes unthrottled.
    return;
  }
  TraceStandalonePrePaintStage("WalkTree after throttle");

  DCHECK_EQ(root_frame_view.GetFrame().GetDocument()->Lifecycle().GetState(),
            DocumentLifecycle::kInPrePaint);
  TraceStandalonePrePaintStage("WalkTree after lifecycle DCHECK");

  PrePaintTreeWalkContext context;
  TraceStandalonePrePaintStage("WalkTree after context");

#if DCHECK_IS_ON()
  bool needed_tree_builder_context_update =
      NeedsTreeBuilderContextUpdate(root_frame_view, context);
#endif
  TraceStandalonePrePaintStage("WalkTree after tree-builder context check");

  VisualViewport& visual_viewport =
      root_frame_view.GetPage()->GetVisualViewport();
  TraceStandalonePrePaintStage("WalkTree after visual viewport");
  if (visual_viewport.IsActiveViewport() &&
      root_frame_view.GetFrame().IsMainFrame()) {
    TraceStandalonePrePaintStage("WalkTree before visual viewport update");
    VisualViewportPaintPropertyTreeBuilder::Update(
        root_frame_view, visual_viewport, *context.tree_builder_context);
    TraceStandalonePrePaintStage("WalkTree after visual viewport update");
  }

  TraceStandalonePrePaintStage("WalkTree before Walk");
  Walk(root_frame_view, context);
  TraceStandalonePrePaintStage("WalkTree after Walk");
  paint_invalidator_.ProcessPendingDelayedPaintInvalidations();
  TraceStandalonePrePaintStage("WalkTree after delayed invalidations");

  bool updates_executed = root_frame_view.ExecuteAllPendingUpdates();
  TraceStandalonePrePaintStage("WalkTree after pending updates");
#if defined(HTML_CSS_RENDERER_STANDALONE)
  TraceStandalonePrePaintStage("WalkTree standalone skipping browser notifications");
  return;
#endif
  if (updates_executed) {
...
void PrePaintTreeWalk::Walk(LocalFrameView& frame_view,
  ...
  TraceStandalonePrePaintStage("Walk frame begin");
  bool needs_tree_builder_context_update =
      NeedsTreeBuilderContextUpdate(frame_view, parent_context);
  TraceStandalonePrePaintStage("Walk frame after needs-tree-builder");
  ...
  TraceStandalonePrePaintStage("Walk frame after throttle");

  PrePaintTreeWalkContext context(parent_context,
                                  needs_tree_builder_context_update);
  TraceStandalonePrePaintStage("Walk frame after context");

  // Block fragmentation doesn't cross frame boundaries.
  context.ResetFragmentation();
  TraceStandalonePrePaintStage("Walk frame after reset fragmentation");

  // Soft navigation tracking doesn't cross frame boundaries.
  context.ResetSoftNavigationContext();
  TraceStandalonePrePaintStage("Walk frame after reset soft navigation");
#if defined(HTML_CSS_RENDERER_STANDALONE)
  context.soft_navigation_paint_attribution_tracker = nullptr;
#else
  context.soft_navigation_paint_attribution_tracker =
      GetSoftNavigationPaintAttrubutionTrackerIfEnabled(frame_view);
#endif
  TraceStandalonePrePaintStage("Walk frame after soft navigation tracker");

  if (context.tree_builder_context) {
    TraceStandalonePrePaintStage("Walk frame before setup context");
    PaintPropertyTreeBuilder::SetupContextForFrame(
        frame_view, *context.tree_builder_context);
    TraceStandalonePrePaintStage("Walk frame after setup context");
  }

  if (LayoutView* view = frame_view.GetLayoutView()) {
    TraceStandalonePrePaintStage("Walk frame before layout view walk");
#if DCHECK_IS_ON()
    ...
    TraceStandalonePrePaintStage("Walk frame after layout view walk");
  ...
  TraceStandalonePrePaintStage("Walk frame before selection invalidation");
  frame_view.GetFrame().Selection().EnsureInvalidationOfPreviousLayoutBlock();
  TraceStandalonePrePaintStage("Walk frame after selection invalidation");

  TraceStandalonePrePaintStage("Walk frame before layout shift finished");
  frame_view.GetLayoutShiftTracker().NotifyPrePaintFinished();
  TraceStandalonePrePaintStage("Walk frame done");
}
...
void PrePaintTreeWalk::WalkInternal(const LayoutObject& object,
  ...
  TraceStandalonePrePaintStage("WalkInternal begin");
  PaintInvalidatorContext& paint_invalidator_context =
      context.paint_invalidator_context;
  TraceStandalonePrePaintStage("WalkInternal after invalidator context");

  if (pre_paint_info) {
    TraceStandalonePrePaintStage("WalkInternal has pre_paint_info");
    ...
    TraceStandalonePrePaintStage("WalkInternal fragmentless box");
  ...
  TraceStandalonePrePaintStage("WalkInternal after fragment setup");

  std::optional<PaintPropertyTreeBuilder> property_tree_builder;
  if (context.tree_builder_context) {
    TraceStandalonePrePaintStage("WalkInternal before builder emplace");
    property_tree_builder.emplace(object, pre_paint_info,
                                  *context.tree_builder_context);
    TraceStandalonePrePaintStage("WalkInternal after builder emplace");
    TraceStandalonePrePaintStage("WalkInternal before UpdateForSelf");
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
    property_tree_builder->UpdateForSelf();
    TraceStandalonePrePaintStage("WalkInternal after UpdateForSelf");
  }
  ...
  // handlers.
  TraceStandalonePrePaintStage("WalkInternal before touch action");
  UpdateEffectiveAllowedTouchAction(object, context);
  TraceStandalonePrePaintStage("WalkInternal after touch action");
  UpdateBlockingWheelEventHandler(object, context);
  TraceStandalonePrePaintStage("WalkInternal after wheel handler");
...
#endif
  TraceStandalonePrePaintStage("WalkInternal after hit-test invalidation");

  if (context.tree_builder_context) {
    TraceStandalonePrePaintStage("WalkInternal before UpdateForChildren");
    property_tree_builder->UpdateForChildren();
    TraceStandalonePrePaintStage("WalkInternal after UpdateForChildren");
    property_tree_builder->IssueInvalidationsAfterUpdate();
    TraceStandalonePrePaintStage("WalkInternal after issue invalidations");
    needs_invalidate_chrome_client_and_intersection_ |=
        property_tree_builder->PropertiesChanged();
    TraceStandalonePrePaintStage("WalkInternal after properties changed");
  }
  TraceStandalonePrePaintStage("WalkInternal done");
}
...
bool PrePaintTreeWalk::CollectMissableChildren(
  ...
  for (const PhysicalFragmentLink& child : parent.Children()) {
    ...
    if (child->IsOutOfFlowPositioned() &&
        (context.current_container || child->IsFixedPositioned())) {
...
const PhysicalBoxFragment* PrePaintTreeWalk::RebuildContextForMissedDescendant(
  ...
  if (context.tree_builder_context && update_tree_builder_context) {
    ...
    if (object.IsOutOfFlowPositioned()) {
      ...
      search_fragment = object.IsFixedPositioned()
  ...
  for (const LayoutObject* child = parent_object.SlowFirstChild(); child;
    ...
    if (inline_cursor) {
      ...
      if (child_box->IsOutOfFlowPositioned()) {
        search_fragment = child_box->IsFixedPositioned()
                              ? context.fixed_positioned_container
                              : context.absolute_positioned_container;
      }
    ...
    if (box_fragment) {
      const PhysicalBoxFragment* container_for_child =
...
void PrePaintTreeWalk::WalkChildren(
  ...
  TraceStandalonePrePaintStage("WalkChildren begin");
  const LayoutBox* box = DynamicTo<LayoutBox>(&object);
  TraceStandalonePrePaintStage("WalkChildren after box cast");
  if (box) {
    TraceStandalonePrePaintStage("WalkChildren has box");
    if (traversable_fragment) {
      TraceStandalonePrePaintStage("WalkChildren has traversable fragment");
...
#endif
      TraceStandalonePrePaintStage("WalkChildren before first fragment");
      ...
      TraceStandalonePrePaintStage("WalkChildren after first fragment");
    }
  }
  TraceStandalonePrePaintStage("WalkChildren after box branch");
  ...
  TraceStandalonePrePaintStage("WalkChildren before OOF context");
  UpdateContextForOOFContainer(object, context, traversable_fragment);
  TraceStandalonePrePaintStage("WalkChildren after OOF context");

  bool has_missable_children = false;
  const PhysicalBoxFragment* fragment = traversable_fragment;
  if (!fragment) {
    TraceStandalonePrePaintStage("WalkChildren no traversable fragment");
  ...
  TraceStandalonePrePaintStage("WalkChildren after fragment fallback");
  if (fragment) {
    ...
    if (!is_inside_fragment_child) {
      TraceStandalonePrePaintStage("WalkChildren before collect missable");
      has_missable_children = CollectMissableChildren(context, *fragment);
      TraceStandalonePrePaintStage("WalkChildren after collect missable");
    }
  }
  TraceStandalonePrePaintStage("WalkChildren before child dispatch");
  ...
  if (traversable_fragment &&
    ...
    TraceStandalonePrePaintStage("WalkChildren before fragmentation children");
    WalkFragmentationContextRootChildren(object, *traversable_fragment,
                                         context);
    TraceStandalonePrePaintStage("WalkChildren after fragmentation children");
  } else {
    TraceStandalonePrePaintStage("WalkChildren before layout children");
    WalkLayoutObjectChildren(object, traversable_fragment, context);
    TraceStandalonePrePaintStage("WalkChildren after layout children");
  }

  if (has_missable_children) {
    TraceStandalonePrePaintStage("WalkChildren before missed children");
    WalkMissedChildren(*fragment, !!traversable_fragment, context);
    TraceStandalonePrePaintStage("WalkChildren after missed children");
  }
  TraceStandalonePrePaintStage("WalkChildren done");
}

void PrePaintTreeWalk::Walk(const LayoutObject& object,
                            const PrePaintTreeWalkContext& parent_context,
                            PrePaintInfo* pre_paint_info) {
  TraceStandalonePrePaintStage("Walk object begin");
  ...
  TraceStandalonePrePaintStage("Walk object after missable erase");

  bool needs_tree_builder_context_update =
      NeedsTreeBuilderContextUpdate(object, parent_context);
  TraceStandalonePrePaintStage("Walk object after needs-tree-builder");

#if DCHECK_IS_ON()
  CheckTreeBuilderContextState(object, parent_context);
#endif
  TraceStandalonePrePaintStage("Walk object after context state check");
  ...
  TraceStandalonePrePaintStage("Walk object after early-out check");

  PrePaintTreeWalkContext context(parent_context,
                                  needs_tree_builder_context_update);
  TraceStandalonePrePaintStage("Walk object after context");

  WalkInternal(object, context, pre_paint_info);
  TraceStandalonePrePaintStage("Walk object after internal");

  bool child_walk_blocked = object.ChildPrePaintBlockedByDisplayLock();
  TraceStandalonePrePaintStage("Walk object after display-lock check");
  ...
  if (!child_walk_blocked) {
    ...
    TraceStandalonePrePaintStage("Walk object before children");
    WalkChildren(object, physical_fragment, context, is_inside_fragment_child);
    TraceStandalonePrePaintStage("Walk object after children");
