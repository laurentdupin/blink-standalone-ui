...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    const gfx::Rect cull_rect_for_log = paint_info.GetCullRect().Rect();
#endif
    return;
  }
...
void BoxFragmentPainter::PaintBoxDecorationBackground(
  ...
  if (paint_info.IsPaintingBackgroundInContentsSpace()) {
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
    if (!scrollable_area) {
      // Content overflow clips are generated by the Blink paint property tree,
      // but the standalone scrollable-area boundary can be absent for reduced
      // scroll/auto fixtures. Keep painting fail-soft instead of dereferencing
      // browser scrollbar/scroll-background chrome state.
    ...
    {
      background_client =
          &scrollable_area->GetScrollingBackgroundDisplayItemClient();
      visual_rect = scrollable_area->ScrollingBackgroundVisualRect(paint_offset);
    }
  } else {
