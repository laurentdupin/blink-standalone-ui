...
#if defined(HTML_CSS_RENDERER_STANDALONE)
void TraceStandalonePaintTreeBuilderStage(const char* stage) {
}
#else
void TraceStandalonePaintTreeBuilderStage(const char*) {}
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
  if (!scrollable_area) {
    TraceStandalonePaintTreeBuilderStage(
        "NeedsScrollAndScrollTranslation missing scrollable area");
    return false;
  }
#endif
...
static bool NeedsPaintOffsetTranslation(
  ...
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation begin");
  if (!object.IsBoxModelObject())
    return false;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after box model check");
  ...
  if (object.IsSVGChild())
    return false;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after svg child check");

  const auto& box_model = To<LayoutBoxModelObject>(object);
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after box cast");
  ...
  if (NeedsIsolationNodes(box_model)) {
    TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation isolation true");
    DCHECK(box_model.HasLayer());
    return true;
  }
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after isolation");

  if (box_model.HasTransform())
    return true;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after transform");
  if (NeedsScrollAndScrollTranslation(object, direct_compositing_reasons)) {
    return true;
  }
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after scroll");
  if (NeedsStickyTranslation(object))
    return true;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after sticky");
  if (NeedsAnchorPositionScrollTranslation(object)) {
    return true;
  }
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after anchor scroll");
  if (NeedsPaintOffsetTranslationForOverflowControls(box_model))
    return true;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after overflow controls");
  if (NeedsReplacedContentTransform(object))
    return true;
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after replaced content");

  // Reference filter and reflection (which creates a reference filter) requires
  // zero paint offset.
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation before filter/reflection");
  if (box_model.HasLayer() &&
      (object.StyleRef().Filter().HasReferenceFilter() ||
       object.HasReflection() || object.StyleRef().HasBackdropFilter())) {
    return true;
  }
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after filter/reflection");

  if (auto* box = DynamicTo<LayoutBox>(box_model)) {
    TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation before fixed-to-view");
    if (box->IsFixedToView(container_for_fixed_position))
      return true;
  }
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after fixed-to-view");
  ...
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation after compositing reason");
  if (has_paint_offset_compositing_reason) {
    ...
    if (RuntimeEnabledFeatures::PaintOffsetTranslationForCompositedEnabled()) {
      TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation paint-offset-for-composited");
  ...
  TraceStandalonePaintTreeBuilderStage("NeedsPaintOffsetTranslation done false");
  return false;
}
...
#else
      bool is_filter_disallowed = false;
#endif
...
void PaintPropertyTreeBuilder::InitPaintProperties() {
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties begin");
#if defined(HTML_CSS_RENDERER_STANDALONE)
  bool needs_paint_properties = false;
  if (ObjectTypeMightNeedPaintProperties()) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsPaintOffsetTranslation");
    ...
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsPaintOffsetTranslation");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsStickyTranslation");
      needs_paint_properties = NeedsStickyTranslation(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsStickyTranslation");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage(
          "InitPaintProperties before NeedsAnchorPositionScrollTranslation");
      needs_paint_properties = NeedsAnchorPositionScrollTranslation(object_);
    }
    TraceStandalonePaintTreeBuilderStage(
        "InitPaintProperties after NeedsAnchorPositionScrollTranslation");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsTranslate");
      needs_paint_properties =
          NeedsTranslate(object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsTranslate");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsRotate");
      needs_paint_properties =
          NeedsRotate(object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsRotate");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsScale");
      needs_paint_properties =
          NeedsScale(object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsScale");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsOffset");
      needs_paint_properties =
          NeedsOffset(object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsOffset");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsTransform");
      needs_paint_properties =
          NeedsTransform(object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsTransform");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage(
          "InitPaintProperties before NeedsEffectIgnoringClipPathAnd2DScale");
      needs_paint_properties = NeedsEffectIgnoringClipPathAnd2DScale(
          object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage(
        "InitPaintProperties after NeedsEffectIgnoringClipPathAnd2DScale");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsClipPathClipOrMask");
      needs_paint_properties = NeedsClipPathClipOrMask(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsClipPathClipOrMask");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsTransformForSVGChild");
      needs_paint_properties = NeedsTransformForSVGChild(
          object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsTransformForSVGChild");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsFilter");
      needs_paint_properties = NeedsFilter(object_, context_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsFilter");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsCssClip");
      needs_paint_properties = NeedsCssClip(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsCssClip");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsBackgroundClip");
      needs_paint_properties = NeedsBackgroundClip(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsBackgroundClip");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsInnerBorderRadiusClip");
      needs_paint_properties = NeedsInnerBorderRadiusClip(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsInnerBorderRadiusClip");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsOverflowClip");
      needs_paint_properties = NeedsOverflowClip(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsOverflowClip");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsPerspective");
      needs_paint_properties = NeedsPerspective(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsPerspective");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage("InitPaintProperties before NeedsReplacedContentTransform");
      needs_paint_properties = NeedsReplacedContentTransform(object_);
    }
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after NeedsReplacedContentTransform");
    if (!needs_paint_properties) {
      TraceStandalonePaintTreeBuilderStage(
          "InitPaintProperties before NeedsScrollAndScrollTranslation");
      needs_paint_properties = NeedsScrollAndScrollTranslation(
          object_, context_.direct_compositing_reasons);
    }
    TraceStandalonePaintTreeBuilderStage(
        "InitPaintProperties after NeedsScrollAndScrollTranslation");
  }
#else
  bool needs_paint_properties =
      ...
      (NeedsPaintOffsetTranslation(object_, context_.direct_compositing_reasons,
       ...
       NeedsScrollAndScrollTranslation(object_,
                                       context_.direct_compositing_reasons));
#endif
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties after needs check");
  ...
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties after fragment");
  if (const auto* properties = fragment.PaintProperties()) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties has old properties");
  ...
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties after old properties");

  if (needs_paint_properties) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties before ensure id");
    fragment.EnsureId();
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties before ensure properties");
    fragment.EnsurePaintProperties();
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties after ensure properties");
  } else if (auto* properties = fragment.PaintProperties()) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties clearing old properties");
  ...
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties after properties branch");

  if (object_.IsSVGHiddenContainer()) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties svg hidden");
  ...
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties after svg branch");

  if (object_.HasLayer()) {
    TraceStandalonePaintTreeBuilderStage("InitPaintProperties before layer svg hidden flag");
    To<LayoutBoxModelObject>(object_).Layer()->SetIsUnderSVGHiddenContainer(
        context_.has_svg_hidden_container_ancestor);
  }
  TraceStandalonePaintTreeBuilderStage("InitPaintProperties done");
}
...
void PaintPropertyTreeBuilder::UpdateForSelf() {
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf begin");
  ...
  if (const auto* box = DynamicTo<LayoutBox>(object_)) {
    ...
    if (auto* scrollable_area = box->GetScrollableArea()) {
      ...
      TraceStandalonePaintTreeBuilderStage("UpdateForSelf after scroll preference");
    }
  }
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after LayoutBox branch");

#if defined(HTML_CSS_RENDERER_STANDALONE)
  const bool is_low_end_device = false;
#else
  const bool is_low_end_device = Platform::Current()->IsLowEndDevice();
#endif
  ...
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after low-end check");
  ...
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after view-transition clip-path");

  context_.was_layout_shift_root =
      IsLayoutShiftRoot(object_, object_.FirstFragment());
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after layout-shift root");

  if (IsA<LayoutView>(object_)) {
    TraceStandalonePaintTreeBuilderStage("UpdateForSelf before global repaint reasons");
#if defined(HTML_CSS_RENDERER_STANDALONE)
    context_.requires_main_thread_for_background_attachment_fixed = false;
#else
    UpdateGlobalMainThreadRepaintReasonsForScroll();
#endif
    TraceStandalonePaintTreeBuilderStage("UpdateForSelf after global repaint reasons");
  }

  context_.old_scroll_offset = gfx::Vector2dF();
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf before old scroll properties");
  ...
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after old scroll properties");
  ...
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after clip-path fallback");

  TraceStandalonePaintTreeBuilderStage("UpdateForSelf before painting layer");
  UpdatePaintingLayer();
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after painting layer");
  UpdateFragmentData();
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after fragment data");
  InitPaintProperties();
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after init properties");

  FragmentPaintPropertyTreeBuilder builder(object_, pre_paint_info_, context_,
                                           GetFragmentData());
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf before fragment builder");
  builder.UpdateForSelf();
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after fragment builder");
  properties_changed_.Merge(builder.PropertiesChanged());
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf after merge");
  ...
  TraceStandalonePaintTreeBuilderStage("UpdateForSelf done");
}
