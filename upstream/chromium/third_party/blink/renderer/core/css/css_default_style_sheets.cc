...
StyleSheetContents* CSSDefaultStyleSheets::ParseUASheet(const String& str) {
  ...
  String parse_text = str;
  ParseSheetResult parse_result = sheet->ParseString(
      parse_text, /*allow_import_rules=*/true, CSSDeferPropertyParsing::kNo);
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
  StyleSheetContents* default_sheet = DefaultStyleSheet();
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
  const MediaQueryEvaluator& screen_eval = ScreenEval();
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
  default_html_style_->AddRulesFromSheet(default_sheet, screen_eval,
                                         /*mixins=*/{});
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
