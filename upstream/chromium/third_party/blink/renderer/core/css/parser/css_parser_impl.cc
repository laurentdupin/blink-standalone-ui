...
static inline void FilterProperties(
  ...
  // This is O(nﾂｲ), but n is typically small, and std::stable_partition
...
static ImmutableCSSPropertyValueSet* CreateCSSPropertyValueSet(
  ...
  if (document && mode == kHTMLQuirksMode &&
    ...
    // See if the properties contain 窶彡ursor: hand窶・without also containing
    // 窶彡ursor: pointer窶・ This is a reasonable approximation for whether
    // removing support for the former would actually matter. (Of course,
    // we don't check whether 窶彡ursor: hand窶・could lose in the cascade
...
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
    StyleRuleBase* result =
        ConsumeStyleRule(stream, nesting_type, parent_rule_for_nesting,
...
void CSSParserImpl::ConsumeBlockContents(
  ...
  while (true) {
    ...
    CSSParserTokenType peek_type = stream.UncheckedPeek().GetType();
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
    switch (peek_type) {
      case kWhitespaceToken:
...
bool CSSParserImpl::ConsumeDeclaration(CSSParserTokenStream& stream,
      ...
      if (unresolved_property == CSSPropertyID::kVariable) {
        ...
        if (observer_) {
          ...
          if (parsed_properties_.size() != properties_count) {
            ...
            stream.Restore(savepoint);
            // NOTE: This call is solely to update 窶彿mportant窶・
  ...
  bool consumed = parsed_properties_.size() != properties_count;
#if defined(HTML_CSS_RENDERER_STANDALONE)
#endif
  return consumed;
}
