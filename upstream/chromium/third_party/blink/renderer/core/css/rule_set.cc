...
// its buckets (e.g. for 窶・foo[baz]窶・ will return foo for class_name and
// baz for attr_name).
...
void RuleSet::AddRuleToLayerIntervals(const CascadeLayer* cascade_layer,
                                      unsigned position) {
  // nullptr in this context means 窶從o layer窶・ i.e., the implicit outer layer.
  if (!cascade_layer) {
...
void RuleSet::ApplyMixin(StyleRule* parent_rule,
  ...
  if (it != mixins.mixins.end()) {
    ...
    if (apply_mixin_rule->GetArguments().size() >
      ...
      // 窶廬f passed a <dashed-function>, the arguments passed to the
      // <dashed-function> are mapped to the mixin窶冱 arguments; if more
      // arguments are passed than the length of the mixin窶冱 argument list,
      // the @apply application does nothing. (Passing too few arguments is
      // fine; the missing arguments take their default values instead.)窶・      return;
    }
...
static wtf_size_t GetMinimumRulesetSizeForSubstringMatcher() {
  ...
  // all of the rules individually lies somewhere around 20窶・0 rules
