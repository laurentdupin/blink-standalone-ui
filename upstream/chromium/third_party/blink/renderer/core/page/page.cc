...
namespace {
void TraceStandalonePageStage(const char* stage) {
}
...
Page* Page::CreateOrdinary(
    ...
    const ColorProviderColorMaps* color_provider_colors) {
  TraceStandalonePageStage("before MakeGarbageCollected<Page>");
  Page* page = MakeGarbageCollected<Page>(
      base::PassKey<Page>(), chrome_client, agent_group_scheduler,
      browsing_context_group_token, color_provider_colors,
      /*is_ordinary=*/true);
  TraceStandalonePageStage("after MakeGarbageCollected<Page>");
  page->opener_ = opener;

  TraceStandalonePageStage("before OrdinaryPages insert");
  OrdinaryPages().insert(page);
  TraceStandalonePageStage("after OrdinaryPages insert");
...
Page::Page(base::PassKey<Page>,
  ...
  TraceStandalonePageStage("Page ctor body");
  TraceStandalonePageStage("before AllPages Contains");
  DCHECK(!AllPages().Contains(this));
  TraceStandalonePageStage("before AllPages insert");
  AllPages().insert(this);
  TraceStandalonePageStage("after AllPages insert");

  TraceStandalonePageStage("before CreatePageScheduler");
  page_scheduler_ = agent_group_scheduler_->CreatePageScheduler(this);
  TraceStandalonePageStage("after CreatePageScheduler");
  ...
  TraceStandalonePageStage("before default color provider maps");
  const ColorProviderColorMaps& standalone_color_provider_colors =
      color_provider_colors && !color_provider_colors->IsEmpty()
          ? *color_provider_colors
          : ColorProviderColorMaps::CreateDefault();
  TraceStandalonePageStage("after default color provider maps");
  UpdateColorProviders(standalone_color_provider_colors);
  TraceStandalonePageStage("after UpdateColorProviders");
}
