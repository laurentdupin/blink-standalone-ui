...
class StandaloneSkiaOutputSurfaceDependency final
 ...
 public:
  ...
  std::string RenderVulkanBackdropMaskToExternalTargetOnCurrentSequence(
    ...
    const std::string render_result = interop_->RenderBorrowedVulkanBackdropMask(
        output_size, css_viewport, ToStandaloneBackdropMaskRegions(regions));
    if (render_result.find(": ok") == std::string::npos) {
      return finish_with_failure(render_result);
    }
  ...
  std::string RenderD3D12BackdropMaskToExternalTargetForTesting(
    ...
    const std::string render_result = interop_->RenderBorrowedD3D12BackdropMask(
        output_size, css_viewport, ToStandaloneBackdropMaskRegions(regions));
    if (render_result.find(": ok") == std::string::npos) {
      return finish_with_failure(render_result);
    }
...
// Chromium Root owns all frame-sink, Display, support, begin-frame, and
// LocalSurfaceId state. This adapter owns only the standalone C API's borrowed
// external-target import and exact-surface CopyOutput completion boundary.
class StandaloneExternalTargetCopyAdapter final {
 public:
  StandaloneExternalTargetCopyAdapter(
      scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner,
  ...
  StandaloneExternalTargetCopyAdapter(
      const StandaloneExternalTargetCopyAdapter&) = delete;
  StandaloneExternalTargetCopyAdapter& operator=(
      const StandaloneExternalTargetCopyAdapter&) = delete;
  ~StandaloneExternalTargetCopyAdapter() = default;

  bool UsesChromiumRoot() const { return chromium_root_ != nullptr; }
  ...
  ExternalTargetDisplayPrepareResult PrepareDisplayForExternalTargetCopy(
    ...
    if (result.ready && draw_after_display_create) {
      DrawVizDisplayNow();
    }
    if (result.ready) {
      result.copy_result = MakeAsyncExternalGpuTargetCopyResult(
  ...
  struct PreparedExternalGpuTargetCopy {
    html_css_renderer::ExternalGpuTargetCopyResult copy_result;
    gfx::Size output_size;
    scoped_refptr<gpu::ClientSharedImage> blit_target;
    uint64_t request_id = 0;
  ...
  PreparedExternalGpuTargetCopy PrepareExternalVkImageCopyTarget(
      const html_css_renderer::ExternalVulkanImageTarget* vulkan_image,
      gfx::Size requested_output_size,
      const char* label) {
    PreparedExternalGpuTargetCopy prepared;
    const std::string label_text = label ? label : "gpu_external_vkimage_copy";
    auto display_ready =
        PrepareDisplayForExternalTargetCopy(requested_output_size, label);
    if (!display_ready.ready) {
      prepared.copy_result = display_ready.copy_result;
    ...
    if (!prepared.blit_target || prepared.blit_target->mailbox().IsZero()) {
      ...
      prepared.copy_result = MakeAsyncExternalGpuTargetCopyResult(
          ...
          label_text + ": failed failure=external Vulkan blit target "
                       "SharedImage is unavailable",
          prepared.output_size);
      prepared.blit_target = nullptr;
      return prepared;
    }
    prepared.ready = true;
    return prepared;
  ...
  gfx::Size ResolveExternalD3D12CopyOutputSize(ID3D12Resource* external_resource,
                                               int width,
                                               int height) const {
    gfx::Size output_size(width, height);
  ...
  PreparedExternalGpuTargetCopy PrepareExternalD3D12CopyTarget(
      ID3D12Resource* external_resource,
      void* shared_handle,
      gfx::Size requested_output_size,
      const char* label,
      bool map_shared_handle_open_failure_to_invalid_argument) {
    PreparedExternalGpuTargetCopy prepared;
    const std::string label_text = label ? label : "gpu_external_d3d12_copy";
    auto display_ready = PrepareDisplayForExternalTargetCopy(
        requested_output_size, label, /*draw_after_display_create=*/true);
    if (!display_ready.ready) {
      prepared.copy_result = display_ready.copy_result;
      prepared.output_size = display_ready.output_size;
      return prepared;
    ...
    if (!target_result.diagnostic.empty()) {
      const bool shared_handle_open_failure =
          ...
          std::string::npos;
      prepared.copy_result = MakeAsyncExternalGpuTargetCopyResult(
          map_shared_handle_open_failure_to_invalid_argument &&
  ...
  html_css_renderer::ExternalGpuTargetCopyResult RenderExternalVkImageToTarget(
    ...
    if (!vulkan_image) {
      return MakeAsyncExternalGpuTargetCopyResult(
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed,
          "gpu_external_vkimage_render_copy: failed failure=external "
    ...
    PreparedExternalGpuTargetCopy prepared = PrepareExternalVkImageCopyTarget(
        vulkan_image, output_size, "gpu_external_vkimage_render_copy");
    if (!prepared.ready) {
  ...
  BeginRenderExternalVkImageToTargetAsync(
      const html_css_renderer::ExternalVulkanImageTarget* vulkan_image) {
    if (!vulkan_image) {
      return MakeAsyncExternalGpuTargetCopyResult(
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed,
          "gpu_external_vkimage_render_copy_async: failed failure=external "
          "Vulkan image is null");
    }
    gfx::Size output_size(vulkan_image->width, vulkan_image->height);
  ...
  html_css_renderer::ExternalGpuTargetCopyResult RenderExternalD3D12ToTarget(
      void* d3d12_resource,
      void* shared_handle,
      int width,
      int height) {
    ...
    const gfx::Size output_size =
        ResolveExternalD3D12CopyOutputSize(external_resource, width, height);
    PreparedExternalGpuTargetCopy prepared = PrepareExternalD3D12CopyTarget(
        external_resource, shared_handle, output_size,
        "gpu_external_d3d12_render_copy",
        /*map_shared_handle_open_failure_to_invalid_argument=*/true);
    if (!prepared.ready) {
      return prepared.copy_result;
    }
  ...
  html_css_renderer::ExternalGpuTargetCopyResult
  BeginRenderExternalD3D12ToTargetAsync(void* d3d12_resource,
                                        void* shared_handle,
                                        int width,
                                        int height) {
    ID3D12Resource* external_resource =
    ...
    if (!external_resource && !shared_handle) {
      return MakeAsyncExternalGpuTargetCopyResult(
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed,
          "gpu_external_d3d12_render_copy_async: failed failure=external "
  ...
  html_css_renderer::ExternalGpuTargetCopyResult
  RenderBackdropMaskToExternalD3D12Target(
      void* d3d12_resource,
      void* shared_handle,
      int width,
      int height,
  ...
  void ReleaseExternalGpuTargetState() {
    ...
    if (async_external_gpu_target_copy_.request_id != 0 &&
      ...
      InvalidatePendingAsyncExternalGpuTargetCopy(
          ...
          async_external_gpu_target_copy_.label +
              ": stale after ReleaseExternalGpuTargetState",
          /*discard_prepared_target=*/false);
      return;
    }
 ...
 private:
  enum class AsyncExternalGpuTargetCopyBackend {
    kNone,
    kVulkan,
    kD3D12,
  };

  struct AsyncExternalGpuTargetCopyState {
    uint64_t request_id = 0;
    uint64_t generation = 0;
    html_css_renderer::ExternalGpuTargetCopyStatus status =
        html_css_renderer::ExternalGpuTargetCopyStatus::kNone;
  ...
  html_css_renderer::ExternalGpuTargetCopyResult
  MakeAsyncExternalGpuTargetCopyResult(
      html_css_renderer::ExternalGpuTargetCopyStatus status,
      std::string diagnostic,
      const gfx::Size& output_size = gfx::Size(),
      uint64_t request_id = 0) const {
    html_css_renderer::ExternalGpuTargetCopyResult result;
    result.status = status;
  ...
  html_css_renderer::ExternalGpuTargetCopyResult
  CurrentAsyncExternalGpuTargetCopyResult() const {
    if (async_external_gpu_target_copy_.request_id == 0) {
      return MakeAsyncExternalGpuTargetCopyResult(
          html_css_renderer::ExternalGpuTargetCopyStatus::kNone,
          "async external GPU target copy is not active");
  ...
  void BeginAsyncExternalGpuTargetCopyState(
    ...
    async_external_gpu_target_copy_.request_id =
        ++next_async_external_gpu_target_copy_request_id_;
    async_external_gpu_target_copy_.generation =
    ...
    std::ostringstream out;
    out << async_external_gpu_target_copy_.label
        << ": pending path=viz_blit_request"
        << " target=" << output_size.width() << "x" << output_size.height();
  ...
  void FinalizeAsyncExternalGpuTargetCopyIfReady() {
    ...
    if (copy_output_.Succeeded()) {
      ...
      std::ostringstream out;
      out << async_external_gpu_target_copy_.label
          << ": ok path=viz_blit_request"
          << " target=" << async_external_gpu_target_copy_.output_size.width()
          << "x" << async_external_gpu_target_copy_.output_size.height();
      async_external_gpu_target_copy_.diagnostic = out.str();
      return;
    }
  ...
  void InvalidatePendingAsyncExternalGpuTargetCopy(
    ...
    ++async_external_gpu_target_copy_generation_;
    async_external_gpu_target_copy_.status =
        wait_for_stale_callback
            ? html_css_renderer::ExternalGpuTargetCopyStatus::kPending
            : status;
    async_external_gpu_target_copy_.diagnostic = std::move(diagnostic);
    if (wait_for_stale_callback) {
  ...
  void ReleaseStaleAsyncExternalGpuTargetCopyIfNeeded(uint64_t generation) {
    ...
    async_external_gpu_target_copy_request_enqueued_ = false;
    stale_async_external_gpu_target_copy_waiting_for_callback_ = false;
  ...
  bool HasExternalDisplay() const {
    return chromium_root_->HasActiveRootSurface();
  }

  bool ExternalVulkanContextProviderAvailable() const {
    return chromium_root_->vulkan_context_provider_available();
  }

  bool ExternalSharedContextStateIsVulkan() const {
    return chromium_root_->shared_context_state_is_vulkan();
  }
  ...
  void InvalidateSupersededExternalTargetCopyForResize() {
    ...
    if (async_external_gpu_target_copy_.status ==
      ...
      InvalidatePendingAsyncExternalGpuTargetCopy(
          html_css_renderer::ExternalGpuTargetCopyStatus::kStale,
          async_external_gpu_target_copy_.label +
              ": stale after external target resize",
          /*discard_prepared_target=*/false);
    }
  }
  ...
  void RequestCopyOutput(const gfx::Size& output_size,
    ...
    const bool enqueued = copy_output_.Request(
        base::BindRepeating(
            &StandaloneExternalTargetCopyAdapter::SubmitCopyOutputRequest,
            base::Unretained(this)),
        output_size, wants_png, wants_raw, wants_gpu,
        std::move(blit_target),
        base::BindOnce(&StandaloneExternalTargetCopyAdapter::OnCopyOutput,
 ...
 private:
  ...
  bool async_external_gpu_target_copy_request_enqueued_ = false;
  bool stale_async_external_gpu_target_copy_waiting_for_callback_ = false;
  ...
  base::WeakPtrFactory<StandaloneExternalTargetCopyAdapter> weak_factory_{this};
};
...
class StandaloneCcLayerHost final
 ...
 public:
  ...
  html_css_renderer::ExternalGpuTargetCopyResult RenderExternalVkImageToTarget(
    ...
    if (!active_frame_sink_) {
      html_css_renderer::ExternalGpuTargetCopyResult result;
      result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_vkimage_render_copy: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
  ...
  BeginRenderExternalVkImageToTargetAsync(
    ...
    if (!active_frame_sink_) {
      ...
      result.status =
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_vkimage_render_copy_async: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
    return active_frame_sink_->BeginRenderExternalVkImageToTargetAsync(
        vulkan_image);
  }
  ...
  RenderBackdropMaskToExternalVkImage(
    ...
    if (!active_frame_sink_) {
      ...
      result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_vkimage_backdrop_mask: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
  ...
  html_css_renderer::ExternalGpuTargetCopyResult RenderExternalD3D12ToTarget(
    ...
    if (!active_frame_sink_) {
      ...
      result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_d3d12_render_copy: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
  ...
  BeginRenderExternalD3D12ToTargetAsync(void* d3d12_resource,
    ...
    if (!active_frame_sink_) {
      ...
      result.status =
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_d3d12_render_copy_async: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
    return active_frame_sink_->BeginRenderExternalD3D12ToTargetAsync(
        d3d12_resource, shared_handle, width, height);
  }
  ...
  RenderBackdropMaskToExternalD3D12Target(
    ...
    if (!active_frame_sink_) {
      ...
      result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "gpu_external_d3d12_backdrop_mask: failed failure=active "
          "LayerTreeFrameSink is not available";
      return result;
    }
  ...
  PollExternalGpuTargetAsyncCopy() {
    if (!active_frame_sink_) {
      ...
      result.status =
          html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
      result.diagnostic =
          "async external GPU target copy poll failed: active "
          "LayerTreeFrameSink is not available";
      return result;
    }
    return active_frame_sink_->PollExternalGpuTargetAsyncCopy();
  }
  html_css_renderer::ExternalGpuTargetCopyResult
  CancelExternalGpuTargetAsyncCopy() {
    if (!active_frame_sink_) {
      ...
      result.status =
          html_css_renderer::ExternalGpuTargetCopyStatus::kNone;
      result.diagnostic = "async external GPU target copy is not active";
      return result;
    }
    return active_frame_sink_->CancelExternalGpuTargetAsyncCopy();
  }
  ...
  bool CreateFrameSink() {
    ...
    external_target_adapter_ =
        std::make_unique<StandaloneExternalTargetCopyAdapter>(
            main_task_runner, initial_output_size,
            ...
            &frame_sink_failure_reason_, chromium_root_frame_sink_.get());
    CHECK(external_target_adapter_->UsesChromiumRoot());
...
StandaloneBlinkLiveFrameBridgeRenderExternalVkImageToTargetForStandaloneRenderer(
  ...
  auto result = cache.cc_layer_host->RenderExternalVkImageToTarget(vulkan_image);
  cache.copy_output_gpu_prepare_requested = false;
  return result;
}
...
StandaloneBlinkLiveFrameBridgeBeginRenderExternalVkImageToTargetAsyncForStandaloneRenderer(
  ...
  if (!cache.cc_layer_host) {
    ...
    result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
    result.diagnostic =
        "gpu_external_vkimage_render_copy_async: failed failure=cc layer host "
        "is not available";
    return result;
  }
  cache.copy_output_gpu_prepare_requested = false;
  return cache.cc_layer_host->BeginRenderExternalVkImageToTargetAsync(
      vulkan_image);
}
...
StandaloneBlinkLiveFrameBridgeRenderBackdropMaskToExternalVkImageForStandaloneRenderer(
  ...
  return cache.cc_layer_host->RenderBackdropMaskToExternalVkImage(
...
StandaloneBlinkLiveFrameBridgeRenderExternalD3D12ToTargetForStandaloneRenderer(
  ...
  auto result =
      cache.cc_layer_host->RenderExternalD3D12ToTarget(
          d3d12_resource, shared_handle, width, height);
  cache.copy_output_gpu_prepare_requested = false;
  return result;
}
...
StandaloneBlinkLiveFrameBridgeBeginRenderExternalD3D12ToTargetAsyncForStandaloneRenderer(
  ...
  if (!d3d12_resource && !shared_handle) {
    ...
    result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
    result.diagnostic =
        "gpu_external_d3d12_render_copy_async: failed failure=external D3D12 "
        "resource/shared handle is null";
    return result;
  }
  if (!cache.cc_layer_host) {
    ...
    result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kFailed;
    result.diagnostic =
        "gpu_external_d3d12_render_copy_async: failed failure=cc layer host is "
        "not available";
    return result;
  }
  cache.copy_output_gpu_prepare_requested = false;
  return cache.cc_layer_host->BeginRenderExternalD3D12ToTargetAsync(
      d3d12_resource, shared_handle, width, height);
}
...
StandaloneBlinkLiveFrameBridgeRenderBackdropMaskToExternalD3D12TargetForStandaloneRenderer(
  ...
  return cache.cc_layer_host->RenderBackdropMaskToExternalD3D12Target(
      d3d12_resource, shared_handle, width, height,
...
StandaloneBlinkLiveFrameBridgePollExternalGpuTargetAsyncCopyForStandaloneRenderer() {
  ...
  if (!cache.cc_layer_host) {
    html_css_renderer::ExternalGpuTargetCopyResult result;
    result.status = html_css_renderer::ExternalGpuTargetCopyStatus::kNone;
    result.diagnostic = "async external GPU target copy is not active";
    return result;
  }
  return cache.cc_layer_host->PollExternalGpuTargetAsyncCopy();
}
...
StandaloneBlinkLiveFrameBridgeCancelExternalGpuTargetAsyncCopyForStandaloneRenderer() {
  ...
  return cache.cc_layer_host->CancelExternalGpuTargetAsyncCopy();
}
