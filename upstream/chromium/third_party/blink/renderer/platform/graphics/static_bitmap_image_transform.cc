...
#if defined(STANDALONE_RENDERER_GN_PROBE)
SkColorType GetDestSkColorType(scoped_refptr<StaticBitmapImage>) {
...
SkColorType GetDestSkColorType(scoped_refptr<StaticBitmapImage> source) {
  return ToClosestSkColorType(
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::ApplyUsingPixmap(
  ...
  // Allocate the cropped source image.
  {
    ...
    const auto bm_info = SkImageInfo::Make(
        source_rect.size(), GetDestSkColorType(source),
...
scoped_refptr<StaticBitmapImage> StaticBitmapImageTransform::ApplyWithBlit(
  ...
  sk_sp<SkSurface> surface = SkSurfaces::Raster(
      SkImageInfo::Make(dest_size.width(), dest_size.height(),
                        GetDestSkColorType(source), dest_alpha_type,
