...
T* MakeGarbageCollected(Args&&... args) {
  auto* state = ThreadStateStorageFor<ThreadingTrait<T>::kAffinity>::GetState();
  T* result = cppgc::MakeGarbageCollected<T>(
      state->allocation_handle(), std::forward<Args>(args)...);
  return result;
}
