...
is still required for the vcpkg scripts and executable; set `VCPKG_ROOT` if it
is not installed at `C:/vcpkg`. The standalone toolchain wrapper keeps mutable
vcpkg state under the repository by default:
